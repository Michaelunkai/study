...
def run(cmd, cwd=None, timeout=7200):
    try:
        ...
        r = subprocess.run(cmd, shell=True, capture_output=True, text=True,
                           encoding='utf-8', errors='replace', cwd=cwd, env=env, timeout=timeout)
...
def push_with_retry(wd, repo_name):
    ...
    for attempt in range(1, 21):
        log(f"[PUSH] Attempt {attempt}/20...")
        rc, out, err = run("git push origin main --force --no-verify", wd, 7200)
        ...
        if any(x in el for x in ["timeout", "connection", "network", "failed to connect"]):
            log("[!] Network error, retrying in 5s...")
            time.sleep(5)
            continue
