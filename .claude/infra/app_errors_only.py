...
def query_loki(query, start=None, end=None, limit=1000):
    """Query Loki with ERROR-ONLY filter"""
    try:
        # CRITICAL: ERROR-ONLY FILTER - No INFO/DEBUG logs
        error_query = '{job=~".+"} |~ "(?i)error|exception|fail|critical|fatal|database.*crash|database.*error|connection.*refused|timeout|500|internal.*server|psycopg2|sqlalchemy.*error|operational.*error|integrity.*error"'
