    ...
    $job = Start-Job -ScriptBlock {
        ...
        try {
            ...
            if (Test-Path $src -PathType Container) {
                ...
                # /E = include empty subdirs, /R:0 = no retries, /W:0 = no wait, /MT:16 = 16 threads
                # /NFL /NDL /NJH /NJS = suppress file/dir/header/summary logging for speed
                # NOTE: robocopy exit codes 0-7 are SUCCESS (bits: 1=files copied, 2=extras, 4=mismatches)
                # Wrap in try-catch and explicitly return size to avoid exit code leaking
                robocopy $src $dst /E /R:0 /W:0 /MT:16 /NFL /NDL /NJH /NJS /XD "node_modules" ".git" "__pycache__" ".venv" "venv" "browser" "platform-tools" "outbound" "canvas" 2>$null | Out-Null
            } else {
                Copy-Item -Path $src -Destination $dst -Force -ErrorAction SilentlyContinue
            }

            # Always return size, never return robocopy exit code
            if (Test-Path $dst) {
...
function Wait-ParallelJobs {
    param([array]$Jobs, [int]$MaxConcurrent = 16, [int]$TimeoutSeconds = 600)

    $completed = @()
    $startTime = Get-Date
    
    while ($Jobs.Count -gt 0) {
        # TIMEOUT CHECK - never hang forever
        if (((Get-Date) - $startTime).TotalSeconds -gt $TimeoutSeconds) {
            Write-Step "  -> Timeout after $TimeoutSeconds seconds - killing remaining jobs" "WARNING"
            $Jobs | ForEach-Object {
        ...
        if ($elapsed -gt 0 -and $elapsed % 15 -eq 0) {
            Write-Step "  -> $($running.Count) jobs running, $($completed.Count) completed, $($Jobs.Count) remaining..." "INFO"
        }
