...
$os = # Get-CimInstance DISABLED (WMI)
$cs = # Get-CimInstance DISABLED (WMI)
$cpu = # Get-CimInstance DISABLED (WMI)
...
try {
    $disks = # Get-CimInstance DISABLED (WMI)
...
$volumes = # Get-CimInstance DISABLED (WMI) | Where-Object { $_.DriveType -eq 3 }
...
$mem = # Get-CimInstance DISABLED (WMI)
...
# Page File
$pagefile = # Get-CimInstance DISABLED (WMI) -ErrorAction SilentlyContinue
if ($pagefile) {
...
$problemDrivers = # Get-CimInstance DISABLED (WMI) | Where-Object { $_.ConfigManagerErrorCode -ne 0 }
if ($problemDrivers) {
