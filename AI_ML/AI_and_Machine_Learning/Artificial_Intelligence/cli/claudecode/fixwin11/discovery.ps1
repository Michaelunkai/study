...
# Get-CimInstance DISABLED (WMI) | Select-Object Name, NumberOfCores, NumberOfLogicalProcessors | Format-List
# Get-CimInstance DISABLED (WMI) | Select-Object Name, DriverVersion, Status | Format-List
$ramGB = [math]::Round(((# Get-CimInstance DISABLED (WMI) | Measure-Object Capacity -Sum).Sum / 1GB), 2)
Write-Host "Total RAM: $ramGB GB"
# Get-CimInstance DISABLED (WMI) | Select-Object Model, MediaType, Status, @{Name="Size(GB)";Expression={[math]::Round($_.Size/1GB,2)}} | Format-Table -AutoSize

Write-Host "`n--- Network Adapters (DO NOT TOUCH) ---" -ForegroundColor Yellow
# Get-WmiObject DISABLED (WMI) | Where-Object {$_.NetConnectionStatus -ne $null} | Select-Object Name, NetConnectionStatus, @{Name="Driver";Expression={$_.Name}} | Format-Table -AutoSize
