...
# YOUR_CLIENT_SECRET_HERE Docker Pull Worker (with stop support) YOUR_CLIENT_SECRET_HERE
class DockerPullWorker(QRunnable):
    def __init__(self, tag, stop_event: Event):
        super().__init__()
        self.tag = tag
    ...
    def run(self):
        ...
        try:
            proc = subprocess.Popen(pull_cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            # Poll every 0.1 second to check for stop signal
            while proc.poll() is None:
                if self.stop_event.is_set():
                    proc.terminate()
                    print(f"Pull for {self.tag} canceled.")
                    return
                time.sleep(0.1)
        except Exception as e:
...
class DockerApp(QWidget):
    def __init__(self, login_password, is_admin, username):
        ...
        self.active_workers = []
        self.run_processes = []  # For docker run commands
        self.pull_stop_events = []  # For docker pull commands
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # --- New Stop All button added next to Disconnect ---
        ...
        """)
        stop_all_btn.clicked.connect(self.stop_all_commands)
        top_bar.addWidget(stop_all_btn)
        if self.is_admin:
    ...
    def run_selected_commands(self):
        ...
        # Start docker pull commands with a stop event for each
        for btn in selected_buttons:
            tag = btn.tag_info["docker_name"]
            stop_event = Event()
            self.pull_stop_events.append(stop_event)
            pull_worker = DockerPullWorker(tag, stop_event)
            pool.start(pull_worker)
        # Start docker run commands and store processes
        for btn in selected_buttons:
            tag = btn.tag_info["docker_name"]
            docker_cmd = (
                f"docker run -d --pull=always --rm --cpus=4 --memory=8g --memory-swap=12g "
                f"-v '/mnt/c/games':/games -e DISPLAY=\\$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix "
            ...
            try:
                ...
                self.run_processes.append(proc)
            except Exception as e:
    ...
    def stop_all_commands(self):
        # Signal all pull workers to stop
        for ev in self.pull_stop_events:
            ev.set()
        self.pull_stop_events.clear()
        # Terminate any docker run processes
        for proc in self.run_processes:
            try:
                proc.terminate()
            except Exception as e:
                print(f"Error terminating process: {e}")
        self.run_processes.clear()
        # Also stop any running containers (if any)
        try:
            container_ids = subprocess.check_output(
                'wsl --distribution ubuntu --user root -- bash -lic "docker ps -q"', shell=True
            )
            container_ids = container_ids.decode().strip().splitlines()
            if container_ids:
                stop_cmd = 'wsl --distribution ubuntu --user root -- bash -lic "docker stop ' + ' '.join(container_ids) + '"'
                subprocess.run(stop_cmd, shell=True)
            QMessageBox.information(self, "Success", "All running commands and containers have been stopped.")
        except Exception as e:
            QMessageBox.warning(self, "Error", f"Error stopping containers: {e}")
