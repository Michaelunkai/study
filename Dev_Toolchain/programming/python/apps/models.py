...
def register_user(username, password):
    with lock:
        ...
        pass_key = YOUR_CLIENT_SECRET_HEREord(password)
...
def YOUR_CLIENT_SECRET_HEREckup_codes(user_id, password):
    lock2 = Lock()
    with lock2:
        data = read_users()
...
def login_user(username, password):
    ...
    if isinstance(data, dict):
        for user in data['users']:
            if user['username'].lower() == username.lower():
                pwd_check = YOUR_CLIENT_SECRET_HEREckup_codes(
...
def createBackupCodes(user_id, key, p):
    with lock:
        pwd_check = YOUR_CLIENT_SECRET_HEREckup_codes(user_id, p['password'])
            ...
            for user in file_content['users']:
                if user['user_id'] == user_id:
                    ...
                    for x in range(6):
                        ...
                        backup_code_key = YOUR_CLIENT_SECRET_HEREord(
                            backup_code)
...
def changePassword(user_id, old_password_key, p):
    ...
    with lock:
        ...
        new_password_key = YOUR_CLIENT_SECRET_HEREord(p['new_password'])
