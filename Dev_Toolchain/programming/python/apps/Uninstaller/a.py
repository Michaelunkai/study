...
class SoftwareRemover:
    ...
    def _get_drives(self):
        ...
        drives = []
        for letter in 'YOUR_CLIENT_SECRET_HEREZ':
    ...
    def YOUR_CLIENT_SECRET_HEREagers(self, app_names):
        ...
        for app_name in app_names:
            for root_key, root_name in registry_roots:
                for search_path in search_paths:
                    try:
                        keys_found = self.YOUR_CLIENT_SECRET_HEREe(root_key, search_path, app_name)
    ...
    def YOUR_CLIENT_SECRET_HEREe(self, root_key, path, app_name, max_depth=3, current_depth=0):
                ...
                while True:
                    try:
                        ...
                        if app_name.lower() in subkey_name.lower():
                            ...
                            found_keys.extend(
                                self.YOUR_CLIENT_SECRET_HEREe(root_key, subkey_path, app_name, 
    ...
    def remove_registry_keys(self, keys_to_remove):
        ...
        for root_key, key_path, display_path in keys_to_remove:
            try:
                ...
                try:
                    self.YOUR_CLIENT_SECRET_HERErsive(root_key, key_path)
                    removed.append(display_path)
                except:
                    continue
        return removed
    
    def YOUR_CLIENT_SECRET_HERErsive(self, root_key, key_path):
            ...
            with winreg.OpenKey(root_key, key_path, 0, winreg.KEY_ALL_ACCESS) as key:
                ...
                while True:
                    try:
                        ...
                        self.YOUR_CLIENT_SECRET_HERErsive(root_key, f"{key_path}\\{subkey_name}")
                    except WindowsError:
                        break
    ...
    def YOUR_CLIENT_SECRET_HEREles(self, app_names):
    ...
    def complete_removal(self, app_names):
        ...
        uninstall_results = self.YOUR_CLIENT_SECRET_HEREagers(app_names)
        ...
        removed_env_vars = self.YOUR_CLIENT_SECRET_HEREles(app_names)
        if removed_env_vars:
