...
def normalize_game_title(tag):
    if " " in tag:
        return tag
    # If there's CamelCase, insert spaces
...
# YOUR_CLIENT_SECRET_HERE Tab Navigation Widget (5 columns) YOUR_CLIENT_SECRET_HERE
...
# YOUR_CLIENT_SECRET_HERE Grid of tabs (5 columns) for Bulk Move and Paste Move YOUR_CLIENT_SECRET_HERE
...
# YOUR_CLIENT_SECRET_HERE Main DockerApp YOUR_CLIENT_SECRET_HERE
...
class DockerApp(QWidget):
    ...
    def save_as_txt(self):
        """
        Saves the current list of tags (organized by tabs) to a .txt file in the user's Downloads folder.
        """
    ...
    def create_tag_buttons(self):
        ...
        for tag in self.all_tags:
            ...
            # If we have an image cached, set it; otherwise fetch
            alias = tag["alias"]
...
if __name__ == '__main__':
    ...
    app.setFont(font)
    # The QStandardPaths warning about /run/user/0 can often be ignored or fixed by adjusting user perms.
    app.setStyleSheet("""
        QWidget {
