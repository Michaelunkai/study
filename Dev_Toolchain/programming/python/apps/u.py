...
class DockerPullWorker(QRunnable):
    ...
    @pyqtSlot()
    def run(self):
        # Using WSL command as specified
...
class MyLinersDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # BackItUp button
        backitup_btn = QPushButton("BackItUp")
        backitup_btn.setStyleSheet("""
            QPushButton {
                background: #8E44AD;
                color: white;
                padding: 8px 12px;
        ...
        """)
        backitup_btn.clicked.connect(self.run_backitup)
        layout.addWidget(backitup_btn)
        # BigitGo button
        bigitgo_btn = QPushButton("BigiTGo")
        bigitgo_btn.setStyleSheet("""
            QPushButton {
                background: #F39C12;
                color: white;
                padding: 8px 12px;
        ...
        """)
        bigitgo_btn.clicked.connect(self.run_bigitgo)
        layout.addWidget(bigitgo_btn)
        # gg button
        gg_btn = QPushButton("gg")
        ...
        """)
        gg_btn.clicked.connect(self.run_gg)
        layout.addWidget(gg_btn)
        self.setLayout(layout)
    def run_backitup(self):
        cmd = "wsl --distribution ubuntu --user root -- bash -lic 'backitup'"
        try:
            subprocess.Popen(cmd, shell=True)
        except Exception as e:
            QMessageBox.warning(self, "Error", f"Error running backitup: {e}")
    def run_bigitgo(self):
        cmd = "wsl --distribution ubuntu --user root -- bash -lic 'bigitgo'"
        try:
            subprocess.Popen(cmd, shell=True)
        except Exception as e:
            QMessageBox.warning(self, "Error", f"Error running bigitgo: {e}")
    def run_gg(self):
        cmd = "wsl --distribution ubuntu --user root -- bash -lic 'gg'"
        try:
            subprocess.Popen(cmd, shell=True)
        except Exception as e:
            QMessageBox.warning(self, "Error", f"Error running gg: {e}")
...
class DockerApp(QWidget):
    ...
    def init_ui(self):
        ...
        if self.is_admin:
            ...
            # New "myLiners" button visible only to admin
    ...
    def open_myliners(self):
        # Only admin can see; open MyLinersDialog
