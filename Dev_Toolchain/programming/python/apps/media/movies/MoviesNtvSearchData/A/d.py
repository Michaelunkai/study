...
TMDB_API_KEY = "YOUR_CLIENT_SECRET_HEREb681f11"
TMDB_ACCESS_TOKEN = "eyJhbGciOiJIUzI1NiJ9.YOUR_CLIENT_SECRET_HEREYOUR_CLIENT_SECRET_HEREYOUR_CLIENT_SECRET_HEREYOUR_CLIENT_SECRET_HEREYOUR_CLIENT_SECRET_HEREYOUR_CLIENT_SECRET_HEREYOUR_CLIENT_SECRET_HERExfQ.YOUR_CLIENT_SECRET_HEREv-zlXWYLWB4iYtH-qo"
...
def YOUR_CLIENT_SECRET_HEREent(item_id, media_type):
...
class DetailTabs(QtWidgets.QTabWidget):
    ...
    def init_tabs(self):
        ...
        similar_widget = self.YOUR_CLIENT_SECRET_HEREb()
    ...
    def YOUR_CLIENT_SECRET_HEREb(self):
        """Enhanced similar content tab with better recommendations."""
        # Get enhanced similar content
        similar_content = YOUR_CLIENT_SECRET_HEREent(self.details.get("id"), self.media_type)
...
class TrendingContentWidget(QtWidgets.QWidget):
    ...
    def load_content(self):
        ...
        class MovieWorker(QtCore.QObject):
            ...
            def run(self):
                results = get_trending_movies(limit=200)  # modified: increased limit to 200 titles
        ...
        class TvWorker(QtCore.QObject):
            ...
            def run(self):
                results = get_trending_tv_shows(limit=200)  # modified: increased limit to 200 titles
                self.finished.emit(results)
    ...
    def create_content_card(self, item, media_type):
        ...
        add_btn.setStyleSheet("font-size:10px; background-color:#e50914; color:white; border:none; border-radius:3px;")
        add_btn.clicked.connect(lambda: self.add_to_list(item, media_type))
    ...
    def add_to_list(self, item, media_type):
        ...
        if any(w.get("id") == item.get("id") and w.get("media_type") == media_type for w in watchlist):
            QtWidgets.QMessageBox.information(self, "Info", "This item is already in your list.")
        else:
            new_item = {
                "id": item.get("id"),
                "media_type": media_type,
            ...
            QtWidgets.QMessageBox.information(self, "Added", f"Added {title} to your list.")
    
    def filter_content(self, filters):
        """Filter trending movies and TV shows based on provided filters.
           filters dict: { "genres": [genre_ids], "year_from": int, "year_to": int, "min_rating": int }"""
        filtered_movies = [m for m in self.trending_movies if 
            (
                (not filters.get("genres") or set(filters["genres"]).intersection(set(m.get("genre_ids", [])))) and
                (not m.get("release_date") or (int(m.get("release_date", "0")[:4]) >= filters["year_from"] and int(m.get("release_date", "0")[:4]) <= filters["year_to"])) and
                (m.get("vote_average", 0) >= filters["min_rating"])
            )
        ]
        filtered_tv = [t for t in self.trending_tv_shows if 
            (
                (not filters.get("genres") or set(filters["genres"]).intersection(set(t.get("genre_ids", [])))) and
                (not t.get("first_air_date") or (int(t.get("first_air_date", "0")[:4]) >= filters["year_from"] and int(t.get("first_air_date", "0")[:4]) <= filters["year_to"])) and
                (t.get("vote_average", 0) >= filters["min_rating"])
            )
        ]
        self.populate_movies(filtered_movies)
        self.populate_tv_shows(filtered_tv)
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def open_watchlist_dialog(self):
        ...
        dialog.item_selected.connect(self.YOUR_CLIENT_SECRET_HEREd)
        dialog.exec_()
    
    def YOUR_CLIENT_SECRET_HEREd(self, item_data):
    ...
    def apply_content_filters(self, filters):
        ...
        QtWidgets.QMessageBox.information(
            self,
            "Filters Applied",
            "Your filters have been applied to trending content!"
        )
        ...
        """)
        if hasattr(self, 'trending_widget'):
            self.trending_widget.filter_content(filters)
