...
from PyQt5.QtGui import QFont, QDrag, QCursor, QPalette, QColor
from PyQt5.QtCore import Qt, QTimer, QThread, pyqtSignal, QMimeData, QPoint
...
# Global persistent settings: { docker_name: {"alias": ..., "category": ...} }
persistent_settings = load_settings()

# --- Worker Thread to Fetch Game Time (non-blocking) ---
class GameTimeWorker(QThread):
    finished = pyqtSignal(str, str)  # Emits (alias, time_info)
    ...
    def run(self):
        time_val = ""
        try:
            results = HowLongToBeat().search(self.alias)
            if results:
            ...
            print(f"Error searching howlongtobeat for '{self.alias}': {e}")
        self.finished.emit(self.alias, time_val)

# --- Helper function to parse time strings ---
def parse_time(time_str):
    if not time_str:
        return 0.0
    token = time_str.split()[0].replace('+','')
    if '-' in token:
        try:
...
# (Renaming on Docker Hub isn’t supported; we update only the local display name.)
    ...
    def mouseMoveEvent(self, event):
        if event.buttons() & Qt.LeftButton:
            if (event.pos() - self._drag_start_pos).manhattanLength() >= self.dragThreshold:
                mimeData = QMimeData()
                # Use docker_name for persistence.
        ...
        if action == change_action:
            ...
            if ok and new_alias:
                ...
                if update_docker_tag_name(old_alias, new_alias):
                    ...
                    worker = GameTimeWorker(new_alias)
                    if main_window and hasattr(main_window, "handle_game_time_update"):
                        worker.finished.connect(main_window.handle_game_time_update)
                    worker.start()
...
class DockerApp(QWidget):
    def __init__(self):
        ...
        # For each tag, set docker_name, alias, and category from persistent settings.
    ...
    def init_ui(self):
        ...
        # Connect tab change signal to check MyBackup access.
    ...
    def create_tag_buttons(self):
        ...
        for tag in self.all_tags:
            ...
            container = {
                "all": self.all_container,
                "finished": self.finished_container,
                "mybackup": self.mybackup_container,
                "not_for_me": self.not_for_me_container
            }.get(cat, self.all_container)
            row, col = pos[cat]
            ...
            if col == 4:
                ...
                row += 1
            pos[cat] = [row, col]
        # Launch game time queries after UI load.
    def start_game_time_queries(self):
        for tag in self.all_tags:
            alias = tag["alias"]
            if alias not in self.game_times_cache:
                worker = GameTimeWorker(alias)
                worker.finished.connect(self.handle_game_time_update)
                worker.start()
    ...
    def on_tab_changed(self, index):
        # When switching to the "MyBackup" tab, force dockerhub password entry (only once per run)
