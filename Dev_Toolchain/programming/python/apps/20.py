...
# Default tab configuration; note that the tab "all" is always present.
DEFAULT_TABS_CONFIG = [
    {"id": "all", "name": "All"},
...
class WorkerSignals(QObject):
    finished = pyqtSignal(object)  # emits a tuple or any object
...
# YOUR_CLIENT_SECRET_HERE Functions for fetching game time, image, and date YOUR_CLIENT_SECRET_HERE
...
# Dummy function for updating Docker Hub tag name (only updates locally)
...
# Helper to parse ISO date strings (removes trailing "Z" if present)
...
class TagContainerWidget(QWidget):
    def __init__(self, type_name, parent=None):
        ...
        self.type_name = type_name  # this will match the tab's id
...
class DockerApp(QWidget):
    def __init__(self):
        ...
        # Initialize each tag's persistent alias, category, and last_updated.
        for tag in self.all_tags:
        ...
        self.tabs_config = load_tabs_config()  # Load dynamic tabs configuration
        self.tab_containers = {}  # Will hold TagContainerWidget instances keyed by tab id
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # ------------------ Create QTabWidget and add dynamic tabs ------------------
    ...
    def sort_tags_by_time(self, descending=True):
        # Assuming fetch_game_time returns a float in the time string; extract the number.
    ...
    def sort_tags_by_date(self, descending=True):
        # Sort based on the "last_updated" field. Tags without a date will be treated as very old.
    ...
    def add_tab(self):
        ...
        if any(tab["id"] == new_id for tab in self.tabs_config):
            ...
            return
        token = self.docker_authenticate()
        if not token:
            return
