...
try {
    ...
    while ($monitoring) {
        ...
        if ($gameProcesses.Count -eq 0) {
            Write-Host "Game processes confirmed closed." -ForegroundColor Green
            $monitoring = $false
        } else {
            Write-Host "Game still running. Checking again in 15 seconds..." -ForegroundColor Yellow
            Start-Sleep -Seconds 15
        }
    }
    
    # Double-check to make absolutely sure the game is closed
    Start-Sleep -Seconds 3
    $finalCheck = Get-Process -Name "eldenring", "start_protected_game" -ErrorAction SilentlyContinue
    if ($finalCheck.Count -gt 0) {
        Write-Host "WARNING: Game processes detected again. Waiting for complete closure..." -ForegroundColor Red
        while ($finalCheck.Count -gt 0) {
            Start-Sleep -Seconds 5
            $finalCheck = Get-Process -Name "eldenring", "start_protected_game" -ErrorAction SilentlyContinue
        }
        Write-Host "Game processes confirmed closed after additional checks." -ForegroundColor Green
    }
    
    # Now it's 100% safe to run sg and continue
    Write-Host "Running sg function now that game is completely closed..." -ForegroundColor Cyan
    sg
    ...
    ########################################################
    # 8) Post-game: Run backup executable and press 1
    ########################################################
