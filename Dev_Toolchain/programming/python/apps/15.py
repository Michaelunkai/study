...
# A styled button class (used in control layouts)
...
# Button used for each tag in the grid.
class GameButton(QPushButton):
    def __init__(self, text, parent=None):
        super().__init__(text, parent)
        # Make the button checkable so you can select it.
...
# Dialog to delete tags (unchanged from previous version)
class DeleteTagDialog(QDialog):
    ...
    def delete_selected(self):
        ...
        username = "michadockermisha"  # Replace with your username
...
class DockerApp(QWidget):
    def __init__(self):
        super().__init__()
        # Fetch all tags (with full_size) from Docker Hub (handles pagination)
    ...
    def run_selected_commands(self):
        ...
        for btn in selected_buttons:
            ...
            # Launch command concurrently.
            proc = subprocess.Popen(docker_command, shell=True)
            processes.append((tag, proc))
        # Disable the Run Selected button until all processes finish.
        sender = self.sender()
        sender.setEnabled(False)
        # Use a QTimer to poll the processes.
