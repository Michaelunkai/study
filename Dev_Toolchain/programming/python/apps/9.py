...
class DockerApp(QWidget):
    def __init__(self):
        super().__init__()
        # Fetch every tag available from the repository
    ...
    def fetch_tags(self):
        """
        Fetch every tag available from the Docker Hub repository.
        Adjust the URL or pagination as needed.
        """
        url = "https://hub.docker.com/v2/repositories/michadockermisha/backup/tags?page_size=100"
        try:
            response = requests.get(url)
            data = response.json()
            # Extract tag names from the JSON results
    ...
    def create_tag_buttons(self):
        ...
        self.buttons = []
        row, col = 0, 0
        # Create a button for every tag
        for tag in self.all_tags:
            button = GameButton(tag)
            # When clicked, queue the docker command for the tag
            button.clicked.connect(lambda checked, t=tag: self.queue_docker_command(t))
    ...
    def queue_docker_command(self, tag):
        """
        Place the Docker command for the given tag into a download queue.
        The process_downloads method will run up to 3 commands in parallel.
        """
        docker_command = (
            f'docker run '
            f'--rm '
