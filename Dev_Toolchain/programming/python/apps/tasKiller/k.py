...
def calculate_similarity(term, name):
    ...
    word_ratio = len(common_words) / max(len(term_words), 1) if term_words else 0
...
def search_processes(processes, search_term):
    ...
    for proc in processes:
        ...
        if similarity > 0.4 or cmdline_match:  # Lower threshold for better matches
...
def find_executable_paths(search_term, min_similarity=0.4, max_results=10):
    """
        ...
        for pattern in ['*.exe', '*/*.exe', '*/*/*.exe']:
            try:
                for exe_path in glob.glob(os.path.join(base_path, pattern)):
                    ...
                    # More lenient matching for applications
...
def run_application(search_terms):
    ...
    found_any = False
    for term in terms:
        # Find all matches with similarity > 0.4
        matches = find_executable_paths(term, min_similarity=0.4, max_results=10)
