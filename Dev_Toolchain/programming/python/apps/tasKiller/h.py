...
def run_application(search_term):
    """
    Searches for and runs an application based on a partial name match.
    """
    common_paths = [
        ...
        'C:\\Games\\',  # Add more paths as needed
        'D:\\Games\\',
        'E:\\Games\\'
    ]
            ...
            for file in files:
                if file.lower().endswith('.exe'):
                    if search_term_lower in file.lower():
                        ...
                        similarity = difflib.SequenceMatcher(None, search_term_lower, file.lower()).ratio()
                        found_executables.append((similarity, full_path, file))

    if not found_executables:
        print(f"{YELLOW}{BOLD}No matching applications found for '{search_term}'{RESET}")
        return

    found_executables.sort(reverse=True)  # Sort by similarity
    
    print(f"\n{CYAN}{BOLD}Found these applications:{RESET}")
    for idx, (_, path, name) in enumerate(found_executables[:5], 1):
        print(f"{idx}. {name} ({path})")
    
    choice = input(f"\n{BOLD}Enter number to run (or press Enter to cancel): {RESET}").strip()
    
    if choice.isdigit() and 1 <= int(choice) <= len(found_executables[:5]):
        selected = found_executables[int(choice)-1]
        try:
            print(f"{GREEN}{BOLD}Starting {selected[2]}...{RESET}")
            subprocess.Popen(selected[1])
        except Exception as e:
            print(f"{RED}{BOLD}Error running application: {e}{RESET}")
...
def main():
    ...
    while True:
        ...
        print(f"{BOLD}[S] Search and kill processes{RESET}")
        print(f"{BOLD}[U] Run application{RESET}")
        ...
        if command == "e":
            ...
            if search_term:
                search_and_kill(processes_sorted, search_term)
        elif command == "u":
            app_name = input(f"{BOLD}Enter application name to run: {RESET}").strip()
            if app_name:
