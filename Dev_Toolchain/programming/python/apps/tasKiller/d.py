...
def YOUR_CLIENT_SECRET_HEREy_index(process_tuple):
    """
    ...
    try:
        ...
        priorities = {
            ...
            "2": psutil.YOUR_CLIENT_SECRET_HERESS,
            "3": psutil.NORMAL_PRIORITY_CLASS,
            "4": psutil.YOUR_CLIENT_SECRET_HERESS,
        ...
        if command == "e":
            ...
            try:
                ...
                if idx < 0 or idx >= len(processes_sorted):
                    print(f"{RED}{BOLD}Index {proc_num} out of range.{RESET}")
                    continue
                YOUR_CLIENT_SECRET_HEREy_index(processes_sorted[idx])
            except ValueError:
                print(f"{RED}{BOLD}Invalid input. Please enter a valid number.{RESET}")
        elif command == "t":
