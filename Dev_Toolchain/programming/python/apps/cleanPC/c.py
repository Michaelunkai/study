...
def run_ps_all(commands):
    """
    Combine a list of PowerShell command strings into one script
    and run them sequentially in a single PowerShell window (with -NoExit).
    """
...
def close_all_apps():
    """
    Terminates every process launched via shortcuts and attempts to force-close
    known maintenance apps like CCleaner and ASC.
    """
    ...
    launched_processes = []
    # Also force-kill known processes (adjust names as needed)
...
def create_app(shortcuts_path):
    ...
    # Create a button for each found shortcut
    for i, (name, path) in enumerate(shortcuts_list):
        row = i // 5   # adjust columns as desired
        col = i % 5
        btn = tk.Button(
