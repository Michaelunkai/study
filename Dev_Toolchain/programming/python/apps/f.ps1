<#
    ...
    This script:
    1. Launches Elden Ring and minimizes the game window
    2. Waits 5 seconds, then runs rmod.exe and sends the '1' keypress
    3. Sets up keyboard shortcuts for Ctrl+S (suspend game) and Ctrl+R (resume game)
    4. Runs the 'desk' function after 10 seconds
    5. After game exit, launches save management tools
#>
...
############################################################
# 4) Launch Elden Ring, minimize, and run rmod after 5 seconds
############################################################
...
Write-Host "Minimizing Elden Ring window..." -ForegroundColor Yellow
Add-Type -AssemblyName System.Windows.Forms
Start-Sleep -Seconds 2 # Wait briefly to ensure the window is available
$hwnd = (Get-Process -Name "start_protected_game", "eldenring" -ErrorAction SilentlyContinue | Select-Object -First 1).MainWindowHandle
if ($hwnd -ne 0) {
    [System.Windows.Forms.Form]::WindowState = 'Minimized'
    [void][System.Runtime.InteropServices.Win32.ShowWindow]($hwnd, 6) # 6 = SW_MINIMIZE
    Write-Host "Game window minimized." -ForegroundColor Green
} else {
    Write-Host "Failed to minimize game window: Window handle not found." -ForegroundColor Red
}

# Wait 5 seconds, then run rmod.exe and send '1' keypress
Write-Host "Waiting 5 seconds before running rmod..." -ForegroundColor Yellow
