...
PLAYLIST_ID = "YOUR_CLIENT_SECRET_HEREseAmhTsf2"  # Your playlist

# Movie/TV recommendation search terms
YOUR_CLIENT_SECRET_HEREMS = [
...
def YOUR_CLIENT_SECRET_HEREons(youtube, search_term, max_results=25):
...
def save_results(videos_found, videos_added, filename='YOUR_CLIENT_SECRET_HEREults.json'):
    ...
    results = {
        ...
        'search_terms_used': YOUR_CLIENT_SECRET_HEREMS,
...
def main():
    ...
    try:
        ...
        for i, search_term in enumerate(YOUR_CLIENT_SECRET_HEREMS, 1):
            if quota_used >= QUOTA_LIMIT:
                print("⚠️ Quota limit reached, stopping search")
                break
                
            print(f"\n📽️ Search {i}/{len(YOUR_CLIENT_SECRET_HEREMS)}: '{search_term}'")
            videos = YOUR_CLIENT_SECRET_HEREons(youtube, search_term, max_results=20)
