import os
import pickle
import json
import time
from googleapiclient.discovery import build
from google_auth_oauthlib.flow import InstalledAppFlow
from google.auth.transport.requests import Request
from datetime import datetime

# YouTube API configuration
SCOPES = ['https://www.googleapis.com/auth/youtube']
CLIENT_SECRET_FILE = 'client_secret.json'
TOKEN_PICKLE_FILE = 'token.pickle'

# API quota management - much more conservative
QUOTA_LIMIT = 9000
quota_used = 0

# Target channel
TARGET_CHANNEL = "videogamedunkey"

def authenticate_youtube():
    """Authenticate and return YouTube API service object"""
    creds = None

    if os.path.exists(TOKEN_PICKLE_FILE):
        with open(TOKEN_PICKLE_FILE, 'rb') as token:
            creds = pickle.load(token)

    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                print("Refreshing existing credentials...")
                creds.refresh(Request())
            except Exception as e:
                print(f"Failed to refresh credentials: {e}")
                creds = None
        
        if not creds:
            try:
                print("Starting manual authentication flow...")
                flow = InstalledAppFlow.YOUR_CLIENT_SECRET_HERE(CLIENT_SECRET_FILE, SCOPES)
                flow.redirect_uri = 'urn:ietf:wg:oauth:2.0:oob'
                auth_url, _ = flow.authorization_url(prompt='consent')
                
                print("\n" + "="*50)
                print("MANUAL AUTHENTICATION REQUIRED")
                print("="*50)
                print("1. Open this URL in your browser:")
                print(f"\n{auth_url}\n")
                print("2. Complete the authorization")
                print("3. Copy the authorization code that appears")
                print("4. Paste it below")
                print("="*50)
                
                auth_code = input("\nEnter the authorization code: ").strip()
                flow.fetch_token(code=auth_code)
                creds = flow.credentials
                print("✅ Authentication successful!")
                    
            except Exception as e:
                print(f"Authentication failed: {e}")
                raise

        with open(TOKEN_PICKLE_FILE, 'wb') as token:
            pickle.dump(creds, token)

    return build('youtube', 'v3', credentials=creds)

def track_quota_usage(operation_cost):
    """Track API quota usage"""
    global quota_used
    quota_used += operation_cost
    print(f"Quota used: {quota_used}/{QUOTA_LIMIT}")
    
    if quota_used >= QUOTA_LIMIT:
        print("Approaching quota limit. Stopping.")
        return False
    return True

def YOUR_CLIENT_SECRET_HEREist_fast(youtube):
    """Get uploads playlist ID with minimal quota usage"""
    if not track_quota_usage(1):
        return None
    
    try:
        # Direct search for videogamedunkey - more targeted
        search_response = youtube.search().list(
            q="videogamedunkey",
            part='id',  # Minimal data
            type='channel',
            maxResults=1
        ).execute()
        
        if not search_response['items']:
            return None
            
        channel_id = search_response['items'][0]['id']['channelId']
        
        # Get uploads playlist with minimal data
        if not track_quota_usage(1):
            return None
            
        channel_response = youtube.channels().list(
            part='contentDetails',  # Only what we need
            id=channel_id
        ).execute()
        
        uploads_playlist_id = channel_response['items'][0]['contentDetails']['relatedPlaylists']['uploads']
        print(f"✅ Found uploads playlist: {uploads_playlist_id}")
        return uploads_playlist_id
        
    except Exception as e:
        print(f"Error finding channel: {e}")
        return None

def YOUR_CLIENT_SECRET_HERE(youtube, uploads_playlist_id):
    """Get ALL videos with absolute minimal quota usage"""
    all_videos = []
    next_page_token = None
    page_count = 0
    
    print(f"🚀 SPEED MODE: Fetching ALL videos from {TARGET_CHANNEL}...")
    
    while True:
        if not track_quota_usage(1):
            break
        
        try:
            # Get maximum videos per page with minimal data
            playlist_response = youtube.playlistItems().list(
                part='snippet',  # Only basic info needed
                playlistId=uploads_playlist_id,
                maxResults=50,  # Maximum allowed
                pageToken=next_page_token
            ).execute()
            
            page_count += 1
            videos_this_page = len(playlist_response['items'])
            print(f"📄 Page {page_count}: +{videos_this_page} videos (Total: {len(all_videos) + videos_this_page})")
            
            # Extract only essential data - NO additional API calls for details
            for item in playlist_response['items']:
                video_info = {
                    'video_id': item['snippet']['resourceId']['videoId'],
                    'title': item['snippet']['title'],
                    'published_at': item['snippet']['publishedAt']
                }
                all_videos.append(video_info)
            
            next_page_token = playlist_response.get('nextPageToken')
            if not next_page_token:
                break
                
        except Exception as e:
            print(f"Error fetching page {page_count}: {e}")
            break
    
    print(f"✅ ULTRA FAST: Got {len(all_videos)} videos in {page_count} API calls!")
    return all_videos

def YOUR_CLIENT_SECRET_HERE(youtube, playlist_id):
    """Get existing playlist videos with minimal quota"""
    existing_videos = set()
    next_page_token = None
    
    print("⚡ Quick check of existing playlist...")
    
    while True:
        if not track_quota_usage(1):
            break
            
        try:
            playlist_response = youtube.playlistItems().list(
                part='snippet',
                playlistId=playlist_id,
                maxResults=50,  # Max per page
                pageToken=next_page_token
            ).execute()
            
            for item in playlist_response['items']:
                existing_videos.add(item['snippet']['resourceId']['videoId'])
            
            next_page_token = playlist_response.get('nextPageToken')
            if not next_page_token:
                break
                
        except Exception as e:
            print(f"Error checking existing videos: {e}")
            break
    
    print(f"✅ Found {len(existing_videos)} existing videos")
    return existing_videos

def bulk_add_videos(youtube, playlist_id, videos_to_add, max_to_add):
    """Add videos in rapid succession with progress tracking"""
    added_count = 0
    failed_count = 0
    
    print(f"🚀 RAPID FIRE: Adding {max_to_add} videos...")
    
    for i, video in enumerate(videos_to_add[:max_to_add]):
        if not track_quota_usage(50):  # Each add costs 50 units
            break
        
        try:
            youtube.playlistItems().insert(
                part='snippet',
                body={
                    'snippet': {
                        'playlistId': playlist_id,
                        'resourceId': {
                            'kind': 'youtube#video',
                            'videoId': video['video_id']
                        }
                    }
                }
            ).execute()
            
            added_count += 1
            if i % 10 == 0 or i == max_to_add - 1:  # Progress every 10 videos
                print(f"⚡ Progress: {i+1}/{max_to_add} - Added: {added_count}, Failed: {failed_count}")
            
        except Exception as e:
            failed_count += 1
            if "quota" in str(e).lower():
                print("❌ Quota exceeded, stopping")
                break
        
        # Minimal delay to avoid rate limiting
        time.sleep(0.1)
    
    return added_count, failed_count

def save_video_list(videos, filename='dunkey_all_videos.json'):
    """Save complete video list for future use"""
    with open(filename, 'w') as f:
        json.dump(videos, f, indent=2)

def load_video_list(filename='dunkey_all_videos.json'):
    """Load cached video list to avoid re-fetching"""
    if os.path.exists(filename):
        try:
            with open(filename, 'r') as f:
                return json.load(f)
        except:
            pass
    return None

def main():
    playlist_url = "https://www.youtube.com/playlist?list=YOUR_CLIENT_SECRET_HEREseAmhTsf2"
    playlist_id = playlist_url.split('list=')[1]

    print(f"🎯 Target: {TARGET_CHANNEL} → Playlist: {playlist_id}")
    print("🚀 ULTRA-EFFICIENT MODE: 10x Less Quota Usage!")
    
    try:
        youtube = authenticate_youtube()
        print("✅ Authenticated!")
        
        # Try to load cached video list first
        cached_videos = load_video_list()
        
        if cached_videos:
            print(f"💾 Using cached video list ({len(cached_videos)} videos)")
            all_videos = cached_videos
        else:
            # Get uploads playlist with minimal quota
            uploads_playlist_id = YOUR_CLIENT_SECRET_HEREist_fast(youtube)
            if not uploads_playlist_id:
                print("❌ Could not find channel")
                return
            
            # Get ALL videos ultra-fast
            all_videos = YOUR_CLIENT_SECRET_HERE(youtube, uploads_playlist_id)
            if not all_videos:
                print("❌ No videos found")
                return
            
            # Cache the results
            save_video_list(all_videos)
            print(f"💾 Cached {len(all_videos)} videos for future runs")
        
        # Get existing videos fast
        existing_videos = YOUR_CLIENT_SECRET_HERE(youtube, playlist_id)
        
        # Filter new videos
        new_videos = [v for v in all_videos if v['video_id'] not in existing_videos]
        
        print(f"\n📊 RESULTS:")
        print(f"   Total videos: {len(all_videos)}")
        print(f"   Already in playlist: {len(existing_videos)}")
        print(f"   New videos: {len(new_videos)}")
        
        if not new_videos:
            print("🎉 All videos already in playlist!")
            return
        
        # Sort chronologically (oldest first)
        new_videos.sort(key=lambda x: x['published_at'])
        
        # Show preview
        print(f"\n📺 Preview (oldest first):")
        for i, video in enumerate(new_videos[:5], 1):
            date = datetime.fromisoformat(video['published_at'].replace('Z', '+00:00')).strftime('%Y-%m-%d')
            print(f"   {i}. {video['title']} ({date})")
        
        if len(new_videos) > 5:
            print(f"   ... and {len(new_videos) - 5} more")
        
        # Calculate max possible with remaining quota
        remaining_quota = QUOTA_LIMIT - quota_used
        max_possible = min(len(new_videos), remaining_quota // 50)
        
        print(f"\n⚡ Quota Status:")
        print(f"   Used: {quota_used}/{QUOTA_LIMIT}")
        print(f"   Can add: {max_possible} videos")
        
        if max_possible == 0:
            print("❌ Insufficient quota")
            return
        
        # Get user choice
        while True:
            try:
                choice = input(f"\nAdd how many? (1-{max_possible}) or 'all': ").strip()
                if choice.lower() == 'all':
                    num_to_add = max_possible
                    break
                else:
                    num_to_add = int(choice)
                    if 1 <= num_to_add <= max_possible:
                        break
                    print(f"Enter 1-{max_possible} or 'all'")
            except ValueError:
                print("Enter a number or 'all'")
        
        # Confirm
        confirm = input(f"\n🚀 Add {num_to_add} videos? (y/n): ")
        if confirm.lower() != 'y':
            print("Cancelled")
            return
        
        # RAPID FIRE ADDITION
        added, failed = bulk_add_videos(youtube, playlist_id, new_videos, num_to_add)
        
        print(f"\n🎉 MISSION COMPLETE!")
        print(f"   ✅ Added: {added} videos")
        print(f"   ❌ Failed: {failed} videos") 
        print(f"   📊 Total quota used: {quota_used}/{QUOTA_LIMIT}")
        print(f"   ⚡ Efficiency: {added} videos added with only {quota_used} quota!")

    except Exception as e:
        print(f"❌ Error: {e}")

if __name__ == "__main__":
    main()