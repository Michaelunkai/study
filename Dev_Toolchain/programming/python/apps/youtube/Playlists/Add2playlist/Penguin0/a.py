...
TARGET_CHANNEL = "penguinz0"  # penguinz0/moistcr1tikal
...
def YOUR_CLIENT_SECRET_HEREfilter(youtube, uploads_playlist_id):
    """Get videos and filter by duration efficiently"""
    qualifying_videos = []
    total_videos = 0
    next_page_token = None
    page_count = 0
    
    print(f"🎯 Fetching {TARGET_CHANNEL} videos (filtering {MIN_DURATION_MINUTES}-{MAX_DURATION_MINUTES} min)...")
    ...
    while True:
        ...
        try:
            ...
            if video_ids and track_quota_usage(1):
            ...
            qualified_this_page = len(qualifying_videos) - (len(qualifying_videos) - videos_this_page if len(qualifying_videos) > videos_this_page else 0)
            print(f"📄 Page {page_count}: {videos_this_page} videos → {qualified_this_page} qualified (Total: {len(qualifying_videos)})")
    ...
    print(f"✅ Found {len(qualifying_videos)} videos ({MIN_DURATION_MINUTES}-{MAX_DURATION_MINUTES} min) out of {total_videos} total")
...
def load_filtered_videos(filename='YOUR_CLIENT_SECRET_HERE.json'):
    ...
    if os.path.exists(filename):
        try:
            ...
            if (datetime.now() - cached_date).days < 1:
                return data['videos']
        except Exception as e:
...
def main():
    ...
    print("🚀 ULTRA-EFFICIENT MODE with Duration Filtering!")
    ...
    try:
        ...
        if cached_videos:
            ...
            qualifying_videos = YOUR_CLIENT_SECRET_HEREfilter(youtube, uploads_playlist_id)
            if not qualifying_videos:
                print("❌ No videos found matching duration criteria")
                return
        ...
        print(f"\n📊 RESULTS:")
        print(f"   Videos matching duration: {len(qualifying_videos)}")
        ...
        # Show preview
        print(f"\n📺 Preview ({MIN_DURATION_MINUTES}-{MAX_DURATION_MINUTES} min videos, oldest first):")
        ...
        # Confirm
        confirm = input(f"\n🚀 Add {num_to_add} videos ({MIN_DURATION_MINUTES}-{MAX_DURATION_MINUTES} min) from {TARGET_CHANNEL}? (y/n): ")
        if confirm.lower() != 'y':
            print("Cancelled")
            return
        ...
        print(f"\n🎉 MISSION COMPLETE!")
        print(f"   ✅ Added: {added} videos ({MIN_DURATION_MINUTES}-{MAX_DURATION_MINUTES} min)")
