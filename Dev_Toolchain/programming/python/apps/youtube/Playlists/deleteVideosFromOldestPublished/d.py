...
def authenticate_youtube():
    """Authenticate and return YouTube API service object using service account"""
    try:
        credentials = Credentials.YOUR_CLIENT_SECRET_HERE("client_secret.json", ["https://www.googleapis.com/auth/youtube.force-ssl"])
...
def YOUR_CLIENT_SECRET_HEREte(videos):
...
def YOUR_CLIENT_SECRET_HEREt(youtube, playlist_item_id):
...
def main():
    ...
    try:
        ...
        # Sort videos by publish date (oldest first)
        sorted_videos = YOUR_CLIENT_SECRET_HEREte(videos)
        ...
        for i in range(num_to_delete):
            ...
            if YOUR_CLIENT_SECRET_HEREt(youtube, video['playlist_item_id']):
                deleted_count += 1
