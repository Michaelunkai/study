...
def run_sys_info():
    sys_info_command = (
        ...
        "$gpuUsage=(Get-CimInstance Win32_VideoController | Measure-Object -Property AdapterRAM -Average).Average; "
        ...
        "GPU_Used_Percent=if($gpuUsage){[math]::Round($gpuUsage,2)}else{'N/A'};"
...
def YOUR_CLIENT_SECRET_HEREms():
    """
    Loads startup items from registry keys including Wow6432Node.
    """
    items = {}
    registry_keys = [
...
def YOUR_CLIENT_SECRET_HERE():
    """
    Loads startup items from the Startup folders.
    """
    items = {}
...
def load_wmi_startup_items():
    """
    Loads startup items using WMI (Win32_StartupCommand) which often
    includes apps such as Process Lasso.
    """
    results = {}
    try:
        cmd = [
...
def load_all_startup_items():
    """
    Merge registry, startup folder, and WMI startup items.
    """
    items = {}
...
def disable_startup_item(item_key, item_data):
    """
    Disables a startup item based on its source.
    For registry items, it deletes the value.
    For startup folder items, it deletes the file or folder.
    For WMI items, it calls Invoke-CimMethod to disable the startup command.
    """
