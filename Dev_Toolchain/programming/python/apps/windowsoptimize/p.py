...
def run_command_as_admin(command_or_list):
    ...
    try:
        if sys.platform.startswith('win'):
            # Check if user is already elevated
...
# YOUR_CLIENT_SECRET_HEREYOUR_CLIENT_SECRET_HERE---------
def YOUR_CLIENT_SECRET_HEREms():
    """
    Load startup items from various registry keys including
    Wow6432Node for 32-bit apps on 64-bit Windows.
    """
    items = {}
    registry_keys = [
...
def load_wmi_startup_items():
    """
    Load startup commands from WMI (Win32_StartupCommand).
    This typically includes additional items visible in Task Manager.
    """
    results = {}
    try:
        cmd = [
            ...
            "-Command",
            "(Get-CimInstance -ClassName Win32_StartupCommand | Select-Object Name, Command, Location) | ConvertTo-Json -Compress"
        ]
        ...
        if isinstance(items, dict):
            items = [items]
        for item in items:
            key = f"WMI:{item.get('Name', 'Unknown')}"
            results[key] = {
                "source": "wmi",
        ...
        print("Error loading WMI startup items:", e)
    return results

def load_all_startup_items():
    """
    Merge registry, startup folder, and WMI items so that
    every app that runs on startup is displayed.
    """
    items = {}
    items.update(YOUR_CLIENT_SECRET_HEREms())
    items.update(YOUR_CLIENT_SECRET_HERE())
    items.update(load_wmi_startup_items())
    return items
...
def is_protected_startup(item_data):
    ...
    if src == "registry":
        ...
        text_to_check = f"{item_data['name']} {item_data.get('command', '')} {item_data.get('location', '')}"
...
def disable_startup_item(item_key, item_data):
    """
    Attempts to disable the startup item, whether registry, startup folder, or WMI.
    """
    reply = QMessageBox.question(
        None,
        "Confirm Disable",
        f"Are you sure you want to disable startup item '{item_data['name']}'?\nDisabling a critical item may cause issues.",
    ...
    if src == "registry":
        ...
        try:
            ...
            if os.path.isdir(path):
                ...
                # If it's a file (shortcut, etc.), remove it
                os.remove(path)
        ...
        # Attempt to disable the WMI-based startup item
        wmi_name = item_data.get("name", "")
        if not wmi_name:
            QMessageBox.warning(None, "Error", "Missing WMI name; cannot disable.")
            return
        # We'll run a PowerShell command that calls Invoke-CimMethod -MethodName Disable
        # for the Win32_StartupCommand item whose Name matches wmi_name.
        cmd = [
            "powershell",
            "-NoProfile",
            "-ExecutionPolicy", "Bypass",
            "-Command",
            f"Get-CimInstance Win32_StartupCommand | Where-Object {{ $_.Name -eq '{wmi_name}' }} | Invoke-CimMethod -MethodName Disable"
        ]
        try:
            run_command_as_admin(cmd)
            QMessageBox.information(None, "Startup", f"Disabled WMI startup item: {wmi_name}")
        except Exception as e:
            QMessageBox.warning(None, "Error", f"Error disabling WMI item '{wmi_name}': {e}")
...
def refresh_startup_tab():
    """
    Clears the startup items layout and rebuilds it from
    registry, startup folders, and WMI items.
    """
    ...
    for key, data in all_items.items():
        ...
        if src == "registry":
            ...
            folder_label = "User Startup" if data.get("folder_label") == "UserStartup" else "All Users Startup"
            text = f"{folder_label}: {data['name']} -> {data.get('path','')}"
        elif src == "wmi":
            text = f"WMI: {data.get('name','')} -> {data.get('command','')} (Location: {data.get('location','')})"
        else:
            text = f"{data.get('name','Unknown Source')}"
        ...
        disable_button.clicked.connect(lambda _, k=key, d=data: disable_startup_item(k, d))
