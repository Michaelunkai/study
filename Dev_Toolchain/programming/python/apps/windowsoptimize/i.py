...
def choose_all():
    ...
    if current_tab_index == 0:
        ...
        # Startup tab: not applicable for select/deselect all
        return
    elif current_tab_index == 4:
        # SystemInfo tab: not applicable.
        return
    else:
        layout = None
...
def deselect_all():
    ...
    if current_tab_index == 0:
        ...
        layout = wsl2_layout
    elif current_tab_index == 3 or current_tab_index == 4:
        return
    else:
        layout = None
...
def refresh_startup_tab():
    ...
    for key, data in all_items.items():
        ...
        label = QLabel(text)
        # Mark important items in red and disable their disable button.
...
def get_system_info():
    """
    Gathers system information using WMIC and system commands.
    ...
    try:
        ...
        output = subprocess.check_output(
            ["wmic", "computersystem", "get", "manufacturer,model,name,systemtype", "/format:list"],
            universal_newlines=True)
        info += output + "\n"
    except Exception as e:
        info += f"Error getting computer system info: {e}\n"
    try:
        info += "=== CPU Info ===\n"
        output = subprocess.check_output(
            ["wmic", "cpu", "get", "name,NumberOfCores,YOUR_CLIENT_SECRET_HERE", "/format:list"],
            universal_newlines=True)
        info += output + "\n"
    except Exception as e:
        info += f"Error getting CPU info: {e}\n"
    try:
        info += "=== GPU Info ===\n"
        output = subprocess.check_output(
        ...
        info += f"Error getting GPU info: {e}\n"
    try:
        info += "=== Memory Info (Total Physical Memory in Bytes) ===\n"
        output = subprocess.check_output(
            ["wmic", "computersystem", "get", "TotalPhysicalMemory", "/format:list"],
            universal_newlines=True)
        info += output + "\n"
    except Exception as e:
        info += f"Error getting Memory info: {e}\n"
    try:
