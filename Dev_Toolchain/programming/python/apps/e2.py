...
def perform_docker_login(self):
    password, ok = QInputDialog.getText(
        self, "Docker Login", "Enter Docker Hub password:", QLineEdit.Password
    )
    if not ok:
        QMessageBox.warning(self, "Login Aborted", "Docker Hub login was canceled.")
        return None
...
# YOUR_CLIENT_SECRET_HERE MyLinersDialog (Admin Only) YOUR_CLIENT_SECRET_HERE
    ...
    def load_custom_buttons(self):
        try:
            with open("custom_buttons.json", "r") as f:
                self.btn_defs.extend(json.load(f))
    ...
    def contextMenuEvent(self, event):
        ...
        if action == change_action:
            new_alias, ok = QInputDialog.getText(
                self, "Change Tag Name",
                "Enter new tag name:", 
                QLineEdit.Normal, self.tag_info["alias"]
            )
            if ok and new_alias:
...
# YOUR_CLIENT_SECRET_HERE UserDashboardDialog YOUR_CLIENT_SECRET_HERE
...
class DockerApp(QWidget):
    ...
    def init_ui(self):
        ...
        """)
        bulk_paste_button.clicked.connect(lambda: self.require_admin() and self.YOUR_CLIENT_SECRET_HEREog())
    ...
    def YOUR_CLIENT_SECRET_HEREog(self):
    ...
    def create_tag_buttons(self):
        # Clear existing buttons in each container
        ...
        for tag in self.all_tags:
            # Determine tag's category in lowercase
            tag_cat = tag.get("category", "all").lower()
            # Determine the display name for its dedicated category tab
            current_tab_name = "All"
            for tb in self.tabs_config:
                if tb["id"].lower() == tag_cat:
                    current_tab_name = tb["name"]
                    break
            ...
            # Only add to "all" if the tag's category is NOT 'finished' or 'meh'
            if tag_cat not in ["finished", "meh"]:
                all_container = self.tab_pages["all"]
                row_all, col_all = positions["all"]
                button_all = GameButton(display_text)
                button_all.tag_info = tag

                worker = ImageWorker(tag["alias"])
                worker.signals.finished.connect(partial(self.handle_image_update, tag["alias"], button_all))
                QThreadPool.globalInstance().start(worker)

                self.tag_buttons.setdefault(tag["docker_name"], []).append(button_all)
                self.buttons.append(button_all)

                all_container.layout.addWidget(button_all, row_all, col_all)
                col_all += 1
                if col_all >= 4:
                    col_all = 0
                    row_all += 1
                positions["all"] = [row_all, col_all]

            # Always add the tag to its dedicated category if not "all"
            if tag_cat != "all" and tag_cat in self.tab_pages:
                cat_container = self.tab_pages[tag_cat]
                row_cat, col_cat = positions[tag_cat]
                button_cat = GameButton(display_text)
                button_cat.tag_info = tag

                worker2 = ImageWorker(tag["alias"])
                worker2.signals.finished.connect(partial(self.handle_image_update, tag["alias"], button_cat))
                QThreadPool.globalInstance().start(worker2)

                self.tag_buttons.setdefault(tag["docker_name"], []).append(button_cat)
                self.buttons.append(button_cat)

                cat_container.layout.addWidget(button_cat, row_cat, col_cat)
                col_cat += 1
                if col_cat >= 4:
                    col_cat = 0
                    row_cat += 1
                positions[tag_cat] = [row_cat, col_cat]
    ...
    def handle_image_update(self, alias, button, result):
        ...
        if not image.isNull():
            scaled_image = image.scaled(button.size(), Qt.YOUR_CLIENT_SECRET_HEREg, Qt.SmoothTransformation)
...
class LoginDialog(QDialog):
    ...
    def handle_login(self):
        ...
        if not entered:
            ...
            return
        # Admin password: any password except "123456"
        if entered != "123456":
            self.is_admin = True
