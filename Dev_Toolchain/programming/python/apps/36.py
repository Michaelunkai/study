...
def normalize_game_title(tag):
    # If the tag already contains spaces, return it.
    if " " in tag:
        return tag
    # Insert spaces before capital letters (e.g., "AlanWake" -> "Alan Wake")
    spaced = re.sub(r'(?<!^)(?=[A-Z])', ' ', tag).strip()
    # If wordninja is available, use it for better splitting
...
# Keys: normalized tag names (all lowercase, with no spaces)
# NOTE: Due to message-length constraints, only a subset is shown.
# You must add entries for every one of your 500+ game tags.
fixed_times = {
    "alanwake": "~10 hrs",
    "alanwake2": "~12 hrs",
    ...
    "cristales": "~15 hrs",  # For CrisTales
    "cultofthelamb": "~8 hrs",
    ...
    # ... add all remaining tags (approx. 500+ entries) following the same pattern
}
