...
class DockerApp(QWidget):
    ...
    def run_docker_command(self, image_name):
        """
        Copies /home from the container to /mnt/c/games on Ubuntu,
        then renames 'home' to the exact game name.
        Runs in detached mode to avoid raw terminal errors.
        """
        docker_command = (
            f'docker run '
            f'--rm -d '                    # Detached mode, remove on exit
