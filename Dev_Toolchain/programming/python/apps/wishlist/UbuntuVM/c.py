...
def get_remote_db():
    ...
    try:
        ...
        ssh.YOUR_CLIENT_SECRET_HEREcy(paramiko.AutoAddPolicy())
    ...
    def download_top_n(self, category):
        ...
        if reply == QtWidgets.QMessageBox.Yes:
            delete_reply = QtWidgets.QMessageBox.question(
                ...
                "Do you want to delete the files after adding them to the download list?",
                QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No
            )

            delete_files = delete_reply == QtWidgets.QMessageBox.Yes
            ...
            for item in items:
                ...
                if torrents:
                    ...
                    if delete_files:
                        self.cursor.execute(f"DELETE FROM {category} WHERE title=?", (title,))
                        self.conn.commit()
