...
def perform_docker_login(self):
    # Obtain the Docker Hub password interactively
    ...
    # Perform the Docker login using the entered password
...
class DockerPullWorker(QRunnable):
    ...
    @pyqtSlot()
    def run(self):
        # Run docker pull inside WSL with a timeout (e.g. 600 seconds)
...
def start_docker_engine():
    # Since Docker Desktop is not used, simply warn if docker engine is not running
...
class MyLinersDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.load_custom_buttons()  # Load custom user-defined buttons
        ...
        sleep_btn = self.create_button("Sleep", self.get_sleep_command())
    ...
    def add_custom_button(self):
        ...
        save_custom_buttons(self.btn_defs)  # Save custom buttons to a file or persistent storage
    ...
    def remove_custom_button(self):
        ...
        self.init_ui()  # Reinitialize UI to refresh buttons

    def load_custom_buttons(self):
        # Read the custom buttons from persistent storage
        try:
    ...
    def get_sleep_command(self):
        # The command that will run the 'ss' function in a PowerShell context
        return (
            ...
            "$powerManagement = Get-WmiObject -Query 'SELECT * FROM Win32_DeviceSettings WHERE InstanceID=`'$deviceID`'' -ErrorAction SilentlyContinue; "
...
class DockerApp(QWidget):
    ...
    def perform_docker_login(self):
        # Build the docker login command string and pass it to WSL
    ...
    def run_selected_commands(self):
        ...
        for btn in selected_buttons:
            tag = btn.tag_info["docker_name"]
            # Build the docker run command separately
            docker_cmd = (
            ...
            # Wrap the docker command inside a bash -lic call via WSL
