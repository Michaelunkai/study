...
class DockerApp(QWidget):
    ...
    def rename_tab(self):
        current_tab_id = self.tabs_config[self.stacked.currentIndex()]["id"]
        current_tab_name = self.tabs_config[self.stacked.currentIndex()]["name"]
        new_name, ok = QInputDialog.getText(self, "Rename Tab", "Enter new tab name:", QLineEdit.Normal, current_tab_name)
        if not (ok and new_name):
            return
        self.tabs_config[self.stacked.currentIndex()]["name"] = new_name
        save_tabs_config(self.tabs_config)
        self.tab_nav.update_tabs(self.tabs_config)
        self.create_tag_buttons()
