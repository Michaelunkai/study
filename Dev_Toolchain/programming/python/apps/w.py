...
class DockerPullWorker(QRunnable):
    ...
    @pyqtSlot()
    def run(self):
        # Run the docker pull command via WSL
        pull_cmd = f'wsl --distribution ubuntu --user root -- bash -lic "docker pull michadockermisha/backup:\\"{self.tag}\\""'
        try:
            subprocess.call(pull_cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        except Exception as e:
...
def dkill():
    ...
    # Redirect output to DEVNULL to avoid any output on exit.
    for cmd in cmds:
        try:
...
class MyLinersDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Define a list of (label, command) pairs.
        btn_defs = [
