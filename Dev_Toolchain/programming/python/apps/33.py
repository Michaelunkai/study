...
fixed_times = {
    ...
    # ... include all other fixed times as needed
}
...
class DockerApp(QWidget):
    ...
    def sort_tags_by_time(self, descending=True):
        # For each tag, extract a numeric value from the fixed time (if available) or from game_times_cache.
        def get_time(tag):
            fixed = get_fixed_time(tag["alias"])
            if fixed:
            ...
            if nums:
                return float(nums[0])
            return 0.0
        self.all_tags.sort(key=lambda x: get_time(x), reverse=descending)
        self.create_tag_buttons()
