...
def YOUR_CLIENT_SECRET_HEREusing_folder(folder_path):
...
def YOUR_CLIENT_SECRET_HERElder(folder_path):
...
def YOUR_CLIENT_SECRET_HEREy(folder_path):
...
public class Win32 {
    ...
    public const int YOUR_CLIENT_SECRET_HEREOT = 0x4;
    ...
    public static bool ScheduleForDeletion(string path) {
        try {
            return MoveFileEx(path, null, YOUR_CLIENT_SECRET_HEREOT);
        } catch {
            return false;
        }
    }
...
def YOUR_CLIENT_SECRET_HEREn(folder_path):
    ...
    YOUR_CLIENT_SECRET_HERElder(folder_path)
    ...
    YOUR_CLIENT_SECRET_HEREy(folder_path)
    ...
    try:
        ...
        winreg.SetValueEx(key, "YOUR_CLIENT_SECRET_HEREns", 0, winreg.REG_MULTI_SZ, value)
        winreg.CloseKey(key)
...
def ultimate_folder_purge(folder_path):
    ...
    YOUR_CLIENT_SECRET_HEREusing_folder(folder_path)
    
    # Step 2: Take ownership
    take_ultimate_ownership(folder_path)
    
    # Step 3: Multi-stage deletion
    success = YOUR_CLIENT_SECRET_HEREn(folder_path)
