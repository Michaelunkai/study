...
def normalize_game_title(tag):
    ...
    If tag is camel case, insert a space before uppercase letters.
...
# YOUR_CLIENT_SECRET_HERE Bulk Move Dialog YOUR_CLIENT_SECRET_HERE
...
class BulkMoveDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Populate combo with available tabs (excluding "All" if desired)
        for tab in tabs_config:
            # Optionally, skip "All" tab
            if tab["id"] != "all":
...
class BulkMoveDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.combo = QComboBox()
        for tab in tabs_config:
            if tab["id"] != "all":  # Optionally exclude "All"
                self.combo.addItem(tab["name"], tab["id"])
        layout.addWidget(QLabel("Move selected tags to:"))
        layout.addWidget(self.combo)
        self.move_button = QPushButton("Move Selected")
...
class DockerApp(QWidget):
    ...
    def init_ui(self):
        ...
        # New "Move Tags" button for bulk move
    ...
    def open_bulk_move_dialog(self):
        ...
        if dialog.exec_():
            # Save changes to persistent settings
            for tag in self.all_tags:
