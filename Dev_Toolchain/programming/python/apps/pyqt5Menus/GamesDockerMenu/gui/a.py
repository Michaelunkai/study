...
def display_games_in_grid(games, title="Games", games_per_row=3, show_selection=True, selected_tags=None, app_instance=None, clear_screen_first=True):
    ...
    for row_start in range(0, len(games), games_per_row):
        ...
        for i, game in enumerate(row_games):
            ...
            image_filename = sanitize_filename(game['docker_name']) + ".png"
            image_path = os.path.join("images", image_filename)
            image_exists = os.path.exists(image_path)
            if image_exists:
                image_url = f"file://{os.path.abspath(image_path)}"
                image_display = f"[IMG: {image_filename}] {image_url}"
            else:
                image_display = f"[NO IMG: {image_filename}]"

            color = Fore.GREEN if is_selected else Fore.WHITE
            print(f"{Fore.YELLOW}[{game_idx:3d}]{color} {selection_marker} {display_name:<30} {Fore.CYAN}{size_str:<8} {Fore.MAGENTA}{completion_time:<12} {Fore.BLUE}{image_display}", end="  ")
