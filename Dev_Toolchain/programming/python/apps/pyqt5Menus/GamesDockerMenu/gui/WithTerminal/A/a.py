...
from PyQt5.QtWidgets import (
    ...
    QStackedWidget, QCheckBox, QTextEdit, QFileDialog, QSplitter
)
...
class MyLinersDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.terminal = self.parent().terminal if hasattr(self.parent(), 'terminal') else None
    ...
    def create_button(self, label, cmd):
        ...
        """)
        btn.clicked.connect(lambda: self.run_command(cmd))
        return btn
...
class DockerApp(QWidget):
    ...
    def create_tag_buttons(self):
        ...
        for tag in self.all_tags:
            ...
            for target_cat in ["all", tag.get("category", "all")]:
                if target_cat == "all" and tag.get("category", "all") in ["meh", "finished"]:
                    continue
    ...
    def run_selected_commands(self):
        ...
        # REMOVED "-d" flag from docker run command to run in attached mode,
        # so you see real‑time progress.
        for btn in selected_buttons:
            tag = btn.tag_info["docker_name"]
            docker_cmd = (
