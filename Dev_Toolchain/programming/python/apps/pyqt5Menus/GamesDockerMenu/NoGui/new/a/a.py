...
GAMES_DATA = {
    "all_games": [
        ...
        "EternalStrands", "YOUR_CLIENT_SECRET_HEREemastered", "GardenPaws", "Hellpoint", "inZOI",
        "JurassicWorldEvolution2", "KingdomComeDeliverance", "legoBatman2", "LEGOHarryPotter57",
        "YOUR_CLIENT_SECRET_HEREalAdventures", "MySummerCar", "MyTimeatSandrock", "Obduction",
        "Omensight", "YOUR_CLIENT_SECRET_HEREeous", "PHOGS", "YOUR_CLIENT_SECRET_HEREime",
        "RajiAnAncientEpic", "YOUR_CLIENT_SECRET_HEREtered", "rivals2", "SamuraiGunn2",
        "ScarletHollow", "ScheduleI", "YOUR_CLIENT_SECRET_HEREn", "SixDaysinFallujah",
        ...
        "TDPAHouseofAshes", "YOUR_CLIENT_SECRET_HEREittleHope", "YOUR_CLIENT_SECRET_HEREittlehope",
        "YOUR_CLIENT_SECRET_HEREanofMedan", "TheEscapists2", "YOUR_CLIENT_SECRET_HERErante",
...
def display_games_in_grid(games, title="Games", games_per_row=3, show_selection=True, selected_tags=None):
    ...
    if choice == 'r':
        ...
        time.sleep(0.5)
        return display_games_in_grid(games, title, games_per_row, show_selection, selected_tags)
    elif choice == 's2':
...
def load_time_data(file_path):
    """Load game time data from file or use embedded data"""
    time_data = {}
        ...
        try:
            with open(file_path, "r", encoding='utf-8') as f:
                for line in f:
                    ...
                    # Handle different separators: – (em dash), - (regular dash), and : (colon)
                    if "–" in line:
                        parts = line.split("–")
                    elif "-" in line:
                        parts = line.split("-")
                    ...
                    if len(parts) >= 2:
                        ...
                        # Also store with different case variations for better matching
...
class YOUR_CLIENT_SECRET_HERE:
    def __init__(self, login_password, is_admin, username):
        ...
        # Load all JSON data
        print_info("Loading all JSON data...")
    ...
    def initialize_app(self):
        ...
        # Force refresh time data to ensure all games have completion times
        self.refresh_time_data()
        
        # Debug: Show time data loading statistics
        self.show_time_data_stats()
        
        # Add active user
        self.add_active_user()
        
        # Start banned user monitoring in background
    ...
    def get_tab_color(self, tab_id):
        """Get tab color from tab ID"""
        for tab in self.tabs_config:
            if tab["id"] == tab_id:
                return tab.get("color", Fore.WHITE)
        return Fore.WHITE

    def filter_games_by_tab(self, tab_id):
        """Filter games by tab category"""
        if tab_id == "all":
            # Show ALL games in the 'All' tab, no exclusions
            return self.all_tags
        else:
            return [tag for tag in self.all_tags if tag.get("category", "all") == tab_id]

    def show_tab_buttons(self):
        """Display all tabs as buttons - 5 per row"""
        print(f"\n{Fore.CYAN}{Style.BRIGHT}TAB NAVIGATION:")
        
        # Calculate how many tabs per row (max 5)
        tabs_per_row = 5
        for i in range(0, len(self.tabs_config), tabs_per_row):
            row_tabs = self.tabs_config[i:i+tabs_per_row]
            for j, tab in enumerate(row_tabs):
                tab_number = i + j + 1
                ...
                if is_active:
                    print(f"{tab_color}{Style.BRIGHT}[{tab_number:2d}]► {tab['name']:<15} ({games_count:3d}){Style.RESET_ALL}", end="  ")
                else:
                    print(f"{tab_color}[{tab_number:2d}]  {tab['name']:<15} ({games_count:3d}){Style.RESET_ALL}", end="  ")
            print()  # New line after each row
    ...
    def show_enhanced_main_menu(self):
        ...
        while self.running:
            ...
            # Show sync options based on guest mode
            if self.is_guest:
                print_menu_option("R", "🚀 RUN SELECTED GAMES (Disabled in Guest Mode)", Fore.RED)
            else:
                print_menu_option("R", "🚀 RUN SELECTED GAMES", Fore.RED)
            
            print_menu_option("S", "Search Games")
            print_menu_option("O", "Sort Games")
            print_menu_option("M", "Selected Games Management")
            print_menu_option("T", "⏱️ Time Statistics", Fore.MAGENTA)
            print_menu_option("TD", "🔍 Time Data Debug", Fore.CYAN)
            print_menu_option("TR", "🔄 Refresh Time Data", Fore.BLUE)
            ...
            print_menu_option("A", "➕ Add Game to Current Tab", Fore.BLUE)
    ...
    def run_selected_games(self):
        ...
        print(f"\n{Fore.YELLOW}Destination Path Configuration:")
        print_menu_option("1", f"Default: {DEFAULT_DESTINATION}")
        print_menu_option("2", "Custom path")
        print_menu_option("3", "F: Drive (F:\\Games)")
        ...
        if path_choice.upper() == "B":
            ...
            if not destination:
                print_warning("No path entered, using default.")
                destination = DEFAULT_DESTINATION
        elif path_choice == "3":
            destination = "F:\\Games"
        else:
        ...
        for i, game in enumerate(selected_games, 1):
            ...
            try:
                ...
                docker_command = (
                    f"wsl -d Ubuntu docker run --pull=always --rm "
                    f"-v {wsl_path}:/games "
                    f"-e DISPLAY=$DISPLAY "
                    f"-v /tmp/.X11-unix:/tmp/.X11-unix "
                    f"--name {game['docker_name']} "
                    f"michadockermisha/backup:{game['docker_name']} "
                    f"sh -c \"apk add rsync pigz && mkdir -p /games/{game['docker_name']} && "
                    f"rsync -aP --compress-level=1 --compress --numeric-ids --inplace --delete-during --info=progress2 --no-i-r /home /games/{game['docker_name']}\""
                )
    ...
    def execute_sync_command(self, docker_name, wsl_path):
        ...
        docker_command = (
            ...
            f"sh -c \"apk add rsync && rsync -av --progress /home /games && "
            f"cd /games && mv home {docker_name} && exit\""
        )
    ...
    def sync_operations(self):
        ...
        print_menu_option("1", "🚀 Run Selected Games" + (f" ({len(self.selected_tag_names)})" if self.selected_tag_names else " (None)"), Fore.RED)
        print_menu_option("2", "Quick Sync Single Game")
        print_menu_option("3", "Bulk Sync by Category")
        print_menu_option("4", "Resume/Monitor Active Syncs")
        print_menu_option("B", "Back")
