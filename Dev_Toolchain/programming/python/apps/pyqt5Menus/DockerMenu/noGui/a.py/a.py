...
DOCKERFILE_TEMPLATE = "/mnt/f/study/containers/docker/dockerfiles/buildthispath/buildthispath"
DOCKER_REPO = "michadockermisha/backup"
...
def quick_build_push(folder):
    tag = normalize_name(folder)
    cmd = (
        f"cd {tag} && "
        f"cp {DOCKERFILE_TEMPLATE} ./Dockerfile && "
        f"docker build -t {DOCKER_REPO}:{tag} . && "
        f"docker push {DOCKER_REPO}:{tag}"
    )
    print(f"Running: {cmd}")
