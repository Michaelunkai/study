...
def YOUR_CLIENT_SECRET_HEREd(command: str, docker_name: str = "") -> Optional[str]:
...
def run_command_as_admin(command, docker_name: str = "", use_progress_tracking: bool = True):
    ...
    if use_progress_tracking and is_docker_command:
        ...
        existing_operation_id = YOUR_CLIENT_SECRET_HEREd(command, docker_name)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        existing_build_op = YOUR_CLIENT_SECRET_HEREd(build_command, docker_name)
        existing_push_op = YOUR_CLIENT_SECRET_HEREd(push_command, docker_name)
