...
class DockerApp(QWidget):
    ...
    def init_ui(self):
        ...
        # Add browse button first so it appears on the far right
        ...
        if self.is_admin:
            ...
            # Add Clear Terminal button next to myLiners (for admins)
            ...
            # For non-admin users, add Clear Terminal button as well
            clear_terminal_btn = QPushButton("Clear Terminal")
            clear_terminal_btn.setStyleSheet("""
                QPushButton {
                    background: #34495E;
    ...
    def run_selected_commands(self):
        ...
        # Ask user to select destination path
        ...
        for btn in selected_buttons:
            tag = btn.tag_info["docker_name"]
            # Build the docker run command with custom destination path
            docker_cmd = (
