...
void UpdateTrayTooltip() {
    if (g_isOptimizationEnabled) {
        wcscpy_s(g_nid.szTip, L"RAM Optimizer - Running (Auto-optimize every 1s)");
    } else {
...
void ToggleOptimization() {
    ...
    if (g_isOptimizationEnabled) {
        SetTimer(g_hWnd, TIMER_ID, 1000, NULL); // 1 second for more aggressive optimization
    } else {
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    SetTimer(g_hWnd, TIMER_ID, 1000, NULL); // Start timer immediately with 1 second interval
