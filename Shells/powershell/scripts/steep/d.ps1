...
<#
.SYNOPSIS
    Complete System Optimization - ALL COMMANDS EMBEDDED VERSION (Docker Safe + Wise Tools)
.DESCRIPTION
    Runs ALL system optimization commands directly - no function calls. Docker data safely cleaned without removing Docker.
...
elseif (-not $SkipConfirmations) {
    ...
    Write-Host "• Docker safe data cleanup (preserving Docker functionality)" -ForegroundColor Yellow
...
function Start-SystemOptimization {
    ...
    if ($global:RestartChoice -eq "none") {
        $totalOperations = 24  # No RESS script but includes Wise tools
    } else {
        $totalOperations = 25  # Include final script + Wise tools
    }
    ...
    # OPERATION 2: SAFE DOCKER DATA CLEANUP - Only clean used space, preserve Docker
    $operationCount++
    Write-ProgressLog "Safe Docker data cleanup (preserving Docker functionality)"
    try {
        Write-Log "=== SAFE DOCKER DATA CLEANUP (PRESERVING DOCKER COMPLETELY) ==="
        Write-Log "Starting safe Docker data cleanup - removing only unused data..."
        
        # Stop all containers gracefully first
        Write-Log "Gracefully stopping all Docker containers..."
        $containers = docker ps -q 2>$null
        if ($containers) {
            Write-Log "Found $($containers.Count) running containers to stop"
            docker stop $containers --time=10 2>$null | Out-Null
            Write-Log "All containers stopped gracefully"
        } else {
            Write-Log "No running containers found"
        }
        
        # Remove stopped containers (safe - they can be recreated)
        Write-Log "Removing stopped containers..."
        $stoppedContainers = docker ps -aq --filter "status=exited" 2>$null
        if ($stoppedContainers) {
            Write-Log "Removing $($stoppedContainers.Count) stopped containers"
            docker rm $stoppedContainers 2>$null | Out-Null
            Write-Log "Stopped containers removed"
        } else {
            Write-Log "No stopped containers to remove"
        }
        
        # Remove dangling images only (safe - preserves all tagged images)
        Write-Log "Removing dangling images only..."
        ...
        if ($danglingImages) {
            ...
            Write-Log "No dangling images to remove"
        }
        
        # Clean unused volumes (safe - preserves volumes still in use)
        Write-Log "Removing unused volumes..."
        $unusedVolumes = docker volume ls -q --filter "dangling=true" 2>$null
        if ($unusedVolumes) {
            Write-Log "Removing $($unusedVolumes.Count) unused volumes"
            docker volume rm $unusedVolumes 2>$null | Out-Null
            Write-Log "Unused volumes removed"
        } else {
            Write-Log "No unused volumes to remove"
        }
        
        # Clean unused networks (safe - preserves default and in-use networks)
        Write-Log "Removing unused networks..."
        docker network prune -f 2>$null | Out-Null
        Write-Log "Unused networks removed"
        
        # Clean build cache (safe - builds will work but may take longer first time)
        Write-Log "Cleaning Docker build cache..."
        docker builder prune -f 2>$null | Out-Null
        Write-Log "Build cache cleaned"
        
        # SAFE docker_data cleanup - target specific cache directories only
        Write-Log "Cleaning Docker data cache directories (SAFE - preserving configuration)..."
        
        # Only clean these specific safe cache directories
        $dockerCacheDirs = @(
            "$env:ProgramData\Docker\tmp",
            "$env:ProgramData\Docker\windowsfilter\*",
            "$env:LOCALAPPDATA\Docker\log",
            "$env:APPDATA\Docker Desktop\logs"
        )
        
        # PRESERVE these critical paths (never touch these)
        $preservedPaths = @(
            "$env:ProgramData\Docker\config",
            "$env:APPDATA\Docker Desktop\settings.json",
            "$env:APPDATA\Docker Desktop\settings",
            "C:\Program Files\Docker"
        )
        
        Write-Log "Cleaning ONLY cache directories (preserving all configuration)..."
        foreach ($cacheDir in $dockerCacheDirs) {
            if (Test-Path $cacheDir) {
                try {
                    Write-Log "Cleaning Docker cache: $cacheDir"
                    Remove-Item -Path $cacheDir -Recurse -Force -ErrorAction SilentlyContinue
                    Write-Log "Cleaned: $cacheDir"
                } catch {
                    Write-Log "Could not clean $cacheDir (may be in use)" "WARN"
                }
            }
        }
        
        Write-Log "Verifying Docker configuration preservation..."
        foreach ($preservedPath in $preservedPaths) {
            if (Test-Path $preservedPath) {
                Write-Log "✓ PRESERVED: $preservedPath"
            }
        }
        
        # Force delete ALL files in Docker images that are not running (aggressive space cleanup)
        Write-Log "Force deleting unused image files for maximum space recovery..."
        
        # Get all image IDs that are currently running
        $runningImageIds = docker ps -q --format "{{.Image}}" 2>$null
        $allImageIds = docker images -q 2>$null
        
        if ($allImageIds) {
            Write-Log "Found $($allImageIds.Count) total images"
            
            # Remove images that are NOT currently running (force delete for space)
            $unusedImages = $allImageIds | Where-Object { $_ -notin $runningImageIds }
            if ($unusedImages) {
                Write-Log "Force deleting $($unusedImages.Count) unused images for space recovery..."
                docker rmi -f $unusedImages 2>$null | Out-Null
                Write-Log "Force deleted unused images successfully"
            } else {
                Write-Log "All images are currently in use - preserving them"
            }
        }
        
        # Clean Docker system (safe comprehensive cleanup)
        Write-Log "Running final Docker system cleanup..."
        docker system prune -f 2>$null | Out-Null
        Write-Log "Docker system cleanup completed"
        
        # Verify Docker is still functional
        Write-Log "Verifying Docker functionality..."
        $dockerVersion = docker version --format "{{.Server.Version}}" 2>$null
        if ($dockerVersion) {
            Write-Log "✅ Docker is functional - version: $dockerVersion"
        } else {
            Write-Log "⚠️  Docker may need restart but is preserved" "WARN"
        }
        
        Write-Log "SAFE DOCKER DATA CLEANUP COMPLETED - All functionality preserved"
        
    } catch {
        Write-Log "Safe Docker data cleanup failed: $_" "ERROR"
    }
    ...
    # OPERATION 4: Post-CCleaner safe Docker cleanup
    $operationCount++
    Write-ProgressLog "Post-CCleaner safe Docker cleanup"
    try {
        # Only clean what we used for CCleaner
        $containers = docker ps -aq --filter "ancestor=michadockermisha/backup:ccleaner" 2>$null
        if ($containers) {
            docker stop $containers 2>$null | Out-Null
            docker rm $containers 2>$null | Out-Null
        }
        
        # Remove only the CCleaner backup image
        $ccleanerImage = docker images -q michadockermisha/backup:ccleaner 2>$null
        if ($ccleanerImage) {
            docker rmi $ccleanerImage 2>$null | Out-Null
        }
        
        # Light cleanup
        docker system prune -f 2>$null | Out-Null
        Write-Log "Post-CCleaner Docker cleanup completed"
    } catch {
        Write-Log "Post-CCleaner Docker cleanup failed: $_" "ERROR"
    }
    ...
    # OPERATION 13: Post-Bleach safe Docker cleanup
    $operationCount++
    Write-ProgressLog "Post-Bleach safe Docker cleanup"
    try {
        # Light cleanup only
        docker system prune -f 2>$null | Out-Null
        Write-Log "Post-Bleach Docker cleanup completed"
    } catch {
        Write-Log "Post-Bleach Docker cleanup failed: $_" "ERROR"
    }
...
"@
    ...
    # OPERATION 19: Final safe Docker cleanup
    $operationCount++
    Write-ProgressLog "Final safe Docker cleanup"
    try {
        # Only clean cache and temporary data
        docker system prune -f 2>$null | Out-Null
        Write-Log "Final Docker cleanup completed"
    } catch {
    ...
    # NEW: Advanced Docker Cleanup (BEFORE reboot if chosen)
    if ($global:RestartChoice -ne "none") {
        Write-Log "=== ADVANCED DOCKER CLEANUP BEFORE REBOOT ==="
        try {
            Write-Log "Executing advanced Docker cleanup and optimization..."
            
            # Execute the comprehensive Docker cleanup command
            $dockerCleanupCommand = @"
docker system prune -a --volumes -f; docker builder prune -a -f; docker buildx prune -a -f; Stop-Process -Name "Docker Desktop" -Force -ErrorAction SilentlyContinue; wsl --shutdown; wsl --export docker-desktop "$env:TEMP\docker-desktop-backup.tar"; wsl --unregister docker-desktop; Remove-Item "C:\Users\misha\AppData\Local\Docker\wsl\disk\docker_data.vhdx" -Force -ErrorAction SilentlyContinue; wsl --import docker-desktop "C:\Users\misha\AppData\Local\Docker\wsl\distro" "$env:TEMP\docker-desktop-backup.tar"; Remove-Item "$env:TEMP\docker-desktop-backup.tar" -Force -ErrorAction SilentlyContinue; Optimize-VHD -Path "C:\Users\misha\AppData\Local\Docker\wsl\disk\docker_data.vhdx" -Mode Full -ErrorAction SilentlyContinue; & "C:\Program Files\Docker\Docker\Docker Desktop.exe"
"@
            
            Write-Log "Starting Docker system prune (all images, containers, volumes)..."
            docker system prune -a --volumes -f 2>$null | Out-Null
            
            Write-Log "Starting Docker builder prune (all build cache)..."
            docker builder prune -a -f 2>$null | Out-Null
            
            Write-Log "Starting Docker buildx prune (all buildx cache)..."
            docker buildx prune -a -f 2>$null | Out-Null
            ...
            Write-Log "✅ Advanced Docker cleanup and optimization completed successfully!"
            
        } catch {
            Write-Log "Advanced Docker cleanup failed: $_" "ERROR"
        }
...
Write-Host "  • Docker Desktop safely optimized (data cleaned, functionality preserved)" -ForegroundColor Green
...
Write-Host "  • 🔧 Advanced Docker cleanup and VHDX optimization (if rebooting) 🔧" -ForegroundColor Yellow
...
switch ($global:RestartChoice) {
    "ress" {
        ...
        Write-Host "• 🔧 Advanced Docker cleanup was executed before reboot" -ForegroundColor Yellow
    ...
    "none" {
        ...
        Write-Host "• Docker has been safely optimized (functionality preserved)" -ForegroundColor Green
        ...
        Write-Host "• 🔧 Advanced Docker cleanup was skipped (no reboot selected)" -ForegroundColor Gray
        ...
        if (-not $SkipConfirmations) {
            ...
            if ($lastChance -match '^[Yy]') {
                Write-Log "User changed mind - executing advanced Docker cleanup and initiating system restart..."
                
                # Execute advanced Docker cleanup since user changed mind
                Write-Host "🔧 Executing advanced Docker cleanup before restart..." -ForegroundColor Yellow
                try {
                    Write-Log "Executing advanced Docker cleanup..."
                    docker system prune -a --volumes -f 2>$null | Out-Null
                    docker builder prune -a -f 2>$null | Out-Null
                    docker buildx prune -a -f 2>$null | Out-Null
                    Stop-Process -Name "Docker Desktop" -Force -ErrorAction SilentlyContinue
                    ...
                    Optimize-VHD -Path "C:\Users\misha\AppData\Local\Docker\wsl\disk\docker_data.vhdx" -Mode Full -ErrorAction SilentlyContinue
                    & "C:\Program Files\Docker\Docker\Docker Desktop.exe"
                    Write-Host "✅ Advanced Docker cleanup completed!" -ForegroundColor Green
                } catch {
                    Write-Log "Advanced Docker cleanup failed: $_" "ERROR"
                }
