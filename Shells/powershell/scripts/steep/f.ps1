...
<#
.SYNOPSIS
    Complete System Optimization - ALL COMMANDS EMBEDDED VERSION (Comprehensive Corruption Repair + Network Repair + System Purge + Clean Exit)
.DESCRIPTION
    Runs ALL system optimization commands directly - no function calls. Includes 17 comprehensive corruption repair operations, 12 comprehensive network repair operations, complete system purge, and proper clean exit handling.
...
elseif (-not $SkipConfirmations) {
    ...
    Write-Host "• Comprehensive network repair (12 operations: DHCP, DNS, Hosts, ARP, etc.)" -ForegroundColor Yellow
    Write-Host "• Network and WiFi performance boost" -ForegroundColor Yellow
    Write-Host "• Comprehensive system corruption repair (17 operations: SFC, DISM, Enhanced CHKDSK, WinGet, Registry, etc.)" -ForegroundColor Yellow
    ...
    Write-Host "[2] Run Fit-Launcher script (F:\study\shells\powershell\scripts\rebootfitlauncher\a.ps1)" -ForegroundColor Green  
...
function Write-ProgressLog {
    ...
    foreach ($proc in $hangingProcesses) {
        ...
        if ($processes) {
            ...
            if (((Get-Date) - $oldestProcess.StartTime).TotalMinutes -gt 15) {
                Write-Log "🚨 Killing hanging process: $proc (running for over 15 minutes)" "WARN"
...
function Start-SystemOptimization {
    ...
    # Global safety mechanism - clean operation without problematic event handlers
    # Background job for progress tracking only
    
    # Start a background job to show progress every 15 seconds
    $global:progressJob = Start-Job {
        $counter = 0
        while ($true) {
            Start-Sleep 15
            $counter++
    ...
    $operationCount = 0
    # Adjust total operations - added comprehensive network repair
    if ($global:RestartChoice -eq "none") {
        $totalOperations = 22  # Added comprehensive network repair operation
    } else {
        $totalOperations = 23  # Added comprehensive network repair operation
    }
    ...
    try {
        ...
        $maxTempCleanupTime = 900  # 15 minutes maximum for all temp cleanup
        
        foreach ($path in $allPaths) {
            # Check if we've exceeded the maximum time
            if (((Get-Date) - $tempCleanupStart).TotalSeconds -gt $maxTempCleanupTime) {
                Write-Log "Temp cleanup timeout reached ($($maxTempCleanupTime/60) minutes) - skipping remaining paths..." "WARN"
                break
            }
...
"@
    ...
    # OPERATION 20: COMPREHENSIVE NETWORK REPAIR AND OPTIMIZATION
    $operationCount++
    Write-ProgressLog "Comprehensive network repair and optimization (Advanced Repair Tool commands)"
    try {
        Write-Log "=== COMPREHENSIVE NETWORK REPAIR AND OPTIMIZATION ==="
        Write-Log "Running ALL Advanced Network Repair Tool commands automatically (15-minute timeout each)..."
        
        # 1. Release and Renew DHCP Address
        Write-Log "1/12: Release and Renew DHCP Address..."
        $dhcpJob = Start-Job {
        ...
        if (Wait-Job $dhcpJob -Timeout 900) {
            Receive-Job $dhcpJob | Out-Null
            Remove-Job $dhcpJob
            Write-Log "✅ DHCP Address release and renew completed"
        } else {
            Stop-Job $dhcpJob
            Remove-Job $dhcpJob
            Write-Log "⚠️ DHCP release/renew timeout (15 minutes) - continuing..." "WARN"
        }
        
        # 2. Clear Hosts File
        Write-Log "2/12: Clear Hosts File..."
        try {
            ...
            if (Test-Path $hostsPath) {
                ...
                # Clear hosts file content (keep only essential entries)
                $defaultHosts = @"
...
"@
                ...
                Write-Log "✅ Hosts file cleared and reset to default"
            }
        } catch {
            Write-Log "⚠️ Could not clear hosts file: $_" "WARN"
        }
        
        # 3. Clear Static IP Settings (enable DHCP)
        Write-Log "3/12: Clear Static IP Settings (enable DHCP)..."
        $staticIpJob = Start-Job {
            # Get all network adapters and set to DHCP
        ...
        if (Wait-Job $staticIpJob -Timeout 900) {
            Receive-Job $staticIpJob | Out-Null
            Remove-Job $staticIpJob
            Write-Log "✅ Static IP settings cleared, DHCP enabled"
        } else {
            Stop-Job $staticIpJob
            Remove-Job $staticIpJob
            Write-Log "⚠️ Static IP clearing timeout (15 minutes) - continuing..." "WARN"
        }
        
        # 4. Change to Google DNS
        Write-Log "4/12: Change to Google DNS..."
        try {
            # Set Google DNS (8.8.8.8, 8.8.4.4) for Wi-Fi adapter
            ...
            # Also set for Ethernet if present
            ...
            Write-Log "✅ Google DNS configured (8.8.8.8, 8.8.4.4)"
        } catch {
            Write-Log "⚠️ Google DNS configuration failed: $_" "WARN"
        }
        
        # 5. Flush DNS Cache
        Write-Log "5/12: Flush DNS Cache..."
        try {
            Start-Process "ipconfig" -ArgumentList "/flushdns" -WindowStyle Hidden -Wait
            Write-Log "✅ DNS Cache flushed"
        } catch {
            Write-Log "⚠️ DNS Cache flush failed: $_" "WARN"
        }
        
        # 6. Clear ARP/Route Table
        Write-Log "6/12: Clear ARP/Route Table..."
        try {
            ...
            Write-Log "✅ ARP/Route table cleared"
        } catch {
            Write-Log "⚠️ ARP/Route table clear failed: $_" "WARN"
        }
        
        # 7. NetBIOS Reload and Release
        Write-Log "7/12: NetBIOS Reload and Release..."
        try {
            ...
            Write-Log "✅ NetBIOS reload and release completed"
        } catch {
            Write-Log "⚠️ NetBIOS reload/release failed: $_" "WARN"
        }
        
        # 8. Internet Options - Clear SSL State
        Write-Log "8/12: Internet Options - Clear SSL State..."
        try {
            # Clear SSL state through registry
            $sslPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Internet Settings"
            Remove-ItemProperty -Path $sslPath -Name "CertificateRevocation" -ErrorAction SilentlyContinue
            
            # Clear SSL cache
            Start-Process "rundll32.exe" -ArgumentList "wininet.dll,PrivacySetZonePreferenceW" -WindowStyle Hidden -Wait -ErrorAction SilentlyContinue
            Write-Log "✅ SSL State cleared"
        } catch {
            Write-Log "⚠️ SSL State clear failed: $_" "WARN"
        }
        
        # 9. Enable LAN Adapters
        Write-Log "9/12: Enable LAN Adapters..."
        try {
            ...
            Write-Log "✅ LAN Adapters enabled"
        } catch {
            Write-Log "⚠️ LAN Adapter enabling failed: $_" "WARN"
        }
        
        # 10. Enable Wireless Adapters
        Write-Log "10/12: Enable Wireless Adapters..."
        try {
            ...
            Write-Log "✅ Wireless Adapters enabled"
        } catch {
            Write-Log "⚠️ Wireless Adapter enabling failed: $_" "WARN"
        }
        
        # 11. Reset Internet Options Security/Privacy
        Write-Log "11/12: Reset Internet Options Security/Privacy..."
        try {
            # Reset Internet Explorer settings
            Start-Process "RunDll32.exe" -ArgumentList "InetCpl.cpl,ResetIEtoDefaults" -WindowStyle Hidden -ErrorAction SilentlyContinue
            
            # Reset Internet security zones
            $securityZones = @(
            ...
            foreach ($zone in $securityZones) {
                Remove-Item -Path $zone -Recurse -Force -ErrorAction SilentlyContinue
            }
            
            Write-Log "✅ Internet Options Security/Privacy reset"
        } catch {
            Write-Log "⚠️ Internet Options reset failed: $_" "WARN"
        }
        
        # 12. Set Network Windows Services to Default
        Write-Log "12/12: Set Network Windows Services to Default..."
        $networkServicesJob = Start-Job {
            $networkServices = @{
                "Server" = "Automatic"
        ...
        if (Wait-Job $networkServicesJob -Timeout 900) {
            Receive-Job $networkServicesJob | Out-Null
            Remove-Job $networkServicesJob
            Write-Log "✅ Network Windows Services set to default"
        } else {
            Stop-Job $networkServicesJob
            Remove-Job $networkServicesJob
            Write-Log "⚠️ Network services configuration timeout (15 minutes) - continuing..." "WARN"
        }
        
        Write-Log "Comprehensive network repair and optimization completed (12 operations)"
        
    } catch {
        Write-Log "Network repair and optimization failed: $_" "ERROR"
    }
    
    # OPERATION 21: RERE - Additional Network boost (FULL NETWORK OPTIMIZATION) with verbose logging
    $operationCount++
    Write-ProgressLog "Additional network optimization (TCP/IP stack)"
    try {
        $commandCount = 0
    ...
    # OPERATION 22: Comprehensive PC Performance Optimization
    $operationCount++
    ...
    # OPERATION 23: Comprehensive Disk Space Cleanup
    $operationCount++
    ...
    try {
        ...
        $maxDiskCleanupTime = 900  # 15 minutes maximum for disk cleanup
        
        foreach ($path in $allCleanupPaths) {
            # Check global timeout
            if (((Get-Date) - $diskCleanupStart).TotalSeconds -gt $maxDiskCleanupTime) {
                Write-Log "Disk cleanup timeout reached ($($maxDiskCleanupTime/60) minutes) - skipping remaining paths..." "WARN"
                break
            }
    ...
    # OPERATION 24: Wise Registry Cleaner with spinning progress
    $operationCount++
    ...
    # OPERATION 25: Wise Disk Cleaner with spinning progress and 2-minute timeout
    $operationCount++
    ...
    # Stop all safety jobs and clean exit
    try {
        Write-Log "=== SCRIPT COMPLETION CLEANUP ==="
        Write-Log "Performing clean script termination..."
        
        # Stop progress job
        if ($global:progressJob) {
            Stop-Job $global:progressJob -ErrorAction SilentlyContinue
            Remove-Job $global:progressJob -ErrorAction SilentlyContinue
            Write-Log "Progress job terminated cleanly"
        }
        
        # Clean up any remaining background jobs
        $remainingJobs = Get-Job -State Running -ErrorAction SilentlyContinue
        if ($remainingJobs) {
            Write-Log "Cleaning up $($remainingJobs.Count) remaining background jobs..."
            $remainingJobs | Stop-Job -ErrorAction SilentlyContinue
            $remainingJobs | Remove-Job -ErrorAction SilentlyContinue
        }
        
        # Only kill processes that might be hanging, exclude our own PowerShell
        $processesToCheck = @("cleanmgr", "wsreset", "dism", "sfc")
        $killedProcesses = 0
        foreach ($processName in $processesToCheck) {
            $processes = Get-Process -Name $processName -ErrorAction SilentlyContinue
            if ($processes) {
                $processes | Stop-Process -Force -ErrorAction SilentlyContinue
                $killedProcesses += $processes.Count
            }
        }
        
        if ($killedProcesses -gt 0) {
            Write-Log "Cleaned up $killedProcesses potentially hanging processes"
        }
        
        Write-Log "✅ Script cleanup completed successfully"
        
    } catch {
        Write-Log "Script cleanup encountered minor issues: $_" "WARN"
    }
    ...
    if ($global:RestartChoice -ne "none") {
        ...
        Write-Log "=== FINAL SYSTEM CLEANUP BEFORE REBOOT (15-MINUTE TIMEOUTS) ==="
        try {
            Write-Log "Running final system cleanup commands before reboot with 15-minute timeouts..."
            $finalCleanupStart = Get-Date
            $maxFinalCleanupTime = 900  # 15 minutes maximum
            
            # 1. Windows Disk Cleanup with 5-minute timeout
            Write-Log "Running Windows Disk Cleanup (5-minute timeout)..."
            $cleanmgrJob = Start-Job {
                Start-Process "cleanmgr.exe" -WindowStyle Hidden -Wait
            }
            if (Wait-Job $cleanmgrJob -Timeout 300) {
                ...
                Remove-Job $cleanmgrJob
                Write-Log "⚠️ Windows Disk Cleanup timeout (5 minutes) - continuing..." "WARN"
                Get-Process -Name "cleanmgr" -ErrorAction SilentlyContinue | Stop-Process -Force
            }
            ...
            # 2. WinGet upgrade with 8-minute timeout
            Write-Log "Running WinGet upgrade (8-minute timeout)..."
            $wingetJob = Start-Job {
                Start-Process "winget" -ArgumentList "upgrade", "--all", "--accept-source-agreements", "--silent" -WindowStyle Hidden -Wait
            }
            if (Wait-Job $wingetJob -Timeout 480) {
                ...
                Stop-Job $wingetJob
                Remove-Job $wingetJob
                Write-Log "⚠️ WinGet upgrade timeout (8 minutes) - continuing..." "WARN"
            ...
            # 3. System Volume Information cleanup with 2-minute timeout
            Write-Log "Cleaning System Volume Information files (2-minute timeout)..."
            $sviJob = Start-Job {
            ...
            if (Wait-Job $sviJob -Timeout 120) {
                ...
                Stop-Job $sviJob
                Remove-Job $sviJob
                Write-Log "⚠️ System Volume Information cleanup timeout (2 minutes) - continuing..." "WARN"
            }
            ...
            # 4. Disk Cleanup sageset with 2-minute timeout
            Write-Log "Configuring Disk Cleanup settings (2-minute timeout)..."
            $sagesetJob = Start-Job {
                Start-Process "cleanmgr" -ArgumentList "/sageset:1" -WindowStyle Hidden -Wait
            }
            if (Wait-Job $sagesetJob -Timeout 120) {
                ...
                Stop-Job $sagesetJob
                Remove-Job $sagesetJob
                Write-Log "⚠️ Disk Cleanup sageset timeout (2 minutes) - continuing..." "WARN"
                Get-Process -Name "cleanmgr" -ErrorAction SilentlyContinue | Stop-Process -Force
            }
            ...
            # 5. Volume shadow copy deletion with 3-minute timeout
            Write-Log "Deleting volume shadow copies (3-minute timeout)..."
            $vssJob = Start-Job {
                Start-Process "vssadmin" -ArgumentList "delete", "shadows", "/for=C:", "/all", "/quiet" -WindowStyle Hidden -Wait
            }
            if (Wait-Job $vssJob -Timeout 180) {
                ...
                Stop-Job $vssJob
                Remove-Job $vssJob
                Write-Log "⚠️ Volume shadow copy deletion timeout (3 minutes) - continuing..." "WARN"
    ...
    # OPERATION 26: Final Script (Conditional based on user choice)
    if ($global:RestartChoice -ne "none") {
        $operationCount++
...
# START THE OPTIMIZATION PROCESS WITH ERROR HANDLING
try {
    Start-SystemOptimization
} catch {
    Write-Log "Script encountered an error during execution: $_" "ERROR"
    Write-Host "❌ Script encountered an error, but continuing to cleanup..." -ForegroundColor Red
} finally {
    # Ensure cleanup always runs even if script fails
    try {
        if ($global:progressJob) {
            Stop-Job $global:progressJob -ErrorAction SilentlyContinue
            Remove-Job $global:progressJob -ErrorAction SilentlyContinue
...
Write-Host "  • 🔧 Comprehensive corruption repair (17 operations: SFC, DISM, Enhanced CHKDSK, WinGet, Registry, Windows Update, Winsock, Store, Group Policy) 🔧" -ForegroundColor Magenta
Write-Host "  • 🌐 Comprehensive network repair (12 operations: DHCP, DNS, Hosts, ARP, NetBIOS, SSL, Adapters, Services) 🌐" -ForegroundColor Cyan
...
Write-Host "  • 🔧 Final system cleanup: cleanmgr, winget upgrade, shadow copy cleanup (15-min timeout) 🔧" -ForegroundColor Yellow
Write-Host "  • ✅ Clean script completion and terminal return ✅" -ForegroundColor Green
...
switch ($global:RestartChoice) {
    "ress" {
        ...
        Write-Host "• 🔧 Final system cleanup executed (cleanmgr, winget, shadow copies - 15-min timeout)" -ForegroundColor Yellow
    ...
    "none" {
        ...
        if (-not $SkipConfirmations) {
            ...
            if ($lastChance -match '^[Yy]') {
                ...
                Write-Host "🔧 Executing final system cleanup before restart (15-minute timeout)..." -ForegroundColor Yellow
                try {
                    Write-Log "Running final system cleanup commands before reboot with 15-minute timeouts..."
                    $finalCleanupStart = Get-Date
                    
                    # Quick cleanup with shorter timeouts since user is waiting
                    Write-Log "Running Windows Disk Cleanup (3-minute timeout)..."
                    $cleanmgrJob = Start-Job { Start-Process "cleanmgr.exe" -WindowStyle Hidden -Wait }
                    if (Wait-Job $cleanmgrJob -Timeout 180) {
                        Receive-Job $cleanmgrJob | Out-Null; Remove-Job $cleanmgrJob
                        Write-Log "✅ Windows Disk Cleanup completed"
                    } else {
                        Stop-Job $cleanmgrJob; Remove-Job $cleanmgrJob
                        Get-Process -Name "cleanmgr" -ErrorAction SilentlyContinue | Stop-Process -Force
                        Write-Log "⚠️ Windows Disk Cleanup timeout (3 minutes) - continuing..." "WARN"
                    }
                    
                    Write-Log "Running WinGet upgrade (5-minute timeout)..."
                    ...
                    if (Wait-Job $wingetJob -Timeout 300) {
                        Receive-Job $wingetJob | Out-Null; Remove-Job $wingetJob
                        Write-Log "✅ WinGet upgrade completed"
                    } else {
                        Stop-Job $wingetJob; Remove-Job $wingetJob
                        Get-Process -Name "winget" -ErrorAction SilentlyContinue | Stop-Process -Force
                        Write-Log "⚠️ WinGet upgrade timeout (5 minutes) - continuing..." "WARN"
                    }
...
Write-Host "• Final system cleanup executed (cleanmgr, winget, shadow copies - 15-min timeout - if rebooting)" -ForegroundColor Yellow

Write-Host "`n=== ANTI-HANGING PROTECTION ACTIVE ===" -ForegroundColor Green
Write-Host "✅ No global timeout - unlimited runtime for thorough optimization" -ForegroundColor Green
Write-Host "✅ Individual operation timeouts (15 minutes each)" -ForegroundColor Green  
Write-Host "✅ Temp cleanup timeout (15 minutes total)" -ForegroundColor Green
Write-Host "✅ Disk cleanup timeout (15 minutes total)" -ForegroundColor Green
Write-Host "✅ Process hanging detection (15-minute limit per operation)" -ForegroundColor Green
...
Write-Host "✅ Final system cleanup 15-minute timeout protection" -ForegroundColor Green
Write-Host "✅ Comprehensive corruption repair 15-minute timeout per operation (17 total operations)" -ForegroundColor Green
Write-Host "✅ System Volume Information aggressive cleanup protection" -ForegroundColor Green
Write-Host "✅ Clean script exit handling (no emergency cleanup triggered)" -ForegroundColor Green
...
Write-Log "Returning to terminal..."

# Ensure clean exit
try {
    # Final cleanup of any PowerShell events that might cause issues
    Get-EventSubscriber -ErrorAction SilentlyContinue | Unregister-Event -ErrorAction SilentlyContinue
    
    # Clear any error variables
    $Error.Clear()
...
# Normal script completion - no exit command needed, let PowerShell finish naturally