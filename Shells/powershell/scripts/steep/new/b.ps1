...
if (-not $SkipDocker) {
    ...
    $svcJob = Start-Job -ScriptBlock { Stop-Service com.docker.service, docker -Force -EA 0 }
    Wait-Job $svcJob -Timeout 10 | Out-Null
    Stop-Job $svcJob -EA 0; Remove-Job $svcJob -Force -EA 0

    # Shutdown WSL (Docker uses WSL2)
    wsl --shutdown 2>$null
    Start-Sleep -Seconds 2

    # Stop Hyper-V VMs with timeout
    Get-VM -EA 0 | Stop-VM -Force -TurnOff -EA 0
    Stop-Process -Name vmwp, vmcompute, vmms -Force -EA 0
    $hvJob = Start-Job -ScriptBlock { Stop-Service vmcompute, vmms -Force -EA 0 }
    Wait-Job $hvJob -Timeout 10 | Out-Null
    Stop-Job $hvJob -EA 0; Remove-Job $hvJob -Force -EA 0
    Start-Sleep -Seconds 2
...
if (-not $SkipCleanup) {
    ...
    # Windows Update: Force kill if net stop hangs
    ...
    $wuResult = Wait-Job $wuJob -Timeout 8
    if (-not $wuResult) {
        Stop-Job $wuJob -EA 0
        # Force kill the service process
        Get-Process -Name svchost -EA 0 | Where-Object {
            $_.Modules.ModuleName -contains 'wuaueng.dll'
        } | Stop-Process -Force -EA 0
    }
    Remove-Job $wuJob -Force -EA 0

    # Clear SoftwareDistribution (quick operation)
    ...
    # Restart Windows Update (don't wait for it)
    Start-Job -ScriptBlock { net start wuauserv 2>$null } | Out-Null
