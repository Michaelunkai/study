...
"@
    ...
    # OPERATION 24: Wise Disk Cleaner with spinning progress
    $operationCount++
    Write-ProgressLog "Wise Disk Cleaner advanced scan"
    try {
        if (Test-Path "F:\backup\windowsapps\installed\Wise\Wise Disk Cleaner\WiseDiskCleaner.exe") {
            Write-Log "Starting Wise Disk Cleaner with animated progress..."
            
            # Execute Wise Disk Cleaner with spinning progress indicator
            $sp='|/-\';$i=0;$p=Start-Process 'F:\backup\windowsapps\installed\Wise\Wise Disk Cleaner\WiseDiskCleaner.exe' -ArgumentList '-a','-adv' -WindowStyle Hidden -PassThru;while(!$p.HasExited){Write-Host -NoNewline "`r$($sp[$i++%$sp.Length]) disk deep-clean running…";Start-Sleep -Milliseconds 200};Write-Host "`r✓ disk deep-clean finished.     "
            
            Write-Log "Wise Disk Cleaner completed successfully"
        } else {
...
Write-Host "  • ✨ Wise Disk Cleaner advanced scan with spinning animation ✨" -ForegroundColor Cyan
...
Write-Host "• Wise Registry and Disk Cleaners completed with animated progress" -ForegroundColor Cyan
