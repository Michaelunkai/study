...
Start-Sleep -Seconds 5 # Give user time to read disclaimer
...
Try {
    ...
    Write-Host "RestoreHealth completed." -ForegroundColorGreen
} Catch {
...
# 14. Disk Clean-up Utility (Interactive)
Write-Host "`nStarting Disk Cleanup utility (cleanmgr.exe). Please follow prompts if any." -ForegroundColor Yellow
Try {
    Start-Process -FilePath "cleanmgr.exe" -Wait -ErrorAction SilentlyContinue | Out-Host
...
foreach ($dll in $wuDlls) {
    Try {
        ...
        # FIX: Added curly braces to $dll to resolve parser error
...
Try {
    Get-AppXPackage -AllUsers | ForEach-Object {
        Try {
            ...
            # FIX: Used $_.Name in string and original message to avoid direct ':' issue
...
Try {
    Get-NetAdapter | Where-Object {$_.Status -eq "Up"} | ForEach-Object {
        ...
        Try {
            ...
            $errorMessage = $_.Exception.Message # Use $_.Exception.Message directly
            # FIX: Added curly braces to ${adapterName} and ${errorMessage}
...
# 54. Clear Thumbnail Cache
Write-Host "`nClearing Thumbnail Cache..." -ForegroundColor Yellow
Try {
    # Disable cache to ensure files are released for deletion
    Set-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name DisableThumbnailCache -Value 1 -ErrorAction SilentlyContinue
    # Restart explorer to release lock on cache files
    taskkill /f /im explorer.exe | Out-Null
    Start-Sleep -Seconds 2
    # Delete cache files
    Remove-Item "$env:LOCALAPPDATA\Microsoft\Windows\Explorer\thumbcache_*.db" -Force -ErrorAction SilentlyContinue
    # Re-enable cache
    Set-ItemProperty "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name DisableThumbnailCache -Value 0 -ErrorAction SilentlyContinue
    # Start explorer again
    Start-Process explorer.exe | Out-Null
    Write-Host "Thumbnail Cache cleared." -ForegroundColor Green
} Catch {
    $errorMessage = $_.Exception.Message # Use $_.Exception.Message directly
    # FIX: Added curly braces to ${errorMessage}
...
Try {
    # Backup repository (optional but recommended for critical systems)
    # winmgmt /backup C:\WMI_Backup_$(Get-Date -Format 'yyyyMMdd_HHmmss')
...
# 60. Flush and rebuild icon cache
Write-Host "`nFlushing and rebuilding Icon Cache..." -ForegroundColor Yellow
Try {
    taskkill /f /im explorer.exe -ErrorAction SilentlyContinue | Out-Null
...
# 61. Clear all Event Logs
Write-Host "`nClearing ALL Event Logs (System, Application, Security, Setup, etc.)..." -ForegroundColor Yellow
Try {
    Get-WinEvent -ListLog * | ForEach-Object {
        Try {
            Clear-EventLog -LogName $_.LogName -ErrorAction Stop
            Write-Host "  - Cleared log: $($_.LogName)" -ForegroundColor DarkGreen
        } Catch {
            $errorMessage = $_.Exception.Message # Use $_.Exception.Message directly
            # FIX: Added curly braces to $($_.LogName) and ${errorMessage}
            Write-Warning "  - Failed to clear log $($_.LogName): ${errorMessage}"
        }
    }
...
Try {
    reg delete HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\Safer /f /va /reg:64 -ErrorAction SilentlyContinue | Out-Null
    reg delete HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\Safer /f /va /reg:32 -ErrorAction SilentlyContinue | Out-Null
...
# 65. Clear Driver Store conflicts
...
Try {
    ...
    foreach ($driver in $orphanedDrivers) {
        Try {
            ...
            pnputil /delete-driver $driver /uninstall /force -ErrorAction Stop | Out-Null
            Write-Host "  - Uninstalled orphaned driver: ${driver}" -ForegroundColor DarkGreen
        } Catch {
            $errorMessage = $_.Exception.Message # Use $_.Exception.Message directly
            # FIX: Added curly braces to ${driver} and ${errorMessage}
...
# 66. Reset Windows Store Cache
Write-Host "`nResetting Windows Store cache..." -ForegroundColor Yellow
Try {
    wsreset.exe -ErrorAction SilentlyContinue | Out-Null
...
Try {
    ...
    Start-Process -FilePath "dism.exe" -ArgumentList "/online /enable-feature /featurename:NetFx4Extended-A" -WindowStyle Hidden -Wait -ErrorAction SilentlyContinue
    ...
    Get-ChildItem -Path "$env:SystemDrive\Users\" -Directory | ForEach-Object {
        ...
        if (Test-Path $userTempPath -PathType Container) {
            ...
            if ($_.Name -ne $env:UserName) {
                ...
                Remove-Item -Path "${userTempPath}\*" -Recurse -Force -ErrorAction SilentlyContinue # FIX: Added curly braces to ${userTempPath}
            }
        }
    }
...
# 74. Empty Recycle Bins for all drives
...
Try {
    ...
    foreach ($drive in $drives) {
        ...
        if (Test-Path $recycleBinPath) {
            Write-Host "  - Emptying Recycle Bin for drive: $($drive.Root)" -ForegroundColor DarkYellow
            # This is a bit tricky; a direct Remove-Item on $Recycle.Bin content often fails due to permissions.
            # Using cmd /c RD /S /Q can be more effective but needs care.
            # A more PowerShell native way using COM object:
            $shell = New-Object -ComObject Shell.Application
            $shellFolder = $shell.NameSpace(10) # 10 is the Recycle Bin constant
            if ($shellFolder) {
...
# 88. Clear User Credentials Cache
...
Try {
    ...
    foreach ($line in $credList) {
        ...
        if ($target -ne "LegacyGeneric:contains(.)" -and $target -ne "") {
            Try {
                cmdkey /delete:$target -ErrorAction Stop | Out-Null
                Write-Host "  - Deleted credential for target: ${target}" -ForegroundColor DarkGreen
            } Catch {
                # FIX: Added curly braces to ${target} and ${errorMessage}
...
Try {
    Get-ScheduledTask | ForEach-Object {
        Try {
            ...
            # Attempt to register/update (might cause errors for some built-in tasks)
            # This is more for fixing broken tasks that can be re-registered from scratch
            # schtasks /change /tn "$($_.TaskPath)$($_.TaskName)" /enable | Out-Null
        } Catch {
            # FIX: Used $_.TaskName in string and original message to avoid direct ':' issue
...
Try {
    netsh advfirewall delete rule name=all | Out-Null # Delete all existing rules
    netsh advfirewall set currentprofile state on | Out-Null # Ensure firewall is on
    netsh advfirewall firewall show rule name=all > $null # Force internal rebuild/load defaults
...
# --- Final Message and Restart Recommendation ---
Write-Host "`n--- Script Execution Completed ---" -ForegroundColor Green
Write-Host "Many repair and optimization operations have been attempted." -ForegroundColor Green
...
# Read-Host "Press Enter to restart your computer, or close this window to restart manually."
