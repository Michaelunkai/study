    ...
    Process {
        Try {
            ...
            foreach ($disk in $disks.InstanceName) {
                ...
                # View full table with 
                #$Output
                
                $diskData = [PSCustomObject]@{
                    "Reallocated Sector Count"                       = ($Output | Where-Object ID -eq 5 | Select-Object -ExpandProperty RawValue)
                    ...
                    "Power On Hours"                                 = ($Output | Where-Object ID -eq 9 | Select-Object -ExpandProperty RawValue)                    
            ...
            if ($Warning) {
                ...
                $data.GetEnumerator() | Foreach-Object {
                    ...
                    $_.Value.psobject.Members | ForEach-Object {
                        $item = $_
                        switch ($_.Name) {
                            #Anything in this section will cause the script to return warning.
                            "Power On Hours" { if ($null -ne $item.Value -and $item.Value -gt 50000) { $diskWarnings.Add($item.Name, $item.Value) } } #Remove line or adjust 50000 number if you don't want Old drives to start returning Warnings
                            "Reallocated Sector Count" { if ($null -ne $item.Value -and $item.Value -gt 1) { $diskWarnings.Add($item.Name, $item.Value) } }
