...
$DisablePasswordAuthentication = $False # Set to $False to allow password authentication
$DisablePubkeyAuthentication = $False   # Set to $False to allow public key authentication
$AutoStartSSHD = $true
...
# ---
## Remove Built-In OpenSSH and Existing Services
...
if (Test-Path $sshdConfigSource) {
    ...
    # Remove existing PasswordAuthentication and PubkeyAuthentication lines if they exist, then add new ones
    (Get-Content $sshdConfigDest) | Where-Object { $_ -notmatch '^\s*PasswordAuthentication' -and $_ -notmatch '^\s*PubkeyAuthentication' } | Set-Content $sshdConfigDest
    Add-Content -Path $sshdConfigDest -Value "PasswordAuthentication $($DisablePasswordAuthentication -as [int])" -ErrorAction Stop
    Add-Content -Path $sshdConfigDest -Value "PubkeyAuthentication $($DisablePubkeyAuthentication -as [int])" -ErrorAction Stop
...
#AuthenticationMethods publickey password # This line is often problematic, letting OpenSSH decide
...
"@
    ...
    # Apply authentication settings based on variables
    (Get-Content $sshdConfigDest) | Where-Object { $_ -notmatch '^\s*PasswordAuthentication' -and $_ -notmatch '^\s*PubkeyAuthentication' } | Set-Content $sshdConfigDest
    Add-Content -Path $sshdConfigDest -Value "PasswordAuthentication $($DisablePasswordAuthentication -as [int])" -ErrorAction Stop
    Add-Content -Path $sshdConfigDest -Value "PubkeyAuthentication $($DisablePubkeyAuthentication -as [int])" -ErrorAction Stop
}
...
# Ensure GSSAPIAuthentication is set to yes if not already present
$configContent = Get-Content $sshConfigPath -Raw
if ($configContent -notmatch 'GSSAPIAuthentication yes') {
    Add-Content -Path $sshConfigPath -Value "`r`nGSSAPIAuthentication yes" -ErrorAction Stop
}


# ---
## Configure and Start Services, Add Firewall Rule, Set Shell

# Now configure the service
Write-Host "Configuring sshd service" -ForegroundColor Green
try {
    # Set to 'Automatic' initially to ensure it starts permanently if $AutoStartSSHD is true
...
if ($AutoStartSSHD) {
    ...
    # If not autostart, set to Manual
    Write-Host "Setting sshd service to Manual start as AutoStartSSHD is false" -ForegroundColor Green
    try {
        Set-Service -Name sshd -StartupType Manual
        Write-Host "sshd service set to manual startup" -ForegroundColor Green
    }
    catch {
        Write-Warning "Failed to set sshd service to manual startup: $_"
    }
}
...
if ($AutoStartSSHAGENT) {
    ...
    # If not autostart, set to Manual
    Write-Host "Setting ssh-agent service to Manual start as AutoStartSSHAGENT is false" -ForegroundColor Green
    try {
        if (Get-Service ssh-agent -ErrorAction SilentlyContinue) {
            Set-Service -Name ssh-agent -StartupType Manual
            Write-Host "ssh-agent service set to manual startup" -ForegroundColor Green
        } else {
            Write-Warning "ssh-agent service not found"
        }
    }
    catch {
        Write-Warning "Failed to set ssh-agent service to manual startup: $_"
    }
}
...
if ($existingPath -notmatch [regex]::Escape($InstallPath)) { # Use regex::Escape for special characters in path
