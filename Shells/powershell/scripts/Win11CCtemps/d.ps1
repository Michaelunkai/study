...
# Function to add file/folder to pending deletion on reboot (silent mode)
    ...
    try {
        ...
        if (Test-Path $Path) {
            if ((Get-Item $Path).PSIsContainer) {
                $size = Get-FolderSizeMB -Path $Path
        ...
        # Silent mode - no individual file output, just count them
        return $true
    }
    catch {
        # Silent mode - don't show individual failures
        return $false
    }
}
...
# Enhanced function to safely clean directory contents with timeout and bulk operations
    ...
    try {
        if (Test-Path $Path) {
            ...
            if ($sizeBefore -gt 0) {
                ...
                if (!$WhatIf) {
                    # Method 1: Try normal deletion first (with 30 second timeout)
                    try {
                        $job = Start-Job -ScriptBlock {
                            param($dir)
                            Get-ChildItem -Path $dir -Recurse -Force -ErrorAction Stop | 
                        ...
                        if ($completed) {
                            ...
                            Stop-Job -Job $job
                            throw "Deletion timeout after 30 seconds"
                        }
                        Remove-Job -Job $job -Force
                    }
                    catch {
                        Write-Host "  Normal deletion failed/timeout: $($_.Exception.Message)" -ForegroundColor Yellow
                        
                        # Method 2: Try with takeown and icacls (with 15 second timeout)
                        try {
                            ...
                            if ($completed2) {
                                ...
                                Stop-Job -Job $job2
                                throw "Advanced deletion timeout"
                            }
                            Remove-Job -Job $job2 -Force
                        }
                        catch {
                            # Method 3: Fast bulk scheduling for reboot deletion
                            Write-Host "  Files locked, bulk scheduling for reboot deletion..." -ForegroundColor Yellow
                            
                            try {
                                # Just schedule the entire directory instead of enumerating individual files
                                Add-PendingDeletion -Path $Path
                                $script:lockedDirectories += $Path
                                
                                # Estimate file count based on size (much faster than enumeration)
                                $estimatedFiles = [math]::Max(1, [math]::Round($sizeBefore * 50, 0))  # Rough estimate
                                Write-Host "  → Directory scheduled for bulk reboot deletion (~$estimatedFiles files)" -ForegroundColor Magenta
                            }
                            catch {
                    ...
                    # Get size after cleanup attempt (with timeout)
                    try {
                        ...
                        $completed3 = Wait-Job -Job $job3 -Timeout 5
                        if ($completed3) {
                            $sizeAfter = Receive-Job -Job $job3
                        } else {
                            Stop-Job -Job $job3
                            $sizeAfter = $sizeBefore  # Assume nothing was deleted if we can't measure
                        }
                        Remove-Job -Job $job3 -Force
                    }
                    catch {
                        $sizeAfter = $sizeBefore  # Assume nothing was deleted on error
                    }
        ...
        if (!$WhatIf) {
            Write-Host "  Scheduling for reboot deletion..." -ForegroundColor Yellow
            Add-PendingDeletion -Path $Path
        }
        return 0
    }
...
# Function to detect running processes that might lock files (but don't stop them)
function Get-LockingProcesses {
    Write-Host "Detecting running processes that might lock cache files..." -ForegroundColor Cyan
    ...
    $runningLockingProcesses = @()
    
    foreach ($process in $lockingProcesses) {
        try {
            $processes = Get-Process -Name $process -ErrorAction SilentlyContinue
            if ($processes) {
                Write-Host "  Detected running: $process (files may be locked)" -ForegroundColor Yellow
                $runningLockingProcesses += $process
            }
        }
        catch {
    ...
    if ($runningLockingProcesses.Count -eq 0) {
        Write-Host "  No locking processes detected" -ForegroundColor Green
    }
    
    return $runningLockingProcesses
}
    ...
    foreach ($profile in $userProfiles) {
        ...
        if (Test-Path $userPath) {
            ...
            $userTempPaths = @(
                ...
                # Wise Utilities Complete Cache Set
                "$userPath\AppData\Roaming\Wise Utilities\EBWebView\Default\Cache",
                "$userPath\AppData\Roaming\Wise Utilities\EBWebView\Default\DawnGraphiteCache",
                "$userPath\AppData\Roaming\Wise Utilities\EBWebView\Default\DawnWebGPUCache",
                "$userPath\AppData\Roaming\Wise Utilities\EBWebView\Default\GPUCache",
                "$userPath\AppData\Roaming\Wise Utilities\EBWebView\GraphiteDawnCache",
                "$userPath\AppData\Roaming\Wise Utilities\EBWebView\ShaderCache",
                "$userPath\AppData\Roaming\Wise Utilities\EBWebView\GrShaderCache",
                ...
                # DirectX Shader Cache (all subfolders)
...
Write-Host "=======================================" -ForegroundColor White
Write-Host "RUNNING IN FULLY AUTOMATIC MODE (NON-INTRUSIVE)" -ForegroundColor Green
Write-Host "Running processes will NOT be stopped - cache files may be rescheduled for reboot deletion" -ForegroundColor Yellow
...
# Detect processes that might lock files (but don't stop them)
$lockingProcesses = @()
if (!$WhatIf) {
...
# Add specific D3DSCache directories for all users
Write-Host "Scanning for DirectX shader cache directories..." -ForegroundColor Green
$d3dCacheDirectories = @()
$userProfiles = Get-WmiObject -Class Win32_UserProfile | Where-Object { $_.Special -eq $false }
foreach ($profile in $userProfiles) {
    $userPath = $profile.LocalPath
    if (Test-Path $userPath) {
        $d3dCachePath = "$userPath\AppData\Local\D3DSCache"
        if (Test-Path $d3dCachePath) {
            $d3dSubfolders = Get-ChildItem -Path $d3dCachePath -Directory -ErrorAction SilentlyContinue
            foreach ($subfolder in $d3dSubfolders) {
                $d3dCacheDirectories += $subfolder.FullName
                Write-Host "  Found D3D cache: $($subfolder.FullName)" -ForegroundColor Gray
            }
        }
    }
}
...
$allDirectories = $systemDirectoriesToClean + $userTempDirectories + $d3dCacheDirectories | Sort-Object | Get-Unique

Write-Host "Starting cleanup of $($allDirectories.Count) directories..." -ForegroundColor Green
Write-Host "  → System directories: $($systemDirectoriesToClean.Count)" -ForegroundColor Cyan
Write-Host "  → User cache/temp directories: $($userTempDirectories.Count)" -ForegroundColor Cyan
Write-Host "  → D3D shader cache directories: $($d3dCacheDirectories.Count)" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "→ Space freed immediately: $([math]::Round($totalFreed, 2)) MB ($([math]::Round($totalFreed / 1024, 2)) GB)" -ForegroundColor Green
...
# Total summary
$totalSpace = $totalFreed + $script:pendingDeletionSize
Write-Host ""
...
Write-Host "→ Freed now: $([math]::Round($totalFreed, 2)) MB | After reboot: $([math]::Round($script:pendingDeletionSize, 2)) MB" -ForegroundColor White -BackgroundColor DarkBlue

# Show detected running processes that might have caused locks
if ($lockingProcesses.Count -gt 0) {
    Write-Host ""
    Write-Host "DETECTED RUNNING PROCESSES:" -ForegroundColor Yellow
    Write-Host "→ Running apps that may have locked files: $($lockingProcesses -join ', ')" -ForegroundColor Yellow
    Write-Host "→ These processes were NOT stopped to avoid disruption" -ForegroundColor Yellow
}
Write-Host ""
