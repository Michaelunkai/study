...
if ($FullSystemMaintenance) {
    Write-Host "FULL SYSTEM MAINTENANCE MODE ENABLED - May take 30+ minutes" -ForegroundColor Red -BackgroundColor Yellow
}
...
# STEP 4: ENHANCED TEMP DIRECTORY CLEANUP
Write-Host ""
Write-Host "STEP 4: Enhanced temp directory cleanup..." -ForegroundColor Green
...
if (!$WhatIf) {
    ...
    Write-Host "Cleaning additional cache directories..." -ForegroundColor Cyan
    try {
    ...
    Write-Host "  [WHATIF] Would clean enhanced temp directories and caches" -ForegroundColor Magenta
}

# STEP 5: NETWORK AND SYSTEM RESETS
Write-Host ""
Write-Host "STEP 5: Network and system resets..." -ForegroundColor Green
...
if (!$WhatIf) {
    ...
    Write-Host "  [WHATIF] Would reset network stack and update system policies" -ForegroundColor Magenta
}

# STEP 6: ADVANCED SYSTEM MAINTENANCE (Optional)
if ($FullSystemMaintenance -and !$WhatIf) {
    Write-Host ""
    Write-Host "STEP 6: Advanced system maintenance (This may take 30+ minutes)..." -ForegroundColor Red -BackgroundColor Yellow
    ...
    Write-Host "To schedule memory diagnostic (requires reboot), run: mdsched.exe" -ForegroundColor Gray
...
if (!$FullSystemMaintenance) {
    ...
    Write-Host ""
    Write-Host "⚠️  Note: Full maintenance can take 30+ minutes" -ForegroundColor Yellow
}
...
Write-Host "• Memory Diagnostic: mdsched.exe (requires reboot)" -ForegroundColor Gray
