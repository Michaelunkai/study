...
# --- NEW: Winget Uninstall Phase ---
Write-Host "`nPhase 0: Winget Uninstall..." -ForegroundColor Cyan
foreach ($toolName in $toolNames) {
    try {
        Write-Host "Running: winget uninstall --silent --accept-source-agreements --accept-package-agreements $toolName" -ForegroundColor Yellow
        $wingetResult = winget uninstall --silent --accept-source-agreements --accept-package-agreements $toolName 2>&1
        if ($wingetResult -match "No installed package found") {
            Write-Host "Winget: $toolName not found" -ForegroundColor DarkGray
        } else {
            Write-Host "Winget attempted uninstall for: $toolName" -ForegroundColor Green
        }
    } catch {
        Write-Host "Winget uninstall failed for ${toolName}: $($_.Exception.Message)" -ForegroundColor Yellow
    }
}
