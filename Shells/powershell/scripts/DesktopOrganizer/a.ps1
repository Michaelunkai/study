...
function Create-FolderShortcut {
    ...
    if ($ShortcutName -eq "Recycle Bin" -or $ShortcutName -like "*Recycle*") {
        Write-Host "[BLOCKED] BLOCKED: Will NOT create Recycle Bin shortcut to prevent duplicates!" -ForegroundColor Red
        return
    }
...
if ($RecycleBins.Count -gt 1) {
    ...
    for ($i = 1; $i -lt $SortedRecycleBins.Count; $i++) {
        ...
        Write-Host "[OK]️ ELIMINATED DUPLICATE RECYCLE BIN: $($SortedRecycleBins[$i].Name)" -ForegroundColor Red
    }
}
...
Write-Host "[BLOCKED] RECYCLE BIN CREATION: PERMANENTLY BLOCKED" -ForegroundColor Red
Write-Host "[FOLDER] GAMES FOLDER: Created and VBScript executed!" -ForegroundColor Cyan
