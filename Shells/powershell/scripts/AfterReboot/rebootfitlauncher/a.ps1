# Bulletproof FitFit Script - PowerShell 5 Compatible (FIXED with FitFit Function)
...
Write-Host "=== BULLETPROOF FITFIT SCRIPT - PS5 COMPATIBLE ===" -ForegroundColor Cyan
...
$oldTasks = @("KeyboardSleepTask", "PostBootPowerConfig", "PostLoginPowerConfig", "PostLoginPowerShell", "AutoSleepTask", "AutoFitFitTask")
...
# STEP 2: Create the fitfit script
Write-Host ""
Write-Host "=== CREATING FITFIT SCRIPT ===" -ForegroundColor Yellow

$scriptDir = "C:\Windows\Temp"
$scriptPath = Join-Path $scriptDir "AutoFitFit.ps1"
...
# Create the fitfit script content
$fitfitScript = @'
# Auto FitFit Script - PowerShell 5 Compatible
param()

$logPath = "C:\Windows\Temp\AutoFitFit.log"
...
function Run-FitFit {
    Write-Log "Starting FitFit function..."

    try {
        Write-Log "Executing 'fitfit' command in PowerShell..."
        
        # Execute fitfit command
        $fitfitResult = Invoke-Expression "fitfit" 2>&1
        
        if ($fitfitResult) {
            Write-Log "FitFit output: $fitfitResult"
        } else {
            Write-Log "FitFit executed successfully with no output"
        }
        
        Write-Log "FitFit command completed successfully"

    } catch {
        Write-Log "Error in FitFit function: $($_.Exception.Message)"
        
        # Fallback - try running fitfit via cmd
        Write-Log "Attempting fallback fitfit execution via cmd..."
        try {
            $cmdResult = cmd /c "powershell -Command fitfit" 2>&1
            Write-Log "Fallback fitfit result: $cmdResult"
        } catch {
...
try {
    Write-Log "AutoFitFit script started"

    Write-Log "Waiting 10 seconds after login..."
    Start-Sleep -Seconds 10
    Write-Log "10 seconds elapsed, executing FitFit function"

    # Execute the FitFit function
    Run-FitFit

    Write-Log "FitFit function executed successfully"

} catch {
    Write-Log "Error in main script: $($_.Exception.Message)"

    # Fallback fitfit execution
    Write-Log "Attempting fallback fitfit execution"
    try {
        $fallbackResult = cmd /c "powershell -Command fitfit" 2>&1
        Write-Log "Fallback fitfit executed: $fallbackResult"
    } catch {
        Write-Log "Fallback failed: $($_.Exception.Message)"
    }
}
...
try {
    ...
    cmd /c "schtasks /delete /tn AutoFitFitTask /f" 2>$null
    Write-Log "Deleted scheduled task"

    # Create self-deletion script
    $deleteScript = @"
Start-Sleep -Seconds 5
Remove-Item 'C:\Windows\Temp\AutoFitFit.ps1' -Force -ErrorAction SilentlyContinue
Remove-Item 'C:\Windows\Temp\DeleteAutoFitFit.ps1' -Force -ErrorAction SilentlyContinue
"@

    $deleteScript | Out-File -FilePath "C:\Windows\Temp\DeleteAutoFitFit.ps1" -Encoding ASCII -Force
    Start-Process -FilePath "powershell.exe" -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -File C:\Windows\Temp\DeleteAutoFitFit.ps1" -WindowStyle Hidden
...
Write-Log "AutoFitFit script completed"
'@

# Write the script to file
try {
    $fitfitScript | Out-File -FilePath $scriptPath -Encoding ASCII -Force
    Write-Host "FitFit script created successfully at: $scriptPath" -ForegroundColor Green
...
if (-not $SkipReboot) {
    ...
    Write-Host "  • Execute 'fitfit' command in PowerShell" -ForegroundColor White
    Write-Host "  • Task will delete itself permanently" -ForegroundColor White
    Write-Host "  • Log file will be created at: C:\Windows\Temp\AutoFitFit.log" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "Task name: $taskName" -ForegroundColor Gray
    Write-Host "Log file: C:\Windows\Temp\AutoFitFit.log" -ForegroundColor Gray
    Write-Host ""
    Write-Host "To test manually run the script file directly" -ForegroundColor Yellow
    Write-Host "To check task status: schtasks /query /tn AutoFitFitTask" -ForegroundColor Yellow
}
