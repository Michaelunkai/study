...
# STEP 2: Get custom command from user or parameter
Write-Host ""
...
if ([string]::IsNullOrWhiteSpace($Command)) {
    ...
    Write-Host "  - qbit; update" -ForegroundColor Gray
    ...
    Write-Host "Command from parameter: $customCommand" -ForegroundColor Green
}
...
# Create a unique flag with timestamp to ensure one-time execution
try {
    $flagContent = @{
    ...
    Write-Host "Execution flag created: $flagPath" -ForegroundColor Green
} catch {
    Write-Host "FAILED to create execution flag: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}

# Create the custom command script content
$customCommandScript = @'
# Auto Custom Command Script - PowerShell 5 Compatible - ONE TIME EXECUTION
param()
...
function Check-ExecutionFlag {
    Write-Log "Checking execution flag..."
    
    if (-not (Test-Path $flagPath)) {
        Write-Log "ERROR: Execution flag not found. Command may have already been executed."
        return $false
    }

    try {
        $flagContent = Get-Content $flagPath -Raw | ConvertFrom-Json
        if ($flagContent.Executed -eq $true) {
            Write-Log "ERROR: Command already executed according to flag."
            return $false
        }
        Write-Log "Flag verified - command ready for execution"
        return $true
    } catch {
        Write-Log "ERROR: Could not read execution flag: $($_.Exception.Message)"
        return $false
    }
}

function Update-ExecutionFlag {
    try {
        if (Test-Path $flagPath) {
            ...
            $flagContent | ConvertTo-Json | Out-File -FilePath $flagPath -Encoding UTF8 -Force
            Write-Log "Execution flag updated - marked as executed"
        }
    } catch {
...
function Run-CustomCommand {
    ...
    # Check if we should execute (one-time only)
    if (-not (Check-ExecutionFlag)) {
        Write-Log "Execution cancelled - command already ran or flag missing"
        return
    }
    ...
    try {
        ...
        # Mark as executed BEFORE running command to prevent re-execution
        Update-ExecutionFlag
...
try {
    ...
    # Execute the Custom Command function (one-time only)
    Run-CustomCommand

    Write-Log "Custom Command function completed"

} catch {
    Write-Log "Error in main script: $($_.Exception.Message)"

    # Emergency fallback - only if flag allows
    if (Check-ExecutionFlag) {
        Write-Log "Attempting emergency fallback command execution"
        try {
            if (Test-Path $commandPath) {
                ...
                Write-Log "Emergency fallback command executed: $fallbackResult"
            }
        } catch {
            Write-Log "Emergency fallback failed: $($_.Exception.Message)"
        }
    }
}
...
try {
    ...
    # Delete the command file and flag
    Remove-Item $commandPath -Force -ErrorAction SilentlyContinue
    Remove-Item $flagPath -Force -ErrorAction SilentlyContinue
    Write-Log "Deleted command file and execution flag"
...
if (-not $SkipReboot) {
    ...
    Write-Host "  • Command will run ONLY ONCE (first login after reboot)" -ForegroundColor Yellow
    ...
    Write-Host ""
    Write-Host "IMPORTANT: Command will run ONLY ONCE after next login!" -ForegroundColor Yellow
