...
# Script to install Go version 1.7.6, set up the environment, and ensure it works in the current shell without refreshing.
...
# Download and install Go version 1.7.6
wget https://go.dev/dl/go1.7.6.linux-amd64.tar.gz
sudo tar -C /usr/local -xzf go1.7.6.linux-amd64.tar.gz
rm go1.7.6.linux-amd64.tar.gz
...
go version
if [[ $(go version) != *"go1.7.6"* ]]; then
    echo "Error: Go version is not 1.7.6. Exiting."
    exit 1
fi
