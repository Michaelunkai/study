...
# Script to install Go version 1.10.8, set up the environment, and ensure it works in the current shell without refreshing.
...
# Download and install Go version 1.10.8
...
go version
if [[ $(go version) != *"go1.10.8"* ]]; then
    echo "Error: Go version is not 1.10.8. Exiting."
    exit 1
fi
