...
# Script to install Go version 1.16.15, set up the environment, and ensure it works in the current shell without refreshing.
...
# Download and install Go version 1.16.15
...
go version
if [[ $(go version) != *"go1.16.15"* ]]; then
    echo "Error: Go version is not 1.16.15. Exiting."
    exit 1
fi
