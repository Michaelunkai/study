...
for home_dir in /home/* /root; do
    if [ -d "$home_dir" ]; then
        ...
        (sudo rm -rf "$home_dir"/.npm "$home_dir"/.yarn "$home_dir"/.cargo/registry "$home_dir"/.cargo/git 2>/dev/null || true) &
...
# Remove ALL Python package caches and compiled files (parallel)
