...
# Script to install Go version 1.9.7, set up the environment, and ensure it works in the current shell without refreshing.
...
# Download and install Go version 1.9.7
...
go version
if [[ $(go version) != *"go1.9.7"* ]]; then
    echo "Error: Go version is not 1.9.7. Exiting."
    exit 1
fi
