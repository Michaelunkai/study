...
# Script to install Go version 1.24.5, set up the environment, and ensure it works in the current shell without refreshing.
...
# Download and install Go version 1.24.5
...
go version
if [[ $(go version) != *"go1.24.5"* ]]; then
    echo "Error: Go version is not 1.24.5. Exiting."
    exit 1
fi
