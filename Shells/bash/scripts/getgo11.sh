...
# Script to install Go version 1.11.13, set up the environment, and ensure it works in the current shell without refreshing.
...
# Download and install Go version 1.11.13
wget https://go.dev/dl/go1.11.13.linux-amd64.tar.gz
...
go version
if [[ $(go version) != *"go1.11.13"* ]]; then
    echo "Error: Go version is not 1.11.13. Exiting."
    exit 1
fi
