...
# Script to install Go version 1.4.3, set up the environment, and ensure it works in the current shell without refreshing.
...
# Download and install Go version 1.4.3
...
go version
if [[ $(go version) != *"go1.4.3"* ]]; then
    echo "Error: Go version is not 1.4.3. Exiting."
    exit 1
fi
