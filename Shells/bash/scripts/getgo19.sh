...
# Script to install Go version 1.19.12, set up the environment, and ensure it works in the current shell without refreshing.
...
# Download and install Go version 1.19.12
...
go version
if [[ $(go version) != *"go1.19.12"* ]]; then
    echo "Error: Go version is not 1.19.12. Exiting."
    exit 1
fi
