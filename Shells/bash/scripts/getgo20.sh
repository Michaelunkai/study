...
# Script to install Go version 1.20.9, set up the environment, and ensure it works in the current shell without refreshing.
...
go version
if [[ $(go version) != *"go1.20.9"* ]]; then
    echo "Error: Go version is not 1.20.9. Exiting."
    exit 1
fi
