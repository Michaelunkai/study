...
# Script to install Go version 1.22.10, set up the environment, and ensure it works in the current shell without refreshing.
...
# Download and install Go version 1.22.10
...
go version
if [[ $(go version) != *"go1.22.10"* ]]; then
    echo "Error: Go version is not 1.22.10. Exiting."
    exit 1
fi
