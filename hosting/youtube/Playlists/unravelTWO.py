# File: YOUR_CLIENT_SECRET_HERElist.py
import os
import pickle
    ...
    if not credentials or not credentials.valid:
        if credentials and credentials.expired and credentials.refresh_token:
            ...
            if not os.path.exists(client_secrets_path):
                raise FileNotFoundError(f"{client_secrets_path} not found. Please ensure it is in the same directory as the script.")
...
if __name__ == '__main__':
    try:
        ...
        # Create a new public playlist named "unravel two".
        playlist_title = "unravel two"
        playlist_description = "All published videos with 'unravel two' in the title, from oldest to newest."
        ...
        # Add every video with "unravel two" in its title to the new playlist in order from oldest to newest.
        YOUR_CLIENT_SECRET_HEREorted(youtube_service, new_playlist_id, "unravel two")
