...
class ProcessDetailsWindow(tk.Toplevel):
    def __init__(self, master, process_info):
        super().__init__(master)
        self.title("Process Details")
        self.geometry("600x400")

        self.process_info = process_info

        self.detail_label = ttk.Label(self, text="Process Details", font=("Helvetica", 16))
        self.detail_label.pack(pady=10)

        self.detail_text = tk.Text(self, wrap="word", font=("Helvetica", 12))
...
class ProcessManagerApp:
    def __init__(self, master):
        ...
        self.header_label = ttk.Label(master, text="Task Manager", font=("Helvetica", 20))
        self.header_label.grid(row=0, column=0, columnspan=8, padx=10, pady=10)

        self.search_label = ttk.Label(master, text="Search process:", font=("Helvetica", 12))
        ...
        self.search_button = ttk.Button(master, text="Search", command=self.search_process, style="Custom.TButton")
        self.search_button.grid(row=1, column=2, padx=10, pady=5)

        self.refresh_button = ttk.Button(master, text="Refresh", command=self.refresh_process_list, style="Custom.TButton")
        self.refresh_button.grid(row=1, column=3, padx=10, pady=5)

        self.memory_button = ttk.Button(master, text="Memory", command=self.sort_by_memory, style="Custom.TButton")
        self.memory_button.grid(row=1, column=4, padx=10, pady=5)

        self.cpu_button = ttk.Button(master, text="CPU", command=self.sort_by_cpu, style="Custom.TButton")
        self.cpu_button.grid(row=1, column=5, padx=10, pady=5)

        self.detail_button = ttk.Button(master, text="Details", command=self.show_process_details, style="Custom.TButton")
        self.detail_button.grid(row=1, column=6, padx=10, pady=5)

        self.kill_button = ttk.Button(master, text="Kill", command=self.kill_process, style="Custom.TButton")
        self.kill_button.grid(row=1, column=7, padx=10, pady=5)

        self.cpu_label = ttk.Label(master, text="CPU Usage: -", font=("Helvetica", 12))
        self.cpu_label.grid(row=2, column=0, padx=10, pady=5, sticky="w")

        self.memory_label = ttk.Label(master, text="Memory Usage: -", font=("Helvetica", 12))
        self.memory_label.grid(row=2, column=1, padx=10, pady=5, sticky="w")

        self.process_tree = ttk.Treeview(master, columns=("PID", "Name", "CPU%", "Memory%"), show="headings", height=15)
        ...
        self.process_tree.grid(row=3, column=0, columnspan=7, padx=10, pady=5, sticky="nsew")
        ...
        self.style.configure("Treeview", background="#f0f0f0", foreground="black", fieldbackground="#d3d3d3", font=("Helvetica", 10))
    ...
    def show_process_details(self):
        ...
        if selected_item:
            ...
            process_info = {
                ...
                "CPU%": f"{selected_process.cpu_percent():.2f}%",
                "Memory%": f"{selected_process.memory_percent():.2f}%"
            }
            details_window = ProcessDetailsWindow(self.master, process_info)
    ...
    def kill_process_thread(self):
        ...
        if selected_item:
            ...
            try:
                ...
                messagebox.showinfo("Process Terminated", f"The process with PID {selected_pid} has been terminated successfully.")
            except psutil.AccessDenied:
                messagebox.showerror("Access Denied", "Permission to terminate the selected process is denied.")
            except psutil.NoSuchProcess:
                messagebox.showerror("Process Not Found", "The selected process is no longer running.")
            self.update_process_list_thread()
