...
def view_wishlist():
    ...
    for index, category in enumerate(categories):
        ...
        if items:
            ...
            category_label.pack()
            category_list = tk.Listbox(category_frame, height=20, width=30, bg="black", fg="white", selectmode=tk.MULTIPLE)
            for item in items:
    ...
    for category, listbox in listbox_widgets.items():
        listbox.bind('<Delete>', lambda event, cat=category, listbox=listbox: delete_selected_items(cat, listbox))
