...
// 404 handler for unknown API routes (must be before static/catch-all)
app.use('/api/*', (req, res) => {
  res.status(404).json({
...
// Serve React build (client/dist) as static SPA
...
async function start() {
  try {
    ...
    await karma.init(db);
    app.listen(PORT, () => {
      console.log(`MegaToDo server running on http://localhost:${PORT}`);
    });
  } catch (err) {
