...
export default function TaskDetailPanel({ task, onClose, onUpdated, onDeleted }) {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/labels`, { headers: getAuthHeaders() })
  ...
  const fetchSubtasks = useCallback(async (taskId) => {
    try {
      const res = await fetch(`${API_BASE}/api/tasks/${taskId}/subtasks`, { headers: getAuthHeaders() })
      if (res.ok) {
  ...
  const fetchComments = useCallback(async (taskId) => {
    try {
      const res = await fetch(`${API_BASE}/api/tasks/${taskId}/comments`, { headers: getAuthHeaders() })
      if (res.ok) {
        const data = await res.json()
