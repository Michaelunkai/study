...
do {
    ...
    $availableUpdates | ForEach-Object {
        $idx++
        $kbId = if ($_.KBArticleIDs) { $_.KBArticleIDs -join ',' } elseif ($_.KB) { $_.KB -replace '^KB', '' } else { 'N/A' }
    ...
    if ($failedKBs.Count -gt 0) {
        ...
        foreach ($failedKB in $failedKBs) {
            $kbId = if ($failedKB.KB) { $failedKB.KB -replace '^KB', '' } elseif ($failedKB.KBArticleIDs) { $failedKB.KBArticleIDs -join ',' } else { 'N/A' }
...
if ($allRebootKBs.Count -gt 0) {
    ...
    try {
        ...
        $allRebootKBs | ForEach-Object {
            $kbId = if ($_.KB) { $_.KB -replace '^KB', '' } elseif ($_.KBArticleIDs) { $_.KBArticleIDs -join ',' } else { 'N/A' }
...
if ($allFailedKBs.Count -gt 0) {
    ...
    $allFailedKBs | ForEach-Object {
        $kbId = if ($_.KBArticleIDs) { $_.KBArticleIDs[0] } elseif ($_.KB) { $_.KB } else { 'N/A' }
        $kbPrefix = if ($kbId -match '^KB') { '' } else { 'KB' }
        Write-Host "  [FAIL] ${kbPrefix}${kbId} - $($_.Title)" -ForegroundColor Red
    }
}
