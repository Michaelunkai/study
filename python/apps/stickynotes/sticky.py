...
class StickyNoteApp:
    def __init__(self, master):
        ...
        style = ttk.Style()
        style.configure("TEntry", padding=5, relief="flat", background='#FF0000', foreground='#000000')
        style.configure("TButton", padding=5, relief="flat", background='#FF0000', foreground='#000000')

        self.note_entry = ttk.Entry(master, width=30, style="TEntry")  # Themed Entry widget
        self.note_entry.pack(pady=10)
        self.note_entry.bind("<Return>", self.add_note_on_enter)

        self.add_button = ttk.Button(master, text="Add Sticky Note", command=self.add_note, style="TButton")  # Themed Button widget
        self.add_button.pack()

        self.notes_frame = tk.Frame(master, bg='#2E2E2E')  # Frame to contain notes and buttons
    ...
    def populate_notes_frame(self):
        ...
        for note in self.notes:
            ...
            note_frame.pack(pady=5, fill=tk.X)

            note_label = tk.Label(note_frame, text=note, bg='#2E2E2E', fg='#FF0000')  # Label for note
