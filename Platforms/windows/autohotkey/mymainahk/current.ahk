...
; Ctrl+` - Swap ALL windows between monitors (everything on main → 2nd and 2nd → main)
; Preserves maximized state and scales proportional positions for non-maximized windows
; SC029 = scan code for backtick/grave accent key — needed because ` is AHK v2 escape char
^SC029::
{
    ...
    ; Move all primary windows → secondary
    ...
    ; Move all secondary windows → primary
...
; yyyy - Split top 2 windows ON the current monitor (where mouse is) into left/right halves
:*:yyyy::
{
    ...
    ; Get windows that are actually ON this monitor
    windowsByMon := EnumerateWindowsByMonitor()
    monWindows := windowsByMon[targetMon]

    if (monWindows.Length < 2) {
        ToolTip("Need at least 2 windows on Monitor " targetMon " (found " monWindows.Length ")")
        SetTimer(() => ToolTip(), -2000)
        return
    }
    ...
    ; First window (topmost Z-order on this monitor) - left half
    WinRestore("ahk_id " monWindows[1])
    WinMove(mLeft, mTop, halfWidth, mHeight, "ahk_id " monWindows[1])

    ; Second window - right half
    WinRestore("ahk_id " monWindows[2])
    WinMove(mLeft + halfWidth, mTop, halfWidth, mHeight, "ahk_id " monWindows[2])

    ToolTip("Split 2 windows on Monitor " targetMon)
    SetTimer(() => ToolTip(), -1500)
}

; tttt - Split top 3 windows ON the current monitor (where mouse is) into equal thirds
:*:tttt::
{
    ; Detect which monitor the mouse is currently on
    MouseGetPos(&mouseX, &mouseY)
    ...
    if (monWindows.Length < 3) {
        ToolTip("Need at least 3 windows on Monitor " targetMon " (found " monWindows.Length ")")
        SetTimer(() => ToolTip(), -2000)
        return
    }
    ...
    WinRestore("ahk_id " monWindows[1])
    WinMove(mLeft, mTop, thirdWidth, mHeight, "ahk_id " monWindows[1])

    ; Second window - middle third
    WinRestore("ahk_id " monWindows[2])
    WinMove(mLeft + thirdWidth, mTop, thirdWidth, mHeight, "ahk_id " monWindows[2])

    ; Third window - right third
    WinRestore("ahk_id " monWindows[3])
    WinMove(mLeft + (thirdWidth * 2), mTop, thirdWidth, mHeight, "ahk_id " monWindows[3])

    ToolTip("Split 3 windows on Monitor " targetMon)
    SetTimer(() => ToolTip(), -1500)
}

; 666y - Move active window to secondary monitor RIGHT half, existing window there goes LEFT half
:*:666y::
{
    if (MonitorGetCount() < 2) {
        ToolTip("Only 1 monitor detected")
    ...
    ; Find secondary (non-primary) monitor - this is the user's "2nd monitor"
    primaryMon := MonitorGetPrimary()
    secondaryMon := 0
    Loop MonitorGetCount() {
        if (A_Index != primaryMon) {
    ...
    ; Get current active window (the one to move)
    try {
        activeHwnd := WinGetID("A")
    } catch {
        activeHwnd := 0
    ...
    ; Find topmost window already on the secondary monitor (excluding the active window)
    windowsByMon := EnumerateWindowsByMonitor()
    secWindows := windowsByMon[secondaryMon]
    partnerHwnd := 0
    for hw in secWindows {
        if (hw != activeHwnd) {
            partnerHwnd := hw
            break
        }
    ...
    if (!partnerHwnd) {
        ToolTip("No window found on secondary monitor to split with")
        SetTimer(() => ToolTip(), -1500)
        return
    }

    ; Get secondary monitor work area
    MonitorGetWorkArea(secondaryMon, &mLeft, &mTop, &mRight, &mBottom)
    mWidth := mRight - mLeft
    mHeight := mBottom - mTop
    halfWidth := mWidth // 2

    ; Existing secondary monitor window → LEFT half
    WinRestore("ahk_id " partnerHwnd)
    Sleep(30)
    WinMove(mLeft, mTop, halfWidth, mHeight, "ahk_id " partnerHwnd)

    ; Moved window (new arrival) → RIGHT half
    WinRestore("ahk_id " activeHwnd)
    Sleep(50)
    WinMove(mLeft + halfWidth, mTop, halfWidth, mHeight, "ahk_id " activeHwnd)

    ToolTip("Moved to secondary monitor - split left/right")
    SetTimer(() => ToolTip(), -1500)
}

; 555t - Move active window to secondary monitor RIGHT third, existing windows go LEFT+MIDDLE thirds
:*:555t::
{
    if (MonitorGetCount() < 2) {
        ToolTip("Only 1 monitor detected")
    ...
    ; Find secondary (non-primary) monitor
    primaryMon := MonitorGetPrimary()
    secondaryMon := 0
    Loop MonitorGetCount() {
        if (A_Index != primaryMon) {
    ...
    ; Find top 2 windows already on the secondary monitor (excluding active window)
    windowsByMon := EnumerateWindowsByMonitor()
    secWindows := windowsByMon[secondaryMon]
    partners := []
    for hw in secWindows {
    ...
    if (partners.Length < 2) {
        ToolTip("Need at least 2 windows on secondary monitor for 3-way split (found " partners.Length ")")
        SetTimer(() => ToolTip(), -2000)
        return
    }
    ...
    ; Existing partner 1 → LEFT third
    WinRestore("ahk_id " partners[1])
    Sleep(30)
    WinMove(mLeft, mTop, thirdWidth, mHeight, "ahk_id " partners[1])

    ; Existing partner 2 → MIDDLE third
    WinRestore("ahk_id " partners[2])
    Sleep(30)
    WinMove(mLeft + thirdWidth, mTop, thirdWidth, mHeight, "ahk_id " partners[2])

    ; Moved window (new arrival) → RIGHT third
    WinRestore("ahk_id " activeHwnd)
    Sleep(50)
    WinMove(mLeft + (thirdWidth * 2), mTop, thirdWidth, mHeight, "ahk_id " activeHwnd)

    ToolTip("Moved to secondary monitor - split 3-way")
    SetTimer(() => ToolTip(), -1500)
}
...
:*:ggggg::
{
    Run("https://game-library-manager-web.vercel.app/")
}
