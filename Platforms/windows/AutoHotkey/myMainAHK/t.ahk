...
ShowHelpShortcuts() {
    try {
        ...
        ; Create a large GUI window (3x bigger than normal)
        helpGui := Gui("+Resize +MaximizeBox", "Complete Desktop Manager - All Shortcuts")
        helpGui.MarginX := 30
        helpGui.MarginY := 20
        
        ; Set large font (2x bigger and bold)
        helpGui.SetFont("s16 Bold", "Consolas")
        
        ; Create the help text content
        ...
        helpText .= "ssss→ssss | pipit→pipit | refresh→refresh | logout→refresh2`n"
        helpText .= "sdesktop→sdesktop | gccleaner→gccleaner | gdb→gdbooster`n"
        helpText .= "goodgame→ws gg | uninstall→uninstall | ext→ext`n"
        helpText .= "amd→AMD Driver Setup`n`n"
        
        helpText .= "••• WEB SHORTCUTS •••`n"
        helpText .= "youtube→YouTube | claude→Claude.ai | chatgpt→ChatGPT`n"
        helpText .= "gmail→Gmail Account 0 | gmail2→Gmail Account 1`n`n"
        
        helpText .= "••• APPLICATION LAUNCHERS •••`n"
        helpText .= "fire→Firefox | chrome→Chrome | wall→WallPaper | kvrt→KVRT`n"
        helpText .= "sgame→Game Save Manager | todo→Todoist | ghelp→GHelper Tray`n"
        helpText .= "ubuntu→Ubuntu WSL2 | redb→RedButton | everything→Everything`n"
        helpText .= "ide→Cursor IDE | ide2→VSCode | installed→Apps Folder`n`n"
        
        helpText .= "••• SYSTEM ACTIONS •••`n"
        helpText .= "bin→Empty Recycle Bin | tttt→Split Top 2 Apps | helpme→This Help`n`n"
        ...
        ; Add scrollable text control (3x bigger size)
        textControl := helpGui.Add("Text", "x10 y10 w1200 h800 VScroll", helpText)
        
        ; Add close button
        closeBtn := helpGui.Add("Button", "x550 y820 w100 h40", "Close")
        closeBtn.OnEvent("Click", (*) => helpGui.Close())
        
        ; Show the GUI (3x bigger than normal)
        helpGui.Show("w1220 h880")
        
        LogMessage("HELP: Large shortcuts help GUI displayed successfully")
...
ClickGHelperTray() {
    try {
        LogMessage("GHELPER: Attempting to click GHelper tray icon")
        
        ; Method 1: Try to find and activate existing GHelper window
        try {
            if WinExist("ahk_exe GHelper.exe") {
                WinActivate("ahk_exe GHelper.exe")
                LogMessage("GHELPER: Activated existing GHelper window")
                return
            }
        } catch {
            ; Continue to tray clicking methods
        }
        
        ; Method 2: Try to click the system tray area where GHelper might be
        try {
            ; Get the taskbar
            trayHwnd := WinGetID("ahk_class Shell_TrayWnd")
            if (trayHwnd) {
                ; Click in the notification area (system tray) - try multiple locations
                try {
                    ; Click notification area
                    ControlClick("ToolbarWindow323", trayHwnd, , "Left", 1, "x-50 y10")
                    Sleep(500)
                } catch {
                    ; Alternative tray click location
                    try {
                        ControlClick("TrayNotifyWnd1", trayHwnd, , "Left", 1, "x20 y10")
                        Sleep(500)
                    } catch {
                        ; Direct coordinate click in tray area
                        Click(A_ScreenWidth - 100, A_ScreenHeight - 50)
                        Sleep(500)
                    }
                }
                
                LogMessage("GHELPER: Clicked system tray area for GHelper")
                
                ; Wait a moment and look for GHelper window
                if WinWait("ahk_exe GHelper.exe", , 3) {
                    WinActivate("ahk_exe GHelper.exe")
                    LogMessage("GHELPER: GHelper window appeared and activated")
                    return
                }
            }
        } catch {
            LogMessage("GHELPER: Tray click method failed")
        }
        
        ; Method 3: Fallback - Launch GHelper normally if tray methods fail
        try {
            if FileExist(ghelperPath) {
                Run('"' . ghelperPath . '"')
                LogMessage("GHELPER: Launched GHelper directly as fallback")
                
                ; Wait and activate
                if WinWait("ahk_exe GHelper.exe", , 5) {
                    WinActivate("ahk_exe GHelper.exe")
                    LogMessage("GHELPER: GHelper window activated")
                }
            } else {
                LogMessage("GHELPER: GHelper executable not found at: " . ghelperPath)
            }
        } catch {
            LogMessage("GHELPER: All GHelper methods failed")
        }
        
    } catch Error as e {
        LogMessage("ERROR in ClickGHelperTray: " . e.message)
        IncrementError()
    }
}
...
LaunchUbuntuWSL() {
    try {
        LogMessage("UBUNTU: Attempting to launch Ubuntu WSL2")
        
        ; Method 1: Try Windows Terminal with Ubuntu distribution specifically
        try {
            Run('wt.exe -p "Ubuntu"')
            LogMessage("UBUNTU: Ubuntu WSL2 launched in Windows Terminal")
            return
        } catch {
            ; Method 2: Try direct Ubuntu command
            try {
                Run("ubuntu.exe")
                LogMessage("UBUNTU: Ubuntu WSL2 launched directly")
                return
            } catch {
                ; Method 3: Try WSL with Ubuntu distribution
                try {
                    Run("wsl.exe -d Ubuntu")
                    LogMessage("UBUNTU: Ubuntu WSL2 launched via wsl command")
                    return
                } catch {
                    ; Method 4: Try launching any WSL distribution
                    try {
                        Run("wsl.exe")
                        LogMessage("UBUNTU: Default WSL launched")
                        return
                    } catch {
                        LogMessage("ERROR: All Ubuntu WSL launch methods failed - WSL may not be installed")
                    }
                }
            }
        }
...
UpdateAMDDrivers() {
    try {
        LogMessage("AMD: Starting comprehensive AMD CPU driver check and setup")
        
        ; Create comprehensive AMD driver update script
        amdScript := 'powershell.exe -Command "'
        amdScript .= 'Write-Host "═══ AMD CPU Driver Check & Setup ═══" -ForegroundColor Cyan;'
        ...
        amdScript .= 'if ($amdCPU) { Write-Host "Found AMD CPU: $($amdCPU.Name)" -ForegroundColor Green };'
        amdScript .= 'if ($amdGPU) { Write-Host "Found AMD GPU: $($amdGPU.Name)" -ForegroundColor Green };'
        amdScript .= 'Write-Host "Opening Device Manager for driver updates..." -ForegroundColor Cyan;'
        amdScript .= 'Start-Process devmgmt.msc;'
        amdScript .= 'Write-Host "Opening Windows Update for driver search..." -ForegroundColor Cyan;'
        amdScript .= 'Start-Process ms-settings:windowsupdate-action;'
        amdScript .= 'Write-Host "Checking for AMD software..." -ForegroundColor Yellow;'
        ...
        amdScript .= 'if ($amdSoftware) { $amdSoftware | ForEach-Object { Write-Host "Found: $($_.DisplayName)" -ForegroundColor Green } } else { Write-Host "No AMD software detected. Consider downloading AMD Ryzen Master or AMD Software" -ForegroundColor Red };'
        amdScript .= 'Write-Host "═══ AMD Setup Complete ═══" -ForegroundColor Cyan;'
        amdScript .= '} catch { Write-Host "Error during AMD detection: $($_.Exception.Message)" -ForegroundColor Red };'
        amdScript .= 'Write-Host "Press any key to close..." -ForegroundColor Green; Read-Host"'
        
        Run(amdScript)
        LogMessage("AMD: Comprehensive AMD driver check and setup initiated")
        
    } catch Error as e {
        LogMessage("ERROR in UpdateAMDDrivers: " . e.message)
        IncrementError()
    }
}
...
RunSpeedtest() {
    try {
        LogMessage("SPEEDTEST: Running network speed test")
        
        ; Create comprehensive speedtest command
        speedtestCmd := 'powershell.exe -Command "'
        speedtestCmd .= 'Write-Host "═══ NETWORK SPEED TEST ═══" -ForegroundColor Cyan;'
        speedtestCmd .= 'Write-Host "Testing network speed... Please wait..." -ForegroundColor Yellow;'
        speedtestCmd .= 'try {'
        speedtestCmd .= '$speedtest = Invoke-RestMethod -Uri "http://www.speedtest.net/api/js/servers?engine=js" -TimeoutSec 10;'
        speedtestCmd .= 'Write-Host "Connected to speed test servers..." -ForegroundColor Green;'
        speedtestCmd .= 'Write-Host "Running comprehensive speed test..." -ForegroundColor Yellow;'
        speedtestCmd .= '$downloadSpeed = (Measure-Command { Invoke-WebRequest -Uri "http://speedtest.ftp.otenet.gr/files/test100k.db" -OutFile "$env:TEMP\speedtest.tmp" }).TotalSeconds;'
        speedtestCmd .= 'if ($downloadSpeed -gt 0) { $speed = [math]::Round(100/($downloadSpeed*8), 2); Write-Host "Estimated Download Speed: $speed Mbps" -ForegroundColor Green } else { Write-Host "Speed test method 1 failed" -ForegroundColor Red };'
        speedtestCmd .= 'Write-Host "Running alternative speed test..." -ForegroundColor Yellow;'
        speedtestCmd .= '$ping = Test-Connection -ComputerName "8.8.8.8" -Count 4 | Measure-Object ResponseTime -Average;'
        speedtestCmd .= 'Write-Host "Average Ping to Google DNS: $([math]::Round($ping.Average, 2)) ms" -ForegroundColor Green;'
        speedtestCmd .= 'Write-Host "Testing DNS resolution..." -ForegroundColor Yellow;'
        speedtestCmd .= '$dnsTest = Measure-Command { Resolve-DnsName "google.com" };'
        speedtestCmd .= 'Write-Host "DNS Resolution Time: $([math]::Round($dnsTest.TotalMilliseconds, 2)) ms" -ForegroundColor Green;'
        speedtestCmd .= '} catch { Write-Host "Network test error: $($_.Exception.Message)" -ForegroundColor Red };'
        speedtestCmd .= 'Write-Host "═══ SPEED TEST COMPLETE ═══" -ForegroundColor Cyan;'
        speedtestCmd .= 'Write-Host "Press any key to close..." -ForegroundColor Green; Read-Host"'
        
        Run(speedtestCmd)
        LogMessage("SPEEDTEST: Network speed test initiated")
...
Open1337x() {
    try {
        LogMessage("WEB: Opening 1337x in Chrome")
        ...
        ; Open 1337x in Chrome new tab
        Run('"' . chromePath . '" "https://1337x.to/"')
        LogMessage("WEB: 1337x opened in Chrome")
...
OpenGmail() {
    try {
        ...
        Run('"' . chromePath . '" "https://mail.google.com/mail/u/0/"')
...
OpenGmail2() {
    try {
        ...
        Run('"' . chromePath . '" "https://mail.google.com/mail/u/1/#inbox"')
...
OpenYoutube() {
    try {
        ...
        Run('"' . chromePath . '" "https://youtube.com"')
...
OpenClaude() {
    try {
        ...
        Run('"' . chromePath . '" "https://claude.ai"')
...
OpenChatGPT() {
    try {
        ...
        Run('"' . chromePath . '" "https://chatgpt.com"')
...
TrayTip("Complete Manager Ready",
    ...
    "••• TEXT SHORTCUTS (Smart Terminal Close) •••`n" .
    "helpme→Large GUI Help | ubuntu→Ubuntu WSL2 | ext→ext | amd→AMD Setup`n" .
    "nvc→nss | biu→ws'backitup' | sleep→ss | cahk→closeahk`n" .
    "reboot→REBOOT | bios→bios | brc→brc | swemod→swemod`n" .
    "nnn→nnn | dsubs→dsubs | cleans→clean | ccbbr→ccbbr | ssss→ssss`n" .
    "pipit→pipit | refresh→refresh | logout→refresh2`n" .
    "sdesktop→sdesktop | gccleaner→gccleaner | gdb→gdbooster`n" .
    "goodgame→ws gg | uninstall→uninstall`n" .
    "••• WEB & APPS •••`n" .
    "youtube→YouTube | claude→Claude.ai | chatgpt→ChatGPT`n" .
    "gmail→Gmail 0 | gmail2→Gmail 1 | everything→Everything Search`n" .
    "fire→Firefox | chrome→Chrome | kvrt→KVRT | ghelp→GHelper Tray`n" .
    "ide→Cursor | ide2→VSCode | installed→Apps Folder`n" .
    "wall→WallPaper | tttt→Split Apps | bin→Empty Recycle Bin`n" .
    "sgame→Game Save Manager | todo→Todoist | redb→RedButton`n" .
    "••• APPLICATIONS •••`n" .
    "Alt+K: FORCE KILL App | Alt+A: WhatsApp`n" .
    "Alt+C: Cheat Engine | Alt+W: WeMod`n" .
    "••• OTHER •••`n" .
    "Win+G: Open Games Folder | Shift+S: Switch Desktops`n" .
    "Alt+T: Terminal | Alt+R: Close Terminal`n" .
    "Ctrl+Z: Suspend | Ctrl+Alt+R: Resume | Ctrl+D: Resume All", 20000)
