...
ExecuteTerminalCommand(command) {
    try {
        ...
        ; Execute command in terminal that auto-closes after completion
        terminalExecuted := false
        
        ; Method 1: Try Windows Terminal with auto-close
        try {
            ; Windows Terminal command that runs and exits
            cmdString := 'wt.exe --title "AutoHotkey Command" cmd /c "' . command . ' & pause & exit"'
            Run(cmdString)
            LogMessage("TERMINAL_CMD: Executed '" . command . "' in auto-closing Windows Terminal")
            terminalExecuted := true
        } catch {
            ; Method 2: Try PowerShell with auto-close (simplified)
            try {
                psCommand := 'powershell.exe -WindowStyle Normal -Command "& {' . command . '; Write-Host `"Press any key to close...`"; Read-Host; exit}"'
                Run(psCommand)
                LogMessage("TERMINAL_CMD: Executed '" . command . "' in auto-closing PowerShell")
                terminalExecuted := true
            } catch {
                ; Method 3: Try CMD with auto-close
                try {
                    cmdCommand := 'cmd.exe /c "' . command . ' & echo. & echo Command finished. Press any key to close... & pause > nul"'
                    Run(cmdCommand)
                    LogMessage("TERMINAL_CMD: Executed '" . command . "' in auto-closing CMD")
                    terminalExecuted := true
                } catch {
                    ; Method 4: Fallback to simple command execution (no terminal window)
                    try {
                        RunWait(command, "", "Hide")
                        LogMessage("TERMINAL_CMD: Executed '" . command . "' in background (no terminal)")
                        terminalExecuted := true
                    } catch {
                        LogMessage("TERMINAL_CMD: Failed to execute command: " . command)
                    }
                }
            }
        }
        ...
        if (!terminalExecuted) {
            LogMessage("ERROR: Could not execute terminal command: " . command)
        }
