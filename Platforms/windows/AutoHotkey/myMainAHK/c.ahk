...
; Double-tap variables for window management
lastWPress := 0
lastEPress := 0
...
ProcessTextBuffer(newChar) {
    global textBuffer, maxBufferLength, textShortcuts
    ...
    ; Check for special reboot command first
    if (InStr(textBuffer, "reboot") > 0) {
        textBuffer := ""
        FastReboot()
        LogMessage("TEXT_SHORTCUT: Triggered 'reboot' -> Fast Windows 11 reboot")
        return
    }
...
FastReboot() {
    try {
        LogMessage("REBOOT: Initiating fast Windows 11 reboot")
        
        ; Show warning notification
        TrayTip("Fast Reboot Initiated", 
            "Windows 11 will reboot in 3 seconds!`n" .
            "All unsaved work will be lost!", 3000)
        
        ; Wait 3 seconds to give user time to see notification
        Sleep(3000)
        
        ; Force immediate reboot with no delay
        ; /r = restart, /f = force close apps, /t 0 = no delay
        RunWait("shutdown.exe /r /f /t 0", "", "Hide")
        
        LogMessage("REBOOT: Fast reboot command executed")
        
    } catch Error as e {
        LogMessage("ERROR in FastReboot: " . e.message)
        IncrementError()
        TrayTip("Reboot Error", "Failed to initiate fast reboot", 2000)
    }
}
...
ExecuteTerminalCommand(command) {
    try {
        ...
        ; Try to find existing terminal window first
        terminalFound := false
        ...
        if WinExist("ahk_exe WindowsTerminal.exe") {
            ...
            Send(command . "{Enter}")
            terminalFound := true
            LogMessage("TERMINAL_CMD: Sent to Windows Terminal")
        }
        ; Check for PowerShell
            ...
            Send(command . "{Enter}")
            terminalFound := true
            LogMessage("TERMINAL_CMD: Sent to PowerShell")
        }
        ; Check for CMD
            ...
            Send(command . "{Enter}")
            terminalFound := true
            LogMessage("TERMINAL_CMD: Sent to CMD")
        }
        
        ; If no terminal found, open new one
        if (!terminalFound) {
            ...
            try {
                ...
                try {
                    ...
                    LogMessage("TERMINAL_CMD: Failed to open any terminal")
                    TrayTip("Terminal Error", "Could not execute: " . command, 2000)
                }
            }
        }
        
        TrayTip("Command Executed", command, 1500)
...
; ••• WINDOW MANAGEMENT •••
; Double-tap W to force windowed mode (small resolution for games)
~w:: {
...
; Double-tap E to force full-size windowed mode (max resolution)
~e:: {
    global lastEPress, doubleTapThreshold
    try {
        currentTime := A_TickCount
        
        ; Check if this is a double-tap
        if (currentTime - lastEPress <= doubleTapThreshold) {
            LogMessage("FULLSIZE: Double-tap E detected")
            ForceFullSizeMode()
            lastEPress := 0  ; Reset to prevent triple-tap issues
        } else {
            lastEPress := currentTime
        }
    } catch Error as e {
        LogMessage("ERROR in double-tap E handler: " . e.message)
        IncrementError()
    }
}
...
TrayTip("Complete Manager Ready",
    ...
    "••• HOTKEYS •••`n" .
    "Double-tap W: Force Windowed Mode (Small)`n" .
    "Double-tap E: Force Full Size Mode (Max)`n" .
    "••• TEXT SHORTCUTS •••`n" .
    "Type 'nvc': Run 'nss' | 'biu': Run 'ws backitup'`n" .
    "Type 'clean': Run 'clean' | 'sleep': Run 'ss'`n" .
    "Type 'reboot': Fast Windows 11 reboot`n" .
    "Type 'bios': Run 'bios' | 'brc': Run 'brc'`n" .
    "Plus: rws, rewsl, rrewsl, nnn, closeahk, sizes,`n" .
    "swemod, dsubs, ubu, ubu2, updates (as commands)`n" .
    "••• OTHER HOTKEYS •••`n" .
