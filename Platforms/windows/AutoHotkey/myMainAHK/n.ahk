...
ExecuteTerminalCommand(command) {
    try {
        ...
        ; Execute command in PowerShell (these commands are PowerShell-specific)
        terminalExecuted := false
        
        ; Method 1: Try Windows Terminal with PowerShell
        try {
            ; Use Windows Terminal with PowerShell (not CMD)
            cmdString := 'wt.exe powershell.exe -NoExit -Command "' . command . '; Write-Host `"Press any key to close...`"; Read-Host; exit"'
            Run(cmdString)
            LogMessage("TERMINAL_CMD: Executed '" . command . "' in Windows Terminal with PowerShell")
            terminalExecuted := true
        } catch {
            ; Method 2: Try PowerShell directly
            try {
                psCommand := 'powershell.exe -NoExit -Command "' . command . '; Write-Host `"Press any key to close...`"; Read-Host; exit"'
                Run(psCommand)
                LogMessage("TERMINAL_CMD: Executed '" . command . "' in PowerShell")
                terminalExecuted := true
            } catch {
                ; Method 3: Try PowerShell with script block (more reliable)
                try {
                    psScript := 'powershell.exe -NoExit -Command "& {' . command . '; Write-Host `"Press any key to close...`"; Read-Host; exit}"'
                    Run(psScript)
                    LogMessage("TERMINAL_CMD: Executed '" . command . "' in PowerShell script block")
                    terminalExecuted := true
                } catch {
                    ; Method 4: Try interactive PowerShell session
                    try {
                        interactivePS := 'powershell.exe -Command "' . command . '"'
                        Run(interactivePS)
                        LogMessage("TERMINAL_CMD: Executed '" . command . "' in interactive PowerShell")
                        terminalExecuted := true
                    } catch {
                        LogMessage("TERMINAL_CMD: Failed to execute PowerShell command: " . command)
                    }
                }
            }
        }
        ...
        if (!terminalExecuted) {
            LogMessage("ERROR: Could not execute PowerShell command: " . command)
        }
