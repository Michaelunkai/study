...
ProcessTextBuffer(newChar) {
    ...
    for shortcut, command in textShortcuts {
        if (InStr(StrLower(textBuffer), StrLower(shortcut)) > 0) {
            ...
            } else if (command = "COMBINE_TERMINALS") {
                CombineAllTerminals()
            } else {
...
CombineAllTerminals() {
    try {
        LogMessage("COMBINE_TERMINALS: Starting terminal combination process")
        
        ; Find all terminal windows
        terminalWindows := []
        terminalProcesses := ["WindowsTerminal.exe", "powershell.exe", "cmd.exe", "pwsh.exe"]
        
        for processName in terminalProcesses {
            try {
        ...
        if (terminalWindows.Length <= 1) {
            LogMessage("COMBINE_TERMINALS: Less than 2 terminals found, nothing to combine")
            return
        }
        
        LogMessage("COMBINE_TERMINALS: Found " . terminalWindows.Length . " terminal windows")
        
        ; Keep the first terminal as the main one
        mainTerminal := terminalWindows[1]
        
        ; Close all other terminals
        for i, terminal in terminalWindows {
            if (i = 1)  ; Skip the first one (main terminal)
                continue
                
            try {
                ; Try graceful close first
                WinClose(terminal.hwnd)
                LogMessage("COMBINE_TERMINALS: Closed " . terminal.process . " - " . terminal.title)
            } catch {
                ; Force close if needed
                try {
                    ProcessClose(terminal.process)
                } catch {
                    LogMessage("COMBINE_TERMINALS: Failed to close " . terminal.process)
                }
            }
        }
        
        ; Activate and focus the remaining terminal
        try {
            WinActivate(mainTerminal.hwnd)
            WinShow(mainTerminal.hwnd)
            
            ; Bring to center for better visibility
            MonitorGet(1, &Left, &Top, &Right, &Bottom)
            screenWidth := Right - Left
            screenHeight := Bottom - Top
            
            ; Size it nicely (80% of screen, centered)
            termWidth := screenWidth * 0.8
            termHeight := screenHeight * 0.8
            termX := (screenWidth - termWidth) / 2
            termY := (screenHeight - termHeight) / 2
            
            WinMove(termX, termY, termWidth, termHeight, mainTerminal.hwnd)
            
            LogMessage("COMBINE_TERMINALS: Kept " . mainTerminal.process . " as main terminal and positioned it centrally")
        } catch Error as e {
            LogMessage("COMBINE_TERMINALS: Failed to position main terminal - " . e.message)
        }
        
        LogMessage("COMBINE_TERMINALS: Successfully combined all terminals into one")
        
    } catch Error as e {
        LogMessage("ERROR in CombineAllTerminals: " . e.message)
        IncrementError()
    }
}
