...
ExecuteTerminalCommand(command) {
    try {
        ...
        ; Try to open new Windows Terminal first (most reliable)
        try {
            Run("wt.exe")
            Sleep(2000)  ; Increased wait time for terminal to fully load
            if WinWait("ahk_exe WindowsTerminal.exe", , 5) {
                WinActivate("ahk_exe WindowsTerminal.exe")
                Sleep(1500)  ; Wait for terminal to be fully ready
                
                ; Send the command more reliably
                Send("^a")  ; Select all existing text first
                Sleep(100)
                SendText(command)
                Sleep(300)
                Send("{Enter}")
                LogMessage("TERMINAL_CMD: Sent command '" . command . "' to Windows Terminal")
                terminalOpened := true
            }
        } catch {
            ; Fallback to new PowerShell if Windows Terminal fails
            try {
                Run("powershell.exe")
                Sleep(2000)
                if WinWait("ahk_exe powershell.exe", , 5) {
                    ...
                    ; Clear any existing text and send command
                    Send("^a")
                    Sleep(100)
                    SendText(command)
                    Sleep(300)
                    Send("{Enter}")
                    LogMessage("TERMINAL_CMD: Sent command '" . command . "' to PowerShell")
                    terminalOpened := true
                }
            } catch {
                ...
                try {
                    ...
                    if WinWait("ahk_exe cmd.exe", , 5) {
                        ...
                        Sleep(300)
                        Send("{Enter}")
                        LogMessage("TERMINAL_CMD: Sent command '" . command . "' to CMD")
                        terminalOpened := true
                    }
                } catch {
                    LogMessage("TERMINAL_CMD: Failed to open any terminal for command: " . command)
                }
            }
        }
