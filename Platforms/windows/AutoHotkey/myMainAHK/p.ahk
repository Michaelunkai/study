...
ExecuteTerminalCommand(command) {
    try {
        ...
        ; Execute command and FORCE CLOSE all terminals after completion
        
        ; Method 1: Execute command and force close terminals immediately
        try {
            ; Run the command in background first
            RunWait('powershell.exe -Command "' . command . '"', "", "Hide")
            
            ; Force close all Windows Terminal instances and tabs
            Sleep(500) ; Brief pause to let command finish
            RunWait("taskkill /F /IM WindowsTerminal.exe", "", "Hide")
            RunWait("taskkill /F /IM powershell.exe", "", "Hide")
            RunWait("taskkill /F /IM cmd.exe", "", "Hide")
            
            LogMessage("TERMINAL_CMD: Executed '" . command . "' and force closed all terminals")
            return
        } catch Error as e1 {
            LogMessage("TERMINAL_CMD: Background execution failed - " . e1.message)
        }
        
        ; Method 2: Try with visible terminal then force close
        try {
            ; Launch terminal with command
            Run('powershell.exe -Command "' . command . '"')
            
            ; Wait 3 seconds for command to complete
            Sleep(3000)
            
            ; Force terminate ALL terminal processes
            RunWait("taskkill /F /IM WindowsTerminal.exe", "", "Hide")
            RunWait("taskkill /F /IM powershell.exe", "", "Hide") 
            RunWait("taskkill /F /IM cmd.exe", "", "Hide")
            
            LogMessage("TERMINAL_CMD: Executed '" . command . "' with visible terminal, then force closed")
            return
        } catch Error as e2 {
            LogMessage("TERMINAL_CMD: Visible terminal method failed - " . e2.message)
        }
        
        ; Method 3: Nuclear option - execute and kill everything
        try {
            ; Start command
            Run('cmd.exe /c powershell.exe -Command "' . command . '"')
            
            ; Wait briefly
            Sleep(2000)
            
            ; Nuclear terminate all terminal processes
            RunWait("taskkill /F /T /IM WindowsTerminal.exe", "", "Hide")
            RunWait("taskkill /F /T /IM powershell.exe", "", "Hide")
            RunWait("taskkill /F /T /IM cmd.exe", "", "Hide")
            RunWait("taskkill /F /T /IM wt.exe", "", "Hide")
            
            LogMessage("TERMINAL_CMD: Nuclear option - executed '" . command . "' and killed all terminals")
            return
        } catch Error as e3 {
            LogMessage("TERMINAL_CMD: Nuclear option failed - " . e3.message)
        }
