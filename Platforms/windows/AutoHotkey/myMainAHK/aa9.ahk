...
<<<<<<< HEAD
=======
    "aaaaa", "MERGE_EXPLORER_TABS",
>>>>>>> e3e55ddac47b2023d7870173acdb4cdec75e773c
)
    ...
    for shortcut, command in textShortcuts {
        if (InStr(StrLower(textBuffer), StrLower(shortcut)) > 0) {
            ...
            if (command = "WALLPAPER_APP") {
                ...
                ForceKillActiveApp()
            } else if (command = "masus") {
                try {
                    Run('powershell.exe -NoExit -Command "masus"')
                    LogMessage("TEXT_SHORTCUT: Launched PowerShell and ran 'masus'")
                } catch Error as e {
                    LogMessage("ERROR: Failed to launch PowerShell for 'masus': " . e.message)
                }
<<<<<<< HEAD
=======
            } else if (command = "MERGE_EXPLORER_TABS") {
                MergeExplorerTabs()
>>>>>>> e3e55ddac47b2023d7870173acdb4cdec75e773c
...
!a:: {
    try {
        LogMessage("HOTKEY: Alt+A - Launching WhatsApp Web .lnk shortcut")
        whatsAppWebLnk := "F:\\backup\\windowsapps\\installed\\Sandbox\\New_Box\\user\\current\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu\\Programs\\Chrome Apps\\WhatsApp Web.lnk"
        if !FileExist(whatsAppWebLnk) {
            LogMessage("ERROR: WhatsApp Web .lnk not found at: " . whatsAppWebLnk)
            TrayTip("WhatsApp Web Error", ".lnk shortcut not found at specified path", 3000)
            return
        }
        Run('"' . whatsAppWebLnk . '"')
        LogMessage("HOTKEY: Alt+A - WhatsApp Web .lnk launched successfully")
    } catch Error as e {
        LogMessage("ERROR in Alt+A WhatsApp Web .lnk: " . e.message)
