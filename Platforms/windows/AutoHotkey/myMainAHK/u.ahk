...
ShowHelpShortcuts() {
    try {
        ...
        helpGui.SetFont("s18 Bold", "Segoe UI")
        ...
        helpText .= "amd→AMD Driver Setup | stopd→Stop Docker Desktop`n`n"
        ...
        helpText .= "1337x→1337x.to Torrent Site | speedtest→Network Speed Test`n`n"
        ...
        helpText .= "vscode→VSCode | installed→Apps Folder`n`n"
        ...
        helpText .= "• Everything Search: allit launches Everything for instant file search`n`n"
...
TrayTip("Complete Manager Ready - ENHANCED VERSION",
    "Script running with admin privileges - ALL FIXES APPLIED`n" .
    "••• MAJOR IMPROVEMENTS •••`n" .
    ...
    "✓ 1337x: Fixed - opens 1337x.to in Chrome new tab`n" .
    "✓ Everything: Fixed launcher (allit)`n" .
    "✓ Help GUI: Full text display, 4x larger window`n" .
    ...
    "••• TEXT SHORTCUTS (Enhanced) •••`n" .
    "helpme→Large Help GUI | ghelp→GHelper GUI | allit→Everything Search`n" .
    "ubuntu→WSL Ubuntu | ubu2→WSL Ubuntu2 | stopd→Stop Docker`n" .
    "speedtest→Real Network Test | 1337x→1337x.to Torrent Site`n" .
    "youtube→YouTube | claude→Claude.ai | chatgpt→ChatGPT`n" .
    "gmail→Gmail 0 | ggmail→Gmail 1 | fire→Firefox | chrome→Chrome`n" .
    "wall→WallPaper | tttt→Split Apps | bin→Empty Recycle Bin`n" .
    "sgame→Game Save Manager | todo→Todoist | kvrt→KVRT`n" .
    "ide→Cursor | vscode→VSCode | installed→Apps Folder`n" .
    "All terminal commands auto-close their windows!", 25000)
