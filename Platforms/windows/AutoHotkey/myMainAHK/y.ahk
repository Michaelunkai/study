...
qbittorrentPath := "F:\backup\windowsapps\installed\qBittorrent\qbittorrent.exe"
    ...
    for shortcut, command in textShortcuts {
        if (InStr(StrLower(textBuffer), StrLower(shortcut)) > 0) {
            ...
            if (command = "WALLPAPER_APP") {
                ...
                StopDockerDesktop()
            } else if (command = "QBITTORRENT_APP") {
                LaunchQBittorrent()
...
ShowHelpShortcuts() {
    try {
        ...
        leftColumnText .= "eee: Force current window to full size mode`n`n"
        
        leftColumnText .= "••• TERMINAL COMMANDS •••`n"
        leftColumnText .= "nvc→nss | biu→ws'backitup' | sleep→ss`n"
        leftColumnText .= "cahk→closeahk | reboot→REBOOT | bios→bios`n"
        leftColumnText .= "brc→brc | swemod→swemod | nnn→nnn`n"
        ...
        leftColumnText .= "goodgame→ws gg | uninstall→uninstall`n"
        leftColumnText .= "stopd→Stop Docker Desktop`n`n"
        
        leftColumnText .= "••• SYSTEM FUNCTIONS •••`n"
        leftColumnText .= "recovery→Create System Restore Point`n"
        leftColumnText .= "parsec→Launch Parsec Remote Desktop`n"
        leftColumnText .= "qbittorrent→Launch qBittorrent`n"
        ...
        leftColumnText .= "ubu2→Open Terminal + Run WSL Ubuntu2`n`n"
        ...
        rightColumnText .= "todo→Todoist | redb→RedButton`n"
        ...
        rightColumnText .= "Ctrl+K: Close Window (Keep App Running)`n"
        ...
        rightColumnText .= "• System Restore: Creates timestamped points`n"
        ...
        rightColumnText .= "• qBittorrent: Torrent client launcher`n"
        rightColumnText .= "• Software Updater: Update management`n"
        rightColumnText .= "• Downloads Folder: Quick file access`n"
        rightColumnText .= "• ASUS ROG: Direct support & downloads access`n`n"
...
LaunchQBittorrent() {
    try {
        LogMessage("LAUNCHER: Attempting to launch qBittorrent")
        
        ; Check if qBittorrent executable exists
        if !FileExist(qbittorrentPath) {
            LogMessage("ERROR: qBittorrent not found at: " . qbittorrentPath)
            return
        }
        ...
        if ProcessExist("qbittorrent.exe") {
            ...
            if WinExist("ahk_exe qbittorrent.exe") {
                ...
                LogMessage("LAUNCHER: qBittorrent already running, activated window")
            } else {
                LogMessage("LAUNCHER: qBittorrent process exists but no window found")
            }
            return
        }
        ...
        ; Launch qBittorrent
        Run('"' . qbittorrentPath . '"')
        LogMessage("LAUNCHER: qBittorrent launched successfully")
        
        ; Optional: Wait and activate window
        Sleep(2000)
        if WinWait("ahk_exe qbittorrent.exe", , 8) {
            WinActivate("ahk_exe qbittorrent.exe")
            LogMessage("LAUNCHER: qBittorrent window activated")
        }
        
    } catch Error as e {
        LogMessage("ERROR in LaunchQBittorrent: " . e.message)
        IncrementError()
    }
}
...
; ••• APPLICATION LAUNCHER •••
; Close window but keep app running: Ctrl + K
^k:: {
    try {
        CloseWindowKeepRunning()
    } catch {
...
TrayTip("Complete Manager Ready - UPDATED VERSION",
    "Script running with admin privileges - ALL FEATURES UPDATED!`n" .
    "••• NEW SHORTCUTS ADDED •••`n" .
    "✓ qbittorrent→Launch qBittorrent Torrent Client`n" .
    "✓ updater→Launch Software Updater`n" .
    "✓ downloads→Open Downloads Folder in Explorer`n" .
    "✓ myasus→Open ASUS ROG Website in Chrome`n" .
    "✓ parsec→Launch Parsec Remote Desktop (Fixed Path)`n`n" .
    "••• UPDATED FEATURES •••`n" .
    "✓ Help GUI: Now displays in 2 columns side by side`n" .
    "✓ Ctrl+K: Close window but keep app running in tray`n" .
    "✓ Speedtest: Removed as requested`n" .
    "✓ Parsec: Fixed to use correct executable path`n`n" .
    "••• EXISTING POWERFUL FEATURES •••`n" .
    "✓ System Restore: recovery creates timestamped restore points`n" .
    "✓ WSL: ubuntu→Terminal+WSL Ubuntu | ubu2→Terminal+WSL Ubuntu2`n" .
    "✓ Docker: Force stop with stopd command`n" .
    "✓ Everything Search: allit launches Everything for instant file search`n`n" .
    "••• HOTKEYS •••`n" .
    "Triple-tap W: Force Windowed Mode | Triple-tap E: Force Full Size`n" .
    "Ctrl+K: Close Window (Keep Running) | Alt+A: WhatsApp`n" .
    ...
    "qbittorrent→qBittorrent | updater→Software Updater`n" .
    "downloads→Downloads Folder | myasus→ASUS Support Page`n" .
    "parsec→Parsec App | recovery→System Restore Point`n" .
    "ubuntu→WSL Ubuntu | stopd→Stop Docker | helpme→2-Column Help`n" .
    "All shortcuts work perfectly with smart management!", 30000)
