...
LaunchWallpaperApp() {
    try {
        ...
        if !FileExist(wallPaperApp) {
            ...
            TrayTip("WallPaper Error", "WallPaper.lnk not found", 2000)
            return
        }
        ...
        TrayTip("WallPaper", "WallPaper app launched", 1000)
        
    } catch Error as e {
        LogMessage("ERROR in LaunchWallpaperApp: " . e.message)
        IncrementError()
        TrayTip("Error", "Failed to launch WallPaper app", 1000)
    }
}
...
ForceKillActiveApp() {
    try {
        ...
        if (killSuccess) {
            TrayTip("Force Kill SUCCESS", 
                "Terminated: " . activeProcess . "`n" .
                "PID: " . activePID . "`n" .
                "Title: " . (StrLen(activeTitle) > 30 ? SubStr(activeTitle, 1, 30) . "..." : activeTitle), 2000)
        } else {
            TrayTip("Force Kill FAILED", "Could not terminate: " . activeProcess, 2000)
        }
...
LaunchChrome() {
    try {
        ...
        if !FileExist(chromePath) {
            ...
            TrayTip("Chrome Error", "Chrome.exe not found", 2000)
            return
        }
        ...
        if ProcessExist("chrome.exe") {
            ...
            if WinExist("ahk_exe chrome.exe") {
                ...
                TrayTip("Chrome", "Chrome activated", 1000)
            } else {
                LogMessage("LAUNCHER: Chrome process exists but no window found")
                TrayTip("Chrome", "Chrome process already running", 1000)
            }
            return
        }
        ...
        TrayTip("Chrome", "Chrome launched", 1000)
        ...
        IncrementError()
        TrayTip("Error", "Failed to launch Chrome", 1000)
    }
}
...
LaunchFirefox() {
    try {
        ...
        if !FileExist(firefoxPath) {
            ...
            TrayTip("Firefox Error", "Firefox.exe not found", 2000)
            return
        }
        ...
        if ProcessExist("firefox.exe") {
            ...
            if WinExist("ahk_exe firefox.exe") {
                ...
                TrayTip("Firefox", "Firefox activated", 1000)
            } else {
                LogMessage("LAUNCHER: Firefox process exists but no window found")
                TrayTip("Firefox", "Firefox process already running", 1000)
            }
            return
        }
        ...
        TrayTip("Firefox", "Firefox launched", 1000)
        ...
        IncrementError()
        TrayTip("Error", "Failed to launch Firefox", 1000)
    }
}
...
LaunchWhatsApp() {
    try {
        ...
        try {
            ...
            Sleep(1000)
            ; Check if WhatsApp window appears
            ...
            ; Method 2: Try direct executable path (if installed traditionally)
            try {
                whatsappExe := A_ProgramFiles . "\WindowsApps\5319275A.WhatsAppDesktop_*\WhatsApp.exe"
                Run(whatsappExe)
                if WinWait("ahk_exe WhatsApp.exe", , 3) {
                    WinActivate("ahk_exe WhatsApp.exe")
                    whatsappLaunched := true
                    LogMessage("LAUNCHER: WhatsApp launched via direct exe")
                }
            } catch {
                ; Method 3: Try PowerShell command to launch UWP app
                try {
        ...
        ; Check if WhatsApp is already running and just activate it
        if (!whatsappLaunched && ProcessExist("WhatsApp.exe")) {
        ...
        if (whatsappLaunched) {
            TrayTip("WhatsApp", "WhatsApp launched/activated", 1000)
        } else {
            TrayTip("WhatsApp Error", "Could not launch WhatsApp. Please install from Microsoft Store.", 2000)
            LogMessage("ERROR: WhatsApp could not be launched - may not be installed")
        }

    } catch Error as e {
        LogMessage("ERROR in LaunchWhatsApp: " . e.message)
        IncrementError()
        TrayTip("Error", "Failed to launch WhatsApp", 1000)
    }
}
LaunchCheatEngine() {
    try {
