...
ShowHelpShortcuts() {
    try {
        LogMessage("HELP: Displaying shortcuts help")
        ...
        helpText .= "amd→AMD Driver Update`n`n"
        
        helpText .= "••• WEB SHORTCUTS •••`n"
        helpText .= "youtube→YouTube in Chrome | claude→Claude.ai in Chrome`n"
        helpText .= "chatgpt→ChatGPT in Chrome`n`n"
        
        helpText .= "••• APPLICATION LAUNCHERS •••`n"
        helpText .= "fire→Firefox | chrome→Chrome | wall→WallPaper | kvrt→KVRT`n"
        helpText .= "sgame→Game Save Manager | todo→Todoist | ghelp→GHelper`n"
        helpText .= "ubuntu→Ubuntu WSL Terminal | redb→RedButton App`n`n"
        
        helpText .= "••• SYSTEM ACTIONS •••`n"
        helpText .= "bin→Empty Recycle Bin | tttt→Split Top 2 Apps`n`n"
        ...
        MsgBox(helpText, "Complete Desktop Manager - All Shortcuts", "OK")
        LogMessage("HELP: Shortcuts help displayed successfully")
...
LaunchGHelper() {
    try {
        LogMessage("LAUNCHER: Attempting to launch GHelper")
        ...
        if ProcessExist("GHelper.exe") {
            ...
            if WinExist("ahk_exe GHelper.exe") {
                ...
                LogMessage("LAUNCHER: GHelper already running, activated window")
            } else {
                LogMessage("LAUNCHER: GHelper process exists but no window found")
            }
            return
        }
        ...
        ; Launch GHelper
        Run('"' . ghelperPath . '"')
        LogMessage("LAUNCHER: GHelper launched successfully")
        
        ; Optional: Wait and activate window
        Sleep(1500)
        if WinWait("ahk_exe GHelper.exe", , 5) {
            WinActivate("ahk_exe GHelper.exe")
            LogMessage("LAUNCHER: GHelper window activated")
        }
...
LaunchUbuntuWSL() {
    try {
        LogMessage("LAUNCHER: Attempting to launch Ubuntu WSL")
        
        ; Try Windows Terminal with Ubuntu first
        try {
            Run("wt.exe -p Ubuntu")
            LogMessage("LAUNCHER: Ubuntu WSL launched in Windows Terminal")
            return
        } catch {
            ; Fallback to direct Ubuntu command
            try {
                Run("ubuntu.exe")
                LogMessage("LAUNCHER: Ubuntu WSL launched directly")
                return
            } catch {
                ; Final fallback to WSL command
                try {
                    Run("wsl.exe -d Ubuntu")
                    LogMessage("LAUNCHER: Ubuntu WSL launched via wsl command")
                } catch {
                    LogMessage("ERROR: All Ubuntu WSL launch methods failed")
                }
            }
        }
...
UpdateAMDDrivers() {
    try {
        LogMessage("AMD: Starting AMD driver check and update")
        
        ; Run AMD driver update command in PowerShell
        amdCommand := 'powershell.exe -Command "Write-Host `"Checking for AMD driver updates...`" -ForegroundColor Green; try { Get-WmiObject Win32_VideoController | Where-Object {$_.Name -like `"*AMD*`" -or $_.Name -like `"*Radeon*`"} | ForEach-Object { Write-Host `"Found AMD GPU: $($_.Name)`" -ForegroundColor Yellow }; Write-Host `"Opening Device Manager for manual driver update...`" -ForegroundColor Cyan; Start-Process devmgmt.msc } catch { Write-Host `"Error checking AMD drivers`" -ForegroundColor Red }; Write-Host `"Press any key to close...`" -ForegroundColor Green; Read-Host"'
        
        Run(amdCommand)
        LogMessage("AMD: AMD driver check initiated")
...
TrayTip("Complete Manager Ready",
    ...
    "helpme→Full Shortcuts List | ubuntu→Ubuntu WSL | ext→ext`n" .
    ...
    "pipit→pipit | refresh→refresh | logout→refresh2 | amd→AMD Drivers`n" .
    ...
    "fire→Firefox | chrome→Chrome | kvrt→KVRT | ghelp→GHelper`n" .
    ...
    "Ctrl+Z: Suspend | Ctrl+Alt+R: Resume | Ctrl+D: Resume All", 18000)
