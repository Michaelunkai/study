...
textShortcuts := Map(
    ...
    "ggggg", "FIT_LAUNCHER_APP" ; <-- NEW: Fit Launcher shortcut
)
    ...
    for shortcut, command in textShortcuts {
        if (InStr(StrLower(textBuffer), StrLower(shortcut)) > 0) {
            ...
            if (command = "WALLPAPER_APP") {
                ...
                ForceKillActiveApp()
            } else if (command = "FIT_LAUNCHER_APP") {
                LaunchFitLauncher()
            } else {
...
LaunchSavegTerminal() {
    try {
        LogMessage("SAVEG: Opening PowerShell and running 'ssave' command")
        
        ; Method 1: Try Windows Terminal with PowerShell and ssave command
        try {
            Run('wt.exe powershell -NoExit -Command "ssave"')
            LogMessage("SAVEG: ssave command launched in Windows Terminal (PowerShell)")
            return
        } catch {
            ; Method 2: Try standalone PowerShell with ssave command
            try {
                Run('powershell.exe -NoExit -Command "ssave"')
                LogMessage("SAVEG: ssave command launched in PowerShell")
                return
            } catch {
                ; Method 3: Try PowerShell ISE as fallback
                try {
                    Run('powershell_ise.exe -Command "ssave"')
                    LogMessage("SAVEG: ssave command launched in PowerShell ISE")
                    return
                } catch {
                    LogMessage("ERROR: All PowerShell ssave command launch methods failed - PowerShell or ssave command may not be available")
                }
            }
        }
...
LaunchFitLauncher() {
    try {
        LogMessage("LAUNCHER: Attempting to launch Fit Launcher")
        fitLauncherPath := "F:\\backup\\windowsapps\\installed\\Fit Launcher\\Fit Launcher.exe"
        if !FileExist(fitLauncherPath) {
            LogMessage("ERROR: Fit Launcher not found at: " . fitLauncherPath)
            TrayTip("Fit Launcher Error", "Fit Launcher.exe not found at specified path", 3000)
            return
        }
        if ProcessExist("Fit Launcher.exe") {
            if WinExist("ahk_exe Fit Launcher.exe") {
                WinActivate("ahk_exe Fit Launcher.exe")
                LogMessage("LAUNCHER: Fit Launcher already running, activated window")
                TrayTip("Fit Launcher", "Already running, window activated", 2000)
            } else {
                LogMessage("LAUNCHER: Fit Launcher process exists but no window found")
            }
            return
        }
        Run('"' . fitLauncherPath . '"')
        LogMessage("LAUNCHER: Fit Launcher launched successfully from: " . fitLauncherPath)
        TrayTip("Fit Launcher", "Launched successfully", 2000)
        Sleep(2000)
        if WinWait("ahk_exe Fit Launcher.exe", , 10) {
            WinActivate("ahk_exe Fit Launcher.exe")
            LogMessage("LAUNCHER: Fit Launcher window activated")
        } else {
            LogMessage("WARNING: Fit Launcher launched but window not detected within 10 seconds")
        }
    } catch Error as e {
        LogMessage("ERROR in LaunchFitLauncher: " . e.message)
        TrayTip("Fit Launcher Critical Error", "Critical error: " . e.message, 3000)
        IncrementError()
    }
}
