...
ShowHelpShortcuts() {
    try {
        ...
        leftColumnText .= "unzipit→PowerShell with 'ext' command`n`n"
...
TrayTip("Complete Manager Ready - LATEST VERSION",
    ...
    "✓ unzipit→PowerShell with 'ext' command`n`n" .
    "••• UPDATED FEATURES •••`n" .
    ...
    "updater→Software Updater | downloads→Downloads Folder`n" .
    "mygames→Terminal+myg | mytodo→Todoist | rebootit→System Reboot`n" .
