...
ExecuteTerminalCommand(command) {
    try {
        ...
        ; Execute command in PowerShell with GUARANTEED auto-close
        
        ; Try Windows Terminal with PowerShell first (preferred) - with enhanced auto-close
        try {
            ; Enhanced auto-close: Run command, show output, wait for keypress, then exit
            cmdString := 'wt.exe powershell.exe -Command "' . command . '; Write-Host `"`nCommand completed. Press any key to close this window...`" -ForegroundColor Green; $null = $Host.UI.RawUI.ReadKey(`"NoEcho,IncludeKeyDown`"); exit"'
            Run(cmdString)
            LogMessage("TERMINAL_CMD: Executed '" . command . "' in auto-closing Windows Terminal")
            ; Success - exit immediately to prevent other methods from running
            return
        } catch Error as e1 {
            LogMessage("TERMINAL_CMD: Windows Terminal failed - " . e1.message)
        }
        
        ; If Windows Terminal failed, try direct PowerShell with auto-close
        try {
            ; Direct PowerShell with guaranteed exit
            psCommand := 'powershell.exe -Command "' . command . '; Write-Host `"`nCommand completed. Press any key to close...`" -ForegroundColor Green; $null = $Host.UI.RawUI.ReadKey(`"NoEcho,IncludeKeyDown`"); exit"'
            Run(psCommand)
            LogMessage("TERMINAL_CMD: Executed '" . command . "' in auto-closing PowerShell")
            ; Success - exit immediately
            return
        } catch Error as e2 {
            LogMessage("TERMINAL_CMD: Direct PowerShell failed - " . e2.message)
        }
        
        ; If that failed, try PowerShell with /c flag (auto-close)
        try {
            ; PowerShell with cmd-style auto-close
            fallbackPS := 'powershell.exe -Command "& {' . command . '; Write-Host `"Press any key to exit...`"; Read-Host}"'
            Run(fallbackPS)
            LogMessage("TERMINAL_CMD: Executed '" . command . "' in fallback auto-closing PowerShell")
            ; Success - exit immediately
            return
        } catch Error as e3 {
            LogMessage("TERMINAL_CMD: Fallback PowerShell failed - " . e3.message)
        }
        
        ; If all methods failed, try simple background execution
        try {
            RunWait(command, "", "Hide")
            LogMessage("TERMINAL_CMD: Executed '" . command . "' in background (no window)")
            return
        } catch Error as e4 {
            LogMessage("TERMINAL_CMD: Background execution failed - " . e4.message)
        }
        
        ; If all PowerShell methods failed, log error
...
TrayTip("Complete Manager Ready",
    ...
    "••• TEXT SHORTCUTS (Auto-Close Terminals) •••`n" .
    ...
    "fire→Firefox | chrome→Chrome | kvrt→KVRT App`n" .
    "wall→WallPaper | tttt→Split Apps | bin→Empty Recycle Bin`n" .
    "sgame→Game Save Manager | todo→Todoist`n" .
    "••• APPLICATIONS •••`n" .
    ...
    "Ctrl+Z: Suspend | Ctrl+Alt+R: Resume | Ctrl+D: Resume All", 15000)
