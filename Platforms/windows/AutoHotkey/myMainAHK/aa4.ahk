...
textShortcuts := Map(
    ...
    "ubu2", "UBUNTU2_WSL",
    "ranch", "RANCH_WSL",
    "nnnn", "C:\\Program Files\\WindowsApps\\SAMSUNGELECTRONICSCoLtd.SamsungNotes_4.3.418.0_x64__wyx1vj98g3asy\\SamsungNotes.exe",
    "phonel", "PHONE_LINK",
    ...
    "ffff", "FIREFOX_APP",
    "saveg", "SAVEG_AFTER_MANAGER",  ; <-- Now triggers new function
    "macro", "MACROCREATOR_RUN_CTRL_R",  ; <-- NEW: MacroCreator shortcut
    "kkkkkkkk", "FORCE_KILL_APP",  ; <-- NEW: Force Kill App shortcut
    "ggggg", "F:\\backup\\windowsapps\\installed\\Fit Launcher\\Fit Launcher.exe"  ; <-- NEW: Fit Launcher shortcut
    , "paird", "PAIRDROP_WEB"  ; <-- NEW: PairDrop shortcut
)
...
!a:: {
    try {
        LogMessage("HOTKEY: Alt+A - Opening WhatsApp Web in Firefox")
        if !FileExist(firefoxPath) {
            LogMessage("ERROR: Firefox not found at: " . firefoxPath)
            TrayTip("WhatsApp Web Error", "Firefox not found at specified path", 3000)
            return
        }
        Run('"' . firefoxPath . '" --new-tab "https://web.whatsapp.com/"')
        LogMessage("HOTKEY: Alt+michaelovsky5@gmail.comA - WhatsApp Web opened in Firefox")
    } catch Error as e {
        LogMessage("ERROR in Alt+A WhatsApp Web: " . e.message)
        TrayTip("WhatsApp Web Error", e.message, 3000)
    }
}
