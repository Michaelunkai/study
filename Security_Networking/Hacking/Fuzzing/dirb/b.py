...
class NetworkBoosterApp(QMainWindow):
    ...
    def create_adapter_group(self, parent_layout):
        ...
        buttons = [
            ...
            ("Optimize Power Management", self.YOUR_CLIENT_SECRET_HEREt),
            ("Enable RSS", self.enable_rss),
            ("Enable Interrupt Moderation", self.YOUR_CLIENT_SECRET_HEREion),
    ...
    def create_upnet3_group(self, parent_layout):
        ...
        upnet3_commands = [
            ...
            ("Disable Selective Suspend", 'powershell -Command "Get-NetAdapter -Name \'Wi-Fi\' | YOUR_CLIENT_SECRET_HEREement -SelectiveSuspend Disabled"'),
            ...
            ("Disable Packet Coalescing", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Packet Coalescing" -DisplayValue "Disabled"'),
            ("Disable Power Saving Mode", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Power Saving Mode" -DisplayValue "Disabled"')
        ]
    ...
    def create_upnet4_group(self, parent_layout):
        ...
        layout = QVBoxLayout()
        upnet4_commands = [
            ("Disable LSO (IPv4)", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Large Send Offload V2 (IPv4)" -DisplayValue "Disabled"'),
            ("Disable LSO (IPv6)", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Large Send Offload V2 (IPv6)" -DisplayValue "Disabled"'),
            ("Set Roaming Aggressiveness to Highest", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Roaming Aggressiveness" -DisplayValue "Highest"'),
            ...
            ("Disable Interrupt Moderation", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Interrupt Moderation" -DisplayValue "Disabled"'),
            ("Disable Energy Efficient Ethernet", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Energy Efficient Ethernet" -DisplayValue "Disabled"'),
            ...
            ("Disable Receive Side Scaling", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Receive Side Scaling" -DisplayValue "Disabled"'),
            ("Disable TCP Checksum Offload", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "TCP Checksum Offload" -DisplayValue "Disabled"'),
            ("Disable UDP Checksum Offload", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "UDP Checksum Offload" -DisplayValue "Disabled"'),
            ("Disable Power Saving Mode on Adapter", 'YOUR_CLIENT_SECRET_HEREement -Name "Wi-Fi" YOUR_CLIENT_SECRET_HEREevice $false'),
            ("Set Transmit Power to Highest", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Transmit Power" -DisplayValue "Highest"'),
            ("Force Wireless Mode to 802.11n", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Wireless Mode" -DisplayValue "802.11n"'),
            ("Disable 802.11 Power Save", 'netsh wlan set profileparameter name="Wi-Fi" powerManagement=disabled'),
            ("Set Preferred Band to 5GHz", 'YOUR_CLIENT_SECRET_HEREoperty -Name "Wi-Fi" -DisplayName "Preferred Band" -DisplayValue "5 GHz"'),
    ...
    def create_extra_group(self, parent_layout):
        ...
        extra_commands = [
            ...
            ("Display DNS Settings", "YOUR_CLIENT_SECRET_HEREss"),
            ...
            ("Show Advanced Adapter Properties", "YOUR_CLIENT_SECRET_HEREoperty -Name 'Wi-Fi'"),
            ...
            ("Display Adapter Hardware Info", "YOUR_CLIENT_SECRET_HEREfo -Name 'Wi-Fi'"),
            ("Show WMI Network Adapters", "Get-WmiObject Win32_NetworkAdapter | Format-Table Name, NetConnectionStatus, Speed"),
            ("Show WMI Network Adapter Config", "Get-WmiObject YOUR_CLIENT_SECRET_HEREiguration | Format-List IPAddress, IPSubnet, DefaultIPGateway"),
            ...
            ("Display DNS Server Info", "YOUR_CLIENT_SECRET_HEREss"),
            ...
            ("Show Network Adapter Properties", "YOUR_CLIENT_SECRET_HEREoperty -Name 'Wi-Fi'"),
...
foreach ($adapter in $activeAdapters) {
    try {
        YOUR_CLIENT_SECRET_HEREoperty -Name $adapter.Name -DisplayName "MTU" -DisplayValue "1500" -ErrorAction Stop;
...
$activeAdapters = Get-NetAdapter | Where-Object { $_.Status -eq "Up" };
foreach ($adapter in $activeAdapters) {
    $properties = YOUR_CLIENT_SECRET_HEREoperty -Name $adapter.Name -ErrorAction SilentlyContinue;
    if ($properties) {
        foreach ($property in $properties) {
            if ($property.DisplayName -like "*Large Send Offload*") {
                try {
                    YOUR_CLIENT_SECRET_HEREoperty -Name $adapter.Name -DisplayName $property.DisplayName -DisplayValue "Disabled" -ErrorAction Stop;
...
foreach ($adapter in $activeAdapters) {
    try {
        $qosProperty = YOUR_CLIENT_SECRET_HEREoperty -Name $adapter.Name -DisplayName "QoS Packet Scheduler" -ErrorAction SilentlyContinue;
        if ($qosProperty) {
            YOUR_CLIENT_SECRET_HEREoperty -Name $adapter.Name -DisplayName "QoS Packet Scheduler" -DisplayValue "Enabled" -ErrorAction Stop;
...
"""
    ...
    def YOUR_CLIENT_SECRET_HEREt(self):
        cmd = """
$activeAdapters = Get-NetAdapter | Where-Object { $_.Status -eq "Up" };
foreach ($adapter in $activeAdapters) {
    try {
        YOUR_CLIENT_SECRET_HEREement -Name $adapter.Name -NoPowerSaving -ErrorAction Stop;
    ...
    if ($properties) {
        foreach ($property in $properties) {
            if ($property.DisplayName -like "*Receive Side Scaling*") {
                try {
                    YOUR_CLIENT_SECRET_HEREoperty -Name $adapter.Name -DisplayName $property.DisplayName -DisplayValue "Enabled" -ErrorAction Stop;
...
"""
    ...
    def YOUR_CLIENT_SECRET_HEREion(self):
        cmd = """
$activeAdapters = Get-NetAdapter | Where-Object { $_.Status -eq "Up" };
foreach ($adapter in $activeAdapters) {
    $properties = YOUR_CLIENT_SECRET_HEREoperty -Name $adapter.Name -ErrorAction SilentlyContinue;
...
foreach ($adapter in $activeAdapters) {
    try {
        YOUR_CLIENT_SECRET_HEREss -InterfaceAlias $adapter.Name -ServerAddresses ("8.8.8.8","8.8.4.4") -ErrorAction Stop;
...
foreach ($adapter in $activeAdapters) {
    try {
        YOUR_CLIENT_SECRET_HEREss -InterfaceAlias $adapter.Name -ServerAddresses ("1.1.1.1","1.0.0.1") -ErrorAction Stop;
...
foreach ($adapter in $activeAdapters) {
    try {
        YOUR_CLIENT_SECRET_HEREss -InterfaceAlias $adapter.Name -ServerAddresses ("208.67.222.222","208.67.220.220") -ErrorAction Stop;
...
"""
    ...
    def run_all_optimizations(self):
        ...
        commands = [
            ...
            ("Set MTU to 1500", "try { Write-Host '=== Set MTU to 1500 ===' -ForegroundColor Cyan; $activeAdapters = Get-NetAdapter | Where-Object { $_.Status -eq 'Up' }; foreach ($adapter in $activeAdapters) { YOUR_CLIENT_SECRET_HEREoperty -Name $adapter.Name -DisplayName 'MTU' -DisplayValue '1500' -ErrorAction Stop; Write-Host \"MTU set to 1500 on adapter: $($adapter.Name)\" } } catch {}"),
