...
echo not_active
iptables -L -n 2>/dev/null | grep -c '^Chain'; true
iptables -L INPUT -n 2>/dev/null | grep -c ACCEPT; true
iptables -L INPUT -n 2>/dev/null | grep -c DROP; true
iptables -L INPUT -n 2>/dev/null | grep -c REJECT; true
echo none
ss -tunap 2>/dev/null | wc -l
ss -tunap 2>/dev/null | grep ESTABLISHED | wc -l
ss -tunap 2>/dev/null | grep -c SYN-SENT; true
ss -tunap 2>/dev/null | grep -c TIME-WAIT; true
{ grep '^Port ' /etc/ssh/sshd_config 2>/dev/null | head -1 | cut -d' ' -f2 | grep .; } || echo 22
{ grep '^PermitRootLogin ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo unknown
{ grep '^PasswordAuthentication ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo unknown
{ grep '^PubkeyAuthentication ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo unknown
{ grep '^PermitEmptyPasswords ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo unknown
{ grep '^MaxAuthTries ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo 6
{ grep '^MaxSessions ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo 10
{ grep '^ClientAliveInterval ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo 0
{ grep '^ClientAliveCountMax ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo 3
{ grep '^X11Forwarding ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo unknown
{ grep '^AllowUsers ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo none
{ grep '^AllowGroups ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo none
{ grep '^DenyUsers ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo none
{ grep '^Protocol ' /etc/ssh/sshd_config 2>/dev/null | tail -1 | cut -d' ' -f2 | grep .; } || echo 2
systemctl is-active fail2ban 2>/dev/null || echo not_installed
{ fail2ban-client status 2>/dev/null | grep 'Number of jail' | rev | cut -d' ' -f1 | rev | grep .; } || echo 0
{ fail2ban-client status sshd 2>/dev/null | grep 'Currently banned' | rev | cut -d' ' -f1 | rev | grep .; } || echo 0
{ fail2ban-client status sshd 2>/dev/null | grep 'Total banned' | rev | cut -d' ' -f1 | rev | grep .; } || echo 0
...
{ grep PASS_MAX_DAYS /etc/login.defs 2>/dev/null | grep -v '^#' | head -1 | tr -s ' \t' ' ' | cut -d' ' -f2 | grep .; } || echo 99999
{ grep PASS_MIN_DAYS /etc/login.defs 2>/dev/null | grep -v '^#' | head -1 | tr -s ' \t' ' ' | cut -d' ' -f2 | grep .; } || echo 0
{ grep PASS_MIN_LEN /etc/login.defs 2>/dev/null | grep -v '^#' | head -1 | tr -s ' \t' ' ' | cut -d' ' -f2 | grep .; } || echo 5
{ grep PASS_WARN_AGE /etc/login.defs 2>/dev/null | grep -v '^#' | head -1 | tr -s ' \t' ' ' | cut -d' ' -f2 | grep .; } || echo 7
{ grep ':0:' /etc/passwd | grep -v ':/0:' | cut -d: -f1 | head -1 | grep .; } || echo root
grep -E '^[^:]+::' /etc/shadow 2>/dev/null | wc -l
cat /etc/passwd | wc -l
cat /etc/group | wc -l
...
openssl version 2>/dev/null | cut -d' ' -f2 || echo unknown
...
dpkg --get-selections | grep -c hold; true
...
docker ps --format '{{.Names}}: {{.Status}}' 2>/dev/null | grep -c '(healthy)'; true
docker ps --format '{{.Names}}: {{.Status}}' 2>/dev/null | grep -c '(unhealthy)'; true
echo none
systemctl is-active docker
docker --version 2>/dev/null | grep -oP '\d+\.\d+\.\d+' | head -1 || echo unknown
test -f /etc/docker/daemon.json && echo yes || echo no
cat /etc/docker/daemon.json 2>/dev/null | grep -c 'live-restore'; true
...
ls -la /var/run/docker.sock 2>/dev/null | cut -d' ' -f1
stat -c '%a' /var/run/docker.sock 2>/dev/null || echo 0
apparmor_status 2>/dev/null | grep -c 'docker'; true
aa-status 2>/dev/null | grep -c 'enforce'; true
