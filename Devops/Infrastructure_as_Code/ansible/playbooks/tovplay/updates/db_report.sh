...
SELECT '"'"'TABLE_COUNT:'"'"' || COUNT(*)::text FROM information_schema.tables WHERE table_schema = '"'"'public'"'"';
SELECT '"'"'USER_COUNT:'"'"' || COUNT(*)::text FROM "User";
SELECT '"'"'ACTIVE_CONN:'"'"' || count(*)::text FROM pg_stat_activity WHERE state = '"'"'active'"'"';
SELECT '"'"'TOTAL_CONN:'"'"' || count(*)::text FROM pg_stat_activity;
SELECT '"'"'MAX_CONN:'"'"' || setting FROM pg_settings WHERE name = '"'"'max_connections'"'"';
SELECT '"'"'UPTIME:'"'"' || (EXTRACT(EPOCH FROM (now() - pg_postmaster_start_time()))/86400)::numeric(10,1)::text;
SELECT '"'"'CACHE_HIT:'"'"' || ROUND(100 * sum(blks_hit) / NULLIF(sum(blks_hit + blks_read), 0), 2)::text FROM pg_stat_database WHERE datname = '"'"'TovPlay'"'"';
SELECT '"'"'DEAD_TUPLES:'"'"' || COALESCE(SUM(n_dead_tup), 0)::text FROM pg_stat_user_tables;
SELECT '"'"'LONG_QUERIES:'"'"' || COUNT(*)::text FROM pg_stat_activity WHERE state = '"'"'active'"'"' AND now() - query_start > interval '"'"'5 minutes'"'"';
SELECT '"'"'LOCKS:'"'"' || COUNT(*)::text FROM pg_locks WHERE NOT granted;
EOF
' 20 | tr -d '\r')

    DB_SIZE=$(echo "$BATCH_BASIC" | grep "DB_SIZE:" | cut -d: -f2- | xargs)
    TABLE_COUNT=$(echo "$BATCH_BASIC" | grep "TABLE_COUNT:" | cut -d: -f2 | xargs)
    USER_COUNT=$(echo "$BATCH_BASIC" | grep "USER_COUNT:" | cut -d: -f2 | xargs)
    ACTIVE_CONN=$(echo "$BATCH_BASIC" | grep "ACTIVE_CONN:" | cut -d: -f2 | xargs)
    TOTAL_CONN=$(echo "$BATCH_BASIC" | grep "TOTAL_CONN:" | cut -d: -f2 | xargs)
    MAX_CONN=$(echo "$BATCH_BASIC" | grep "MAX_CONN:" | cut -d: -f2 | xargs)
    UPTIME=$(echo "$BATCH_BASIC" | grep "UPTIME:" | cut -d: -f2 | xargs)
    CACHE_HIT=$(echo "$BATCH_BASIC" | grep "CACHE_HIT:" | cut -d: -f2 | xargs)
    DEAD_TUPLES=$(echo "$BATCH_BASIC" | grep "DEAD_TUPLES:" | cut -d: -f2 | xargs)
    LONG_QUERIES=$(echo "$BATCH_BASIC" | grep "LONG_QUERIES:" | cut -d: -f2 | xargs)
    LOCKS=$(echo "$BATCH_BASIC" | grep "LOCKS:" | cut -d: -f2 | xargs)
    ...
    echo -e "\n${CYAN}Performance:${NC}"
    check_info "Uptime: ${UPTIME} days"
    CACHE_HIT_INT=$(echo "${CACHE_HIT:-0}" | cut -d. -f1)
    [ "${CACHE_HIT_INT:-0}" -ge 95 ] 2>/dev/null && check_pass "Cache hit ratio: ${CACHE_HIT}%" || { check_warn "Cache hit ratio: ${CACHE_HIT}%"; add_low "Low cache hit"; }
...
SELECT schemaname || '"'"'.'"'"' || relname as table_name,
       ...
       ROUND(100 * n_dead_tup::numeric / NULLIF(n_live_tup, 0), 1) as bloat_pct,
...
FROM pg_stat_user_tables
WHERE seq_scan > 100 AND (idx_scan = 0 OR idx_scan < seq_scan / 10) AND n_live_tup > 1000
...
if [ "$DB_CONN" = true ]; then
    ...
    # WAL status
    WAL_POS=$(ssh_prod 'export PGPASSWORD="CaptainForgotCreatureBreak"; psql -h 45.148.28.196 -U "raz@tovtech.org" -d TovPlay -t -c "SELECT pg_current_wal_lsn();" 2>/dev/null | xargs' 10)
    WAL_SIZE_BYTES=$(ssh_prod 'export PGPASSWORD="CaptainForgotCreatureBreak"; psql -h 45.148.28.196 -U "raz@tovtech.org" -d TovPlay -t -c "SELECT pg_wal_lsn_diff(pg_current_wal_lsn(), '"'"'0/0'"'"');" 2>/dev/null | xargs' 10)

    if [ -n "$WAL_POS" ] && [ -n "$WAL_SIZE_BYTES" ]; then
        WAL_MB=$((${WAL_SIZE_BYTES:-0} / 1024 / 1024))
