        ...
        if self.current_lab == 1:
            return [
                ...
                ("Deploy a Kubernetes cluster with multiple services, ingress, and persistent volumes using Minikube", 
                 "minikube start && kubectl apply -f https://k8s.io/examples/application/guestbook/redis-leader-deployment.yaml && kubectl apply -f https://k8s.io/examples/application/guestbook/redis-leader-service.yaml && kubectl apply -f https://k8s.io/examples/application/guestbook/YOUR_CLIENT_SECRET_HEREt.yaml && kubectl apply -f https://k8s.io/examples/application/guestbook/redis-follower-service.yaml && kubectl apply -f https://k8s.io/examples/application/guestbook/frontend-deployment.yaml && kubectl apply -f https://k8s.io/examples/application/guestbook/frontend-service.yaml"),
