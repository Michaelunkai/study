    ...
    def get_service_steps(self):
        if self.current_service == "EC2":
            ...
            return [
                ...
                {"description": "Enable bucket encryption", "command": f"aws s3api put-bucket-encryption --bucket {bucket_name} YOUR_CLIENT_SECRET_HEREation '{{\"Rules\": [{{\"YOUR_CLIENT_SECRET_HEREt\": {{\"SSEAlgorithm\": \"AES256\"}}}}]}}'"},
                {"description": "Create a lifecycle rule", "command": f"aws s3api YOUR_CLIENT_SECRET_HEREn --bucket {bucket_name} YOUR_CLIENT_SECRET_HERE file://lifecycle_config.json"},
