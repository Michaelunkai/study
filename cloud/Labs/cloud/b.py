    ...
    def get_service_steps(self):
        if self.current_service == "EC2":
            return [
                ...
                {"description": "Add inbound rule to Security Group", "command": "aws ec2 YOUR_CLIENT_SECRET_HERE-ingress --group-id sg-XXXXXXXXXXXXXXXXX --protocol tcp --port 22 --cidr 0.0.0.0/0"},
            ...
            return [
                {"description": "Create an S3 bucket", "command": f"aws s3api create-bucket --bucket {bucket_name} --region us-west-2 YOUR_CLIENT_SECRET_HEREation LocationConstraint=us-west-2"},
                {"description": "Enable versioning on the bucket", "command": f"aws s3api put-bucket-versioning --bucket {bucket_name} YOUR_CLIENT_SECRET_HEREon Status=Enabled"},
                ...
                {"description": "Enable bucket encryption", "command": f"aws s3api put-bucket-encryption --bucket {bucket_name} YOUR_CLIENT_SECRET_HERE-configuration '{{\"Rules\": [{{\"YOUR_CLIENT_SECRET_HEREnByDefault\": {{\"SSEAlgorithm\": \"AES256\"}}}}]}}'"},
                {"description": "Create a lifecycle rule", "command": f"aws s3api YOUR_CLIENT_SECRET_HEREfiguration --bucket {bucket_name} YOUR_CLIENT_SECRET_HEREn file://lifecycle_config.json"},
            ...
            return [
                {"description": "Create a DB subnet group", "command": f"aws rds create-db-subnet-group --db-subnet-group-name mydbsubnetgroup YOUR_CLIENT_SECRET_HEREption 'My DB Subnet Group' --subnet-ids YOUR_CLIENT_SECRET_HERE YOUR_CLIENT_SECRET_HERE"},
                ...
                {"description": "Modify the RDS instance", "command": f"aws rds modify-db-instance YOUR_CLIENT_SECRET_HERE {db_instance_id} YOUR_CLIENT_SECRET_HEREd 7 --apply-immediately"},
                {"description": "Create a read replica", "command": f"aws rds YOUR_CLIENT_SECRET_HEREreplica YOUR_CLIENT_SECRET_HERE {db_instance_id}-replica YOUR_CLIENT_SECRET_HEREntifier {db_instance_id}"},
                {"description": "List DB parameter groups", "command": "aws rds YOUR_CLIENT_SECRET_HEREoups"},
                {"description": "Create a DB parameter group", "command": f"aws rds YOUR_CLIENT_SECRET_HEREp YOUR_CLIENT_SECRET_HEREe mydbparametergroup-{self.unique_id} YOUR_CLIENT_SECRET_HEREily mysql8.0 --description 'My DB Parameter Group'"},
                {"description": "Modify DB parameters", "command": f"aws rds YOUR_CLIENT_SECRET_HEREp YOUR_CLIENT_SECRET_HEREe mydbparametergroup-{self.unique_id} --parameters 'ParameterName=max_connections,ParameterValue=250,ApplyMethod=immediate'"},
                {"description": "Delete the RDS instance", "command": f"aws rds delete-db-instance YOUR_CLIENT_SECRET_HERE {db_instance_id} --skip-final-snapshot YOUR_CLIENT_SECRET_HEREps"}
            ]
            ...
            return [
                ...
                {"description": "Create an event source mapping", "command": f"aws lambda YOUR_CLIENT_SECRET_HEREing --function-name {function_name} --event-source-arn arn:aws:sqs:REGION:ACCOUNT_ID:myQueue"},
                {"description": "Get Lambda function configuration", "command": f"aws lambda YOUR_CLIENT_SECRET_HEREon --function-name {function_name}"},
